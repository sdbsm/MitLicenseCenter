...
public static class SettingDefinitions
{
    public static readonly IReadOnlyDictionary<string, SettingDefinition> All =
        new Dictionary<string, SettingDefinition>(StringComparer.Ordinal)
        {
            ...
            // MLC-231 (60_SAFETY №3/№4): безопасный сбор ТЖ. Окно авто-снятия — короткий дефолт по
            // политике безопасности (полный ТЖ забивает диск за минуты, MLC-229; фильтр длительности
            // объём не страхует). Сбор всегда снимается сам, даже если оператор забыл остановить.
            [SettingKey.TechLogMaxDurationMinutes] = new(
                SettingKey.TechLogMaxDurationMinutes,
                IsSecret: false,
                Description: "Окно сбора технологического журнала, минуты. По истечении панель сама снимает сбор (авто-стоп), даже если оператор забыл остановить. Короткое окно ограничивает объём на диске.",
                Kind: SettingValueKind.Number,
                DefaultValue: "10",
                Min: 1,
                Max: 1440),

            // Потолок размера каталога сбора: при достижении — авто-стоп (DiskLimit). Лимит места
            // критичнее обычного — фильтр длительности в logcfg для JSON-ТЖ 8.5 не работает (MLC-229),
            // объём режется только типом события и арендатором, полный ТЖ = ~1.4 ГБ за минуты.
            [SettingKey.TechLogDiskLimitMb] = new(
                SettingKey.TechLogDiskLimitMb,
                IsSecret: false,
                Description: "Потолок размера каталога сбора технологического журнала, МБ. При достижении сбор автоматически останавливается (защита диска от переполнения).",
                Kind: SettingValueKind.Number,
                DefaultValue: "2048",
                Min: 64,
                Max: 1048576),

            // Порог свободного места на диске каталога сбора перед стартом: меньше — сбор не запускается
            // (структурный отказ InsufficientDiskSpace). 0 — проверку отключить.
            [SettingKey.TechLogMinFreeDiskMb] = new(
                SettingKey.TechLogMinFreeDiskMb,
                IsSecret: false,
                Description: "Минимум свободного места (МБ) на диске каталога сбора, без которого сбор технологического журнала не стартует (защита от переполнения диска). 0 — проверку отключить.",
                Kind: SettingValueKind.Number,
                DefaultValue: "1024",
                Min: 0,
                Max: 1048576),

            // MLC-247 A2 (41_LOGCFG_SPEC §6): аккаунт процессов 1С/агента кластера, который ПИШЕТ ТЖ под
            // своим именем и должен иметь полные права на каталог сбора (и \dumps). Панель лишь создаёт
            // каталог — без гранта агенту платформа молча не пишет ТЖ («пустые дела»). Пусто (дефолт) —
            // панель не знает аккаунт: установка не блокируется, лишь предупреждение с шаблоном icacls.
            // Заполнить аккаунтом процессов кластера (например, .\mitpro или доменным), затем проба прав
            // (паттерн RAS-healing) на установке детектит отсутствие Modify и отдаёт точную команду icacls.
            [SettingKey.TechLogCollectionAgentAccount] = new(
                SettingKey.TechLogCollectionAgentAccount,
                IsSecret: false,
                Description: "Windows-аккаунт процессов 1С (агент кластера), который пишет файлы технологического журнала. Ему нужны полные права на каталог сбора — иначе платформа молча не пишет ТЖ. Пример: .\\mitpro или доменный аккаунт. Пусто — проверка прав пропускается (выдаётся предупреждение с шаблоном команды).",
                Kind: SettingValueKind.Text),
