...
/// <summary>
/// Группа похожих SQL-запросов (агрегат по нормализованному тексту, MLC-234/248).
...
public sealed record SlowQueryGroup
{
    ...
    // Число вхождений формы запроса по ВСЕМ DBMSSQL с непустым Sql (MLC-248: включая под-пороговые).
...
public sealed class SlowQueryAnalysisResult
{
    ...
    // Группы похожих запросов по нормализованному тексту Sql (MLC-248: по ВСЕМ DBMSSQL с непустым Sql,
    // НЕЗАВИСИМО от порога TopQueries — «много мелких в сумме»). Отсортированы по суммарной длительности
    // убывающим, ограничены topN. Запросы без поля Sql в группировку не входят (нечего нормализовать, §7).
    ...
    // ─── MLC-252 B: счётчик покрытия привязки контекста (корреляция SQL↔CALL) ──────────────────────
    // GroupsTotal — сколько групп похожих запросов всего (== SimilarGroups.Count); GroupsWithContext —
    // скольким из них удалось атрибутировать контекст 1С (SampleContext != null). На стенд-приёмке 1.2
    // (параллельные нагрузки) контекст не привязался ни к одной группе — без явного счётчика пустота
    // молчала. FE показывает «контекст привязан к N из M групп», чтобы оператор видел: это не «нет
    // данных», а «корреляция не сцепилась» (нет CALL с контекстом на том же соединении).
    public int GroupsTotal { get; init; }

    public int GroupsWithContext { get; init; }
}
