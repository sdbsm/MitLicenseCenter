...
public interface ILogcfgStore
{
    ...
    // Потоковое чтение сырья ТЖ из каталога сбора (MLC-238, конвейер анализа). Перечисляет строки всех
    // NDJSON-файлов каталога (рекурсивно), файл за файлом, ленивым перечислением — память не растёт на
    // больших журналах (40_TECHLOG §4). Never-throws: каталога нет / ошибка чтения файла → пропуск (пустая
    // последовательность, если читать нечего). Seam для детерминированных тестов через FakeLogcfgStore.
    IEnumerable<string> ReadCollectionLines(string directory);

    // Удаление сырья ТЖ из каталога сбора (MLC-238, решение MLC-237 Q2: сырьё по умолчанию удаляется после
    // успешного анализа — Finding'и несут разобранный результат). Идемпотентно и never-throws: каталога
    // нет / файл занят → no-op (best-effort). Seam для тестов (проверяют факт вызова).
    void DeleteCollectionFiles(string directory);
