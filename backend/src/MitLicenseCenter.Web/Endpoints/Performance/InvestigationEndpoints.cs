...
public static class InvestigationEndpoints
{
    ...
    private static ReportItem BuildReportItem(Finding f)
    {
        ...
        var (headline, recommendation) = f.Kind switch
        {
            ...
            FindingKind.Call => (
                "Серверные вызовы 1С",
                "Обнаружены серверные вызовы 1С с заметным временем выполнения. Это время на стороне 1С "
                + "(вычисления между запросами к СУБД). Сгруппируйте вызовы по контексту: повторяющийся "
                + "контекст с большим суммарным временем — кандидат на оптимизацию серверного кода."),
