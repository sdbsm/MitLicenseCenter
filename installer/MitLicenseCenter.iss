...
; (под выбранным ОС-аккаунтом или LocalSystem). Аккаунт/логин с правами в SQL (db_owner на
; базе панели + VIEW SERVER STATE; sysadmin НЕ нужен, MLC-152) и саму базу создаёт оператор
; ЗАРАНЕЕ (scripts/sql-setup-least-privilege.sql) — установщик их потребляет и проверяет, не создаёт.
;
...
begin
  ...
  PageAuthMode := CreateInputOptionPage(PageSql.ID,
    ...
    'Учётную запись с правами в SQL (db_owner на базе панели + VIEW SERVER STATE; sysadmin не нужен) и базу оператор готовит ЗАРАНЕЕ скриптом sql-setup-least-privilege.sql — установщик их только использует, не создаёт.',
    True, False);
  ...
  begin
    ...
    begin
      ...
      PageCreds.SubCaptionLabel.Caption :=
        ...
        '1. SQL: аккаунту достаточно db_owner на базе панели + VIEW SERVER STATE (sysadmin НЕ нужен; ' +
        'база и права готовятся скриптом scripts\sql-setup-least-privilege.sql). ' +
