...
interface BulkUnpublishDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  publications: PublicationListItem[];
  /** Снять успешно снятые с публикации из выделения (упавшие/пропущенные остаются для повтора). */
...
// MLC-184b: массовое снятие публикаций из IIS через webinst -delete. Действие обратимо
// (публикацию можно создать заново) → по ADR-45 «да/нет» с явным N, БЕЗ пометки
// необратимости и БЕЗ type-to-confirm. Кнопка подтверждения — destructive-стиль (как в
// UnpublishPublicationDialog). Пачка исполняется пулом; по завершении инвалидируется список.
export function BulkUnpublishDialog({
  open,
  onOpenChange,
  publications,
  onRunComplete,
  filterSummary,
}: BulkUnpublishDialogProps) {
  ...
  const runItem = useCallback(
    (id: string) =>
      api<PublicationStatusResponse>(`/api/v1/publications/${id}/unpublish`, {
        method: "POST",
      }).then(() => undefined),
    []
  );
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent showCloseButton={phase !== "running"}>
        <DialogHeader>
          <DialogTitle>{t("publications.bulk.unpublish.title")}</DialogTitle>
          <DialogDescription>
            {t("publications.bulk.unpublish.body", { count: publications.length })}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          {phase === "idle" && (
            <>
              ...
              >
                {t("publications.bulk.unpublish.confirm", { count: publications.length })}
              </Button>
            </>
          )}
          {phase === "running" && (
