...
/**
 * Тесты режима «Расследование» (MLC-242, ADR-57):
 *   - Мастер запуска (InvestigationWizard, экран 2)
...
describe("InvestigationMode — переключение по статусу активного дела (MLC-242)", () => {
  beforeEach(() => {
  ...
  it("без активного дела рендерится Мастер запуска", () => {
    renderMode();
    // Мастер содержит заголовок и группу выбора сценария
    expect(screen.getByText("Новое расследование")).toBeInTheDocument();
  });

  it("при активном деле (Collecting) рендерится Прогресс, а не Мастер", () => {
    ...
    expect(screen.queryByText("Новое расследование")).not.toBeInTheDocument();
  });
  ...
  it("завершённое дело (Completed) не считается активным → показывает Мастер", () => {
