...
> **`MLC-087` + `MLC-088` закрыты (2026-06-10)** — сессия 1 трека «Этап 2: single-host бек-чистка»
> выполнена (один PR, коммиты раздельно). Следующая сессия трека — `MLC-089` + `MLC-090` (PR кода),
> затем `MLC-091` (канон+ADR). Назначение `NEXT TASK` — за куратором.
