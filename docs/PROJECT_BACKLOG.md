...
**`MLC-159` · Backend · P2 · Medium · Module: Clusters/RAS** — обнаружение службы RAS +
адаптер управления её жизненным циклом.

- **Что:** новый порт `IRasServiceManager` (Infrastructure) поверх `sc.exe` — обнаружение
  службы Windows по `binPath`, содержащему `ras.exe` (имя службы не стандартизировано),
  чтение состояния (running/stopped) + версии платформы из пути + порта из аргументов;
  операции `register` / `update` / `start`. Discovery `ras.exe` — в тех же bin-каталогах,
  что и `rac.exe` (переиспользовать `OneCInstallRoots` + `PlatformVersionDiscovery`).
- **API:** `GET /api/v1/ras-service/status` (диагностика из 4 состояний — ОК / нет службы /
  устарела / остановлена, Admin), `POST /api/v1/ras-service/{register|update|start}`
  (Admin, аудит). Порт службы = `OneC.RAS.Endpoint` (единый источник); хост `localhost`.
- **Аудит:** новая 600-серия (frozen-int) — `RasServiceRegistered=600`,
  `RasServiceUpdated=601`, `RasServiceStarted=602` (603+ резерв) + i18n-ключи действий.
- **Гочи:** синтаксис `ras.exe`/`sc create` под 8.5.x **сверить с офиц. докой 1С** (cluster +
  service + порт + локальный ragent); OEM-декод вывода `sc.exe` как у `rac.exe`/`iisreset.exe`;
  экранирование кавычек/`]` в `binPath`. Канон — **ADR-47**. Спека — план-файл
  `.claude/plans/ras-service-autoregister.md`.

> Трек «Авто-регистрация службы RAS» (`MLC-159..161`, релиз `0.6.0-beta`). Порядок:
> `MLC-159` (BE) → `MLC-160` (FE: блок в Настройках) → `MLC-161` (FE: сигнал дашборда +
> бамп версии). Постановка и архитектура — ADR-47 + `ROADMAP.md` (релиз `0.6.0-beta`).

### Трек «Авто-регистрация службы RAS» — `MLC-159..161` (релиз `0.6.0-beta`)

- `MLC-159` — **NEXT TASK** (см. выше). BE: обнаружение + адаптер `IRasServiceManager` +
  эндпоинты `/ras-service/*` + аудит 600-серии.
- `MLC-160` — FE: блок диагностики RAS (4 состояния) + лечение в секции «Подключение к 1С/RAS»
  (`/settings`, Admin-only); диалоги-подтверждения показывают человеческое описание **и**
  точную команду `sc …` (стиль ADR-45); Zod parity + omit-null тест; статусы только через
  `StatusBadge`.
- `MLC-161` — FE: карточка здоровья RAS на дашборде сигналит «не зарегистрирована»/«остановлена»
  ...
  `0.6.0-beta` (`frontend/package.json` + `README.md`) + ROADMAP release-секция → «выпущен».
