...
> **`MLC-133` — R11d-хвост (FE-09): Zod на `useUsers` и `useUnassignedInfobases`.**
> Frontend-only · P3 · Small. Завершает FE-09/R11 (основная часть закрыта `MLC-132`). Остались два хука
> без схем:
> - `frontend/src/features/users/useUsers.ts` (~5 raw-кастов: список пользователей + мутации) — схема для
>   read-ответа (список/деталь пользователя) в `users/types.ts`.
> - `frontend/src/features/infobases/unassigned/useUnassignedInfobases.ts` (~3 каста) — схема ответа
>   `/infobases/unassigned` (`{ items, missingItems, ... }`) в `unassigned/types.ts`.
> Те же гочи: **omit-null** → `omittable()`/`.nullish()`; **enum-строки** → `z.enum().or(transform)`; типы
> `z.infer`. Образец — любой `types.ts`, покрытый в MLC-132 (напр. `reports/types.ts`, `settings/types.ts`).
> Мутации с тривиальным/echo-телом можно пропустить (отметить). Обновить ассерты затронутых тестов под
> добавленный `{ schema }` (как в MLC-132). Канон 05 §5.3 — дописать эти границы.
> Критерии готовности: read-границы обоих хуков валидируются Zod; `build.ps1` зелёный; канон обновлён.
> **После MLC-133 R11 закрыт полностью.** Далее — R12 (полировка UX, многокластерная: терминология/тексты,
> доступность WCAG AA, онбординг, FE-07/08 сброс диалогов, BE-02 optimistic concurrency [нужна миграция],
> BE-13 freeze-тесты enum с `HasConversion<int>`). Исполнитель — субагент (worktree); модель выбирает куратор.
> _Источник детализации — `ROADMAP.md` R11/R12; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..132` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-133` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-134`.** Куратор инкрементирует при выдаче; выдавать занятые
