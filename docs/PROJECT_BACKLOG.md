...
**`MLC-213` · Backend · P2 · Агрегатор статусов служб узла + эндпоинты.** Поверх контроллера
`IWindowsServiceController` (MLC-212) — read-модель статуса всех служб стека: discovery сервера 1С
(`ragent`, при нескольких версиях платформы — списком), RAS (переиспользовать `IRasServiceManager`
discovery), статус SQL Server (**только наблюдение** — управление не давать, ADR-54), сводка IIS
(переиспользовать `IIisLifecycleService`) + общий индикатор здоровья узла. Эндпоинты группы
`/api/v1/server/*` (server-scope, `tenantId:null`): **read** для Viewer (статусы), **мутирующие**
(старт/стоп/рестарт сервера 1С/RAS/IIS) — для Admin, через `IWindowsServiceController`; маппинг
`WindowsServiceOperationException → 409` (по образцу `RAS_SERVICE_OPERATION_FAILED`). Аудит — 800-серия
(`OneCServerStarted/Stopped/Restarted = 800..802`, объявлена в MLC-212; писать здесь через
`EndpointHelpers.AuditAsync`). **Доступ:** паттерн «Параметров»/«Пользователей» (Viewer видит, Admin
управляет). **Без FE** (его даёт `MLC-214`). **Приёмка:** unit/endpoint-тесты на агрегат статусов,
ролевой доступ и маппинг 409; локальный `build.ps1` зелёный. Якоря-образцы — в секции трека ниже.
...
- **`MLC-213`** (NEXT) · BE · агрегатор статусов всех служб (полная постановка — в «NEXT TASK» выше).
