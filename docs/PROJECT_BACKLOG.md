...
|---|---|---|---|---|---|
| `MLC-108` | Maintainability | P1 | High | Детерминированный состав релизного артефакта: чистка OutputDir в `publish-release.ps1` + sanity-чек состава в `build-installer.ps1` (REL-01; в Setup.exe фактом уехали 82 чужих файла) | Done (2026-06-12) |
| `MLC-109` | Security | P1 | High | Немедленный отзыв доступа: SecurityStampValidator (1–5 мин) + `UpdateSecurityStampAsync` в disable/reset/смене роли; self-смена пароля не разлогинивает (SEC-01) | Done (2026-06-12) |
| `MLC-110` | Security | P1 | High | Секреты на диске: ACL `%ProgramData%\MitLicenseCenter` + `appsettings.Production.json` (оба режима установки), `CryptographicException` → понятная ошибка, честный ADR-8 «ключи переносимы, защита ACL» (KEYRING-01(в) + SEC-04 + SEC-02) | Done (2026-06-13) |
| `MLC-111` | Frontend | P1 | High | Форма привязки базы: убрать предвыбор `tenants[0]`, пустое значение + placeholder; `defaultTenantId` сохраняет предвыбор (UX-42) | Done (2026-06-13) |
| `MLC-112` | Maintainability | P1 | High | Страховка апгрейда: экран «бэкап БД + key ring» до замены файлов (только апгрейд); провал старта службы = явная ошибка мастера, не «успех» (REL-02; сверить с уже сделанным в MLC-107) | Done (2026-06-13) |
| `MLC-113` | Backend | P1 | High | Снятие публикации из IIS — полная реализация: unpublish за ADR-20-границей, `POST /publications/{id}/unpublish` (Admin), UI «Снять публикацию» + опция в диалоге удаления ИБ, новый замороженный int аудита (UX-43) | Done (2026-06-13) |
| `MLC-114` | Backend | P2 | Medium | Пакет валидации/наблюдаемости: runtime-диапазон `MaxConcurrentLicenses` (BE-03), per-item catch в `PublicationStatusRefreshJob` (BE-05), аудит неудачных входов — новый int (BE-10), валидация символов логина BE+FE: 500→400 (UX-01-остаток) | Done (2026-06-13) |
...
Done-строки трека (передаточные записки; полные отчёты — в архиве):
- `MLC-108` — Done (2026-06-12, PR #119): `publish-release.ps1` чистит/пересоздаёт OutputDir
  перед publish (отказ для пустого пути и корня диска); `build-installer.ps1` падает ДО ISCC,
  если в publish-каталоге есть `*.deps.json`/`*.runtimeconfig.json` (чек работает и при
  `-SkipPublish`). `.iss` по-прежнему пакует каталог целиком — детерминированность держится
  на чистке + sanity-чеке.
- `MLC-109` — Done (2026-06-12, PR #121): SecurityStampValidator подключён к `OnValidatePrincipal`
  (интервал ревалидации 2 мин; сам валидатор + опции регистрируются явно в Infrastructure/DI —
  `AddIdentityCore` их не ставит); `UpdateSecurityStampAsync` в disable/смене роли (reset
  ротирует stamp внутри Identity, покрыто тестом); self-смена пароля живёт за счёт уже
  существовавшего `RefreshSignInAsync`. **Гоча:** ручной cookie-пайплайн обязан регистрировать
  no-op куку `IdentityConstants.TwoFactorRememberMeScheme` — отзыв делает sign-out и по ней,
  без регистрации первый же реальный отзыв падал бы исключением.
- `MLC-110` — Done (2026-06-13, PR #123): `.iss` ужесточает ACL в [Code] ssPostInstall в ОБОИХ
  режимах (icacls `/inheritance:r`, SID-формы `*S-1-5-18`/`*S-1-5-32-544`; каталог ProgramData —
  ПОСЛЕ записи `initial-admin.secret`, конфиг — сразу после генерации; сбой icacls =
  предупреждение, не abort); `SettingsStore` оборачивает `CryptographicException` в операторскую
  `InvalidOperationException` (+2 теста); ADR-8 честный («ключи plaintext + NTFS ACL»).
  **Гочи:** в Pascal-комментариях `.iss` нельзя писать `{app}` (закрывает комментарий);
  ADR-21/CLAUDE.md всё ещё говорят «DPAPI» — терминологию выровняет D1. Стендовая
  icacls-проверка обоих режимов — в кураторском прогоне Setup.exe после трека.
- `MLC-111` — Done (2026-06-13, PR #127): в `useInfobaseForm.ts` убран fallback
  `tenants[0]` в `defaultValues` (frontend-only, одна строка) — при создании вне контекста
  карточки клиента `tenantId` пустой, срабатывают существующие placeholder «Выберите
  клиента» и required-валидация (zod `min(1)+GUID`); `defaultTenantId` предвыбор сохраняет.
  `tenants` убран из деструктуризации хука (стал неиспользуем), но остаётся в контракте
  `UseInfobaseFormArgs` — диалог рендерит из него список. Dialog/validation/i18n не тронуты.
  Тесты `useInfobaseForm`: пустой `tenantId` без `defaultTenantId`, ошибка поля + отсутствие
  POST при submit без выбора, предвыбор при `defaultTenantId`.
- `MLC-112` — Done (2026-06-13, PR #129): только `installer/MitLicenseCenter.iss`.
  (1) Новая страница-предупреждение `PageUpgradeBackup` (`CreateInputOptionPage`, один
  чекбокс) — обязательное подтверждение «сделал бэкап БД SQL + key ring `%ProgramData%\
  MitLicenseCenter`» с гейтом в `NextButtonClick`; показывается ТОЛЬКО на апгрейде
  (`ShouldSkipPage` → `not ServiceExists`), на чистой установке пропущена. **Гоча:** из-за
  обратного порядка вставки страниц Inno (одинаковый `AfterID=wpSelectDir`) страница
  фактически встаёт ПОСЛЕДНЕЙ перед «Готово к установке» — это и нужно (свежее напоминание
  перед заменой файлов). (2) Флаг `ServiceStartFailed` ставится в `ConfigureService` на
  провал `sc start` (rc<>0); `CurPageChanged(wpFinished)` при нём перекрашивает финал в
  ОШИБКУ (`FinishedHeadingLabel`=«…с ОШИБКОЙ», `FinishedLabel.Font.Color:=clRed`, ссылка на
  Журнал событий / services.msc / восстановление из бэкапа) — три success-ветки обёрнуты в
  `not ServiceStartFailed`. Покрывает и апгрейд, и чистую установку. RaiseException/Abort на
  провал старта НЕ добавляли (по постановке: явный экран ошибки, не откат посреди апгрейда);
  ветка `sc create`-провала (чистая, с `RaiseException`) не тронута. Юнит-тестов для `.iss`
  нет — гарантия машинная: ISCC `Successful compile`. Ручной прогон апгрейда со сломанным
  стартом — в кураторском прогоне Setup.exe после трека.
- `MLC-113` — Done (2026-06-13, PR #131): снятие IIS-публикации через `webinst -delete`
  (симметрично publish, за ADR-20-границей). `IWebinstPublisher.UnpublishAsync` →
  `OneCWebinstPublisher` общим конвейером `RunWebinstAsync` (publish/unpublish одним путём,
  отличие — builder `WebinstArgs.BuildUnpublish`: `-delete -iis -wsdir -dir -connstr`; по
  доке 1С для `-delete` достаточно `-wsdir`, остальное «to control the operation»).
  `POST /publications/{id}/unpublish` (Admin) — зеркаль publish без confirm-гейта (токен в UI),
  сбой webinst → 409 `UNPUBLISH_FAILED` без аудита, успех → `RefreshOneAsync` (статус
  NotPublished); `Source` НЕ сбрасывается. Удаление ИБ: `DELETE /infobases/{id}?unpublishFromIis=true`
  снимает из IIS ДО удаления строк, при сбое 409 и **ничего не удаляет** (защита от сиротства —
  суть UX-43). Аудит — новый замороженный `PublicationUnpublished = 23` (группа Publication
  20–22; freeze-тест покрывает автоматически). UI: пункт «Снять публикацию» + `UnpublishPublicationDialog`
  (подтверждение токеном), чекбокс «также снять из IIS» в диалоге удаления ИБ (отмечен при
  Published, скрыт в диалоге обратного дрейфа). **Гоча для FE:** новое активно-пишущееся
  аудит-действие требует зеркала в `features/audit/types.ts` (union + `AUDIT_ACTION_TYPES`) —
  иначе парити-тест `auditActionTypes.test.ts` красный. Локальный гейт: BE 649/0, FE 364/0,
  type-check/lint/format чисто.
- `MLC-114` — Done (2026-06-13, PR #133): четыре независимых фикса. **BE-03** — ручной
  диапазон `0 ≤ MaxConcurrentLicenses ≤ 100_000` в `TenantsEndpoints` (`ValidateName`→
  `ValidateTenant`, оба хендлера) → 400 (DataAnnotations `[Range]` в minimal API в runtime
  не прогоняются; ≤0 молча отключал enforcement тенанта). Поле тенанта, **не** инфобазы →
  parity не нужен. **BE-05** — per-item try/catch вокруг `ProcessOneAsync` в
  `PublicationStatusRefreshJob` (`OperationCanceledException` пробрасывается, остальное —
  `LogStatusItemFailed` по Id и цикл продолжается; + detach «отравленного» probe-attach после
  неуспешного SaveChanges, иначе всплыл бы на следующем элементе). **BE-10** — новый
  замороженный `LoginFailed = 108` (100-серия, 107 был максимум); аудит на `!result.Succeeded`
  вкл. lockout, initiator = введённое имя, **пароль в описание не пишется**; BE-freeze
  `[InlineData(LoginFailed,108)]` + FE-зеркало (`types.ts` union+`AUDIT_ACTION_TYPES`, `ru.json`).
  **UX-01-остаток** — логин с пробелом раньше доходил до `PasswordSignInAsync` и падал 500 в
  пайплайне Identity; BE режет по whitelist `AllowedUserNameCharacters` ДО sign-in → 400, FE
  zod `.regex` тем же набором (логин-scoped, к infobase-parity отношения нет). Гейт: BE 658/0,
  FE 364/0, type-check/lint/format чисто (проверено куратором независимо в worktree).
...
> **`MLC-115` — Пакет поставки/тестов (REL-10 + FE-01).** Последняя задача трека.
> (1) **REL-10:** `THIRD_PARTY_LICENSES.txt` в поставку — добавить файл в собранный артефакт
> через `scripts/publish-release.ps1` (и/или `installer/MitLicenseCenter.iss`): Hangfire —
> **LGPL v3** (вшить тексты GPL/LGPL), SheetJS/`xlsx` — **Apache 2.0**, перечень остальных
> MIT-зависимостей. Список сгенерировать по `backend/Directory.Packages.props` (prod-пакеты) +
> `frontend/package.json` (prod-`dependencies`, не dev). Гоча `MLC-108`: `publish-release.ps1`
> чистит/пересоздаёт OutputDir перед publish, а `build-installer.ps1` падает на `*.deps.json`/
> `*.runtimeconfig.json` — новый файл лицензий не должен ронять sanity-чек.
> (2) **FE-01:** тестовое покрытие фичи profile (сейчас 0%, регресс запирает пользователя):
> happy-path смены пароля + цепочка `onSuccess → invalidateQueries(ME_KEY)`, снимающая
> блокирующий экран `ForcePasswordChange`. Тесты Vitest.
> Критерии: файл лицензий присутствует в составе publish (проверить перечень publish);
> тесты profile покрывают happy-path смены пароля и снятие ForcePasswordChange-экрана.
> `OPERATIONS.md`/`SECURITY.md` НЕ трогать (перепишет D1). Полная постановка —
> `.claude/plans/audit-fix-prerelease.md` (MLC-115); findings — `audit/2026-06/MASTER-REPORT.md`.
> Исполнитель — субагент (opus, worktree); куратор ревьюит, гонит тесты, вливает; после
> закрытия `MLC-115` трек завершён → куратор схлопывает секцию в архив и запускает D1.
