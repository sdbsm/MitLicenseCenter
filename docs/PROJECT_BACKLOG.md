...
**MLC-227 · Backend/Installer · P1 · High · Устойчивость службы · `installer/include/code-service-sql.iss`**

**Проблема.** Установщик прописывает службе `MitLicenseCenter` жёсткую SCM-зависимость от локальной
SQL-службы (`sc config … depend= MSSQLSERVER|MSSQL$NAME`, REL-03/ADR-40). Побочный эффект Windows:
ручная остановка SQL (штатное обслуживание) каскадно гасит и панель, а при обратном старте SQL
зависимые службы Windows сам **не** поднимает → панель остаётся выключенной (запускать её вручную
никто не будет). При этом БД панели лежит на том же SQL, и панель умеет переподключаться сама
(EF `EnableRetryOnFailure(3)`, см. OPERATIONS «SQL недоступен в рантайме» — перезапуск службы не
нужен). Зависимость противоречит этой модели и сломала ровно прод-сценарий обслуживания SQL.
Подтверждено на стенде (служба `Stopped` при `MSSQLSERVER Running`).

**Решение.** Убрать зависимость; устойчивость остаётся на recovery-политике (`sc failure` — основной
механизм по ADR-40, покрывает все размещения SQL, включая удалённый). Установщик:
- больше **не** выводит и не прописывает `depend=` — удалить `ConfigureSqlDependency` и ставшую
  неиспользуемой `DeriveLocalSqlServiceName`, снять вызов в `StartServiceAndFinalize`;
- **активно снимает** уже выставленную зависимость и на чистой установке, и на апгрейде:
  `sc config {#MyServiceName} depend= /` (best-effort: rc<>0 → `Log` + предупреждение, без `Abort`),
  иначе обновление не вылечит существующие стенды.

**Канон (в том же PR, doc-driven).** Amend **ADR-40** (производная SQL-зависимость отозвана: причина —
каскадная остановка при ручном стопе SQL; единственный механизм устойчивости — recovery-политика);
**OPERATIONS**, раздел «SQL недоступен в рантайме» — служба больше не зависит от SQL + команда
лечения уже установленных стендов `sc config MitLicenseCenter depend= /` (без простоя, действует со
следующего старта службы).

**Релиз.** Патч `1.1.4`: бамп `<Version>` в `backend/Directory.Build.props` +
`artifacts/1.1.4/RELEASE_NOTES.md` (формат как 1.1.3). Тег `v1.1.4` и публикация GitHub Release — за
владельцем (ADR-51).

**Гейт.** `scripts/build.ps1` зелёный; ISCC-компиляция `.iss` — за куратором перед слиянием
(у исполнителя ISCC нет). Без миграций/FE/parity/enum.

**За владельцем (приёмка на стенде):** после обновления на 1.1.4 — остановить SQL вручную (панель
должна остаться `Running` и показывать недоступность SQL), затем запустить SQL (панель отвечает сама,
без ручного старта службы); проверить `sc qc MitLicenseCenter` → `DEPENDENCIES` пусто.
...
`PROJECT_BACKLOG_ARCHIVE.md`. Активного трека нет; следующую работу ставит куратор постановочным PR.

**Следующий свободный номер: `MLC-228`.** (`MLC-227` — отзыв SCM-зависимости службы от SQL, активна; `MLC-222..226` — хотфиксы трека «Сервер» по приёмке на стенде: DI-lifetime старта, квотирование `sc`, таймаут верификации, полная остановка перед стартом, единицы памяти rphost; см. архив.)
