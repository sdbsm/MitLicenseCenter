...
(ревизия канона) в основном; частично R3/R4/R5/R7/R8 (отмечено в ROADMAP значком ✅).
...
> **`MLC-124` — R8: Релизный конвейер.** Backend/Tooling · P3 (DOC-08 — P2) · Medium.
> Зависимость: независима (параллельна R9). Цель — воспроизводимая, автоматически проверяемая
> сборка релизного артефакта; правки в скриптах (`scripts/publish-release.ps1`,
> `scripts/build-installer.ps1`), установщике (`.iss`), CI-конфиге и шаблоне конфигурации.
> Состав (находки `audit/2026-06/TECH-DEBT.md`):
> - **DOC-08 (P2):** в шаблоне `appsettings.Production.json` (для ручного деплоя по
>   `OPERATIONS`) нет ключа `Urls` — следование инструкции даёт деплой на дефолтном
>   `localhost:5000` без предупреждения. Добавить `Urls` в шаблон (GUI-инсталлятор пишет его
>   сам — не регресснуть его путь).
> - **REL-13:** при апгрейде старые хэшированные ассеты SPA (`wwwroot/assets/*-<hash>.js`) не
>   удаляются — мусор у заказчика + риск трудно-диагностируемого белого экрана. Чистить
>   `wwwroot/assets` перед раскладкой новой версии (установщик `[InstallDelete]`/`[Code]` либо
>   publish-скрипт), не задев key ring/конфиг/БД.
> - **REL-20:** убрать захардкоженный личный путь к ISCC (`C:\Users\andre\...`) из трекаемого
>   `build-installer.ps1` — параметр/детект/переменная окружения, чтобы скрипт работал на
>   чужой машине.
> - **REL-12:** CI-гейт релизного пути (`publish` → ISCC → `Setup.exe`) — сейчас `.iss` не
>   валидируется автоматически, несобираемость всплыла бы в день поставки. Авторинг workflow.
> - **REL-14/REL-21:** гигиена зависимостей/сборки — dependabot, `pnpm audit` /
>   `dotnet list package --vulnerable`, `format:check` в CI, paths-фильтры в Actions,
>   открепить .NET SDK от патча сборочной машины (SDK-чеклист перед релизом).
> Критерии готовности: ручной прогон релизного пути воспроизводим на чужой машине (нет личных
> путей); апгрейд не оставляет старых ассетов; шаблон конфигурации даёт рабочий деплой;
> CI-конфиг релизного гейта + гигиены зависимостей **закоммичен**; **canon правится в том же
> PR** (`OPERATIONS` — шаблон/деплой; ADR-14 update-нота при изменении CI-модели, если
> затронута); `build.ps1` зелёный. **Гоча (биллинг):** GitHub Actions красный по биллингу
> (решение 2026-06-10 — локальный прогон штатный гейт). REL-12/REL-14/21 = **авторинг**
> YAML-конфига (вступит в силу при включении биллинга); зелёный CI в критерий приёмки **не**
> входит — гейтом остаётся локальный `build.ps1` + ручной прогон релизного пути. Исполнитель —
> субагент (worktree); модель выбирает куратор. Куратор ревьюит, вливает, ставит R9 в `NEXT TASK`.
> _Источник детализации — `ROADMAP.md` R8; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..123` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-124` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-125`.** Куратор инкрементирует при выдаче; выдавать занятые
