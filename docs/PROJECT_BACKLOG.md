...
**`MLC-150` · Серверный фильтр «Базы» по «не найдена в кластере»** — трек «Релиз 0.5.0-beta»
(полная постановка в одноимённом разделе ниже). Одна задача за сессию; после закрытия куратор
сдвигает NEXT TASK на следующую по порядку трека. **Закрыто:** `MLC-149`/`148`/`147` (волна 1) +
`MLC-027` Done (2026-06-14, PR #221/#222/#220/#224). Осталось: 150 → 151 → 152.
...
### MLC-027 · Разбить `i18n/ru.json` на per-feature файлы · Frontend/Maintainability · S · **Done** (2026-06-14, PR #224)

`ru.json` (~1210 строк) разнесён на 19 per-feature файлов `frontend/src/i18n/ru/<key>.json`;
`i18n/index.ts` собирает их спредом в один namespace `translation` (структура словаря идентична,
доказано побайтовым сравнением). Call-sites не тронуты. **Гоча:** новый словарь экспортируется
именованным `{ ru }` из `@/i18n` (тесты/код, читавшие `@/i18n/ru.json` напрямую, идут через него).

### MLC-150 · Серверный фильтр «Базы» по «не найдена в кластере» · Backend/Frontend · P2 · S–M · ← NEXT TASK
