...
**`MLC-215` · FE+BE · P2 · Переезд управления IIS в вкладку «IIS» экрана «Сервер».** Перенести
детальное управление IIS (пулы / сайты / `iisreset`) из вкладки «IIS» дома «Базы» (`InfobasesPage` +
`features/publications/iis/*`) в **новую вкладку «IIS»** экрана «Сервер» (`features/server/iis/`);
прежнюю вкладку из «Баз» **удалить**. Здесь же `ServerPage` обретает **таб-обёртку** (`Tabs`,
образец `PerformanceDrillDown` `forceMount hidden`): «Службы» (из MLC-214) + «IIS». Доступ как везде в
разделе: Viewer видит статусы пулов/сайтов, Admin управляет (recycle/start/stop/restart/`iisreset` —
существующие действия с их Confirm-гейтами). **Контракты `/api/v1/iis/*` НЕ меняются** (BE-эндпоинты
`IisEndpoints` уже есть — переезд преимущественно FE: перенос компонентов/хуков, маршрут, навигация);
BE-часть — только если всплывёт связка с «Базами» (напр. чистка ссылок). Zod-границы IIS не меняются
(контракт прежний). Канон `docs/05_FRONTEND`/`06_UI_GUIDE` — «Базы» теряют вкладку «IIS», «Сервер»
получает; ADR-54 (дом IIS = «Сервер»). **Приёмка:** компонент-тесты вкладки «IIS» на новом месте +
отсутствие вкладки в «Базах»; контракт-тесты IIS не сломаны; локальный `build.ps1` зелёный.
Якоря-образцы — в секции трека ниже.

См. трек «Сервер» ниже — полная цепочка `MLC-212..219` (`MLC-212`..`MLC-214` закрыты).
...
- **`MLC-214`** ✅ Done (2026-06-19) · FE · экран «Сервер» + вкладка «Службы» + индикатор здоровья.
  ...
  (`omittable()`, `overall`/`state` строкой). **Гочи для `MLC-215`:** `ServerPage` сейчас БЕЗ таб-бара
  (одна вкладка) — `MLC-215` вводит `Tabs`-обёртку при добавлении вкладки «IIS»; контент «Службы» вынести
  во вкладку.
- **`MLC-215`** (NEXT) · FE+BE · переезд управления IIS в вкладку «IIS» экрана «Сервер» + удаление
  вкладки «IIS» из «Баз» (полная постановка — в «NEXT TASK» выше).
...
(`MLC-212..219`, релиз 1.1.3) — постановка выше (ADR-54/55); `MLC-212`..`MLC-214` закрыты (2026-06-19), NEXT = `MLC-215`.
