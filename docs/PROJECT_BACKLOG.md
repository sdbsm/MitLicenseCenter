...
> **`MLC-113` — Снятие публикации из IIS, полная реализация (UX-43).**
> Finding A4·UX-43 (High, подтверждён): снять публикацию через панель невозможно (ни API,
> ни UI); удаление инфобазы оставляет публикацию в IIS бесхозной. **Решение владельца —
> ПОЛНАЯ реализация до релиза, не минимум.** Объём: (1) метод unpublish в
> `IWebinstPublisher`/`IIisPublishingService` (webinst -delete либо управляемое удаление
> приложения + vrd/web.config) — СТРОГО за anti-corruption границей (ADR-20: никаких
> `Microsoft.Web.Administration`/webinst в Web-проекте); (2) эндпоинт
> `POST /api/v1/publications/{id}/unpublish` (политика `Admin`); (3) UI: пункт «Снять
> публикацию» с подтверждением + опция «также снять из IIS» в диалоге удаления ИБ; (4)
> аудит-событие — НОВЫЙ замороженный int в `AuditActionType` (число не переиспользовать!),
> обновить freeze-тест; (5) валидация/parity при необходимости — обе стороны
> (BE `InfobaseValidationRules.cs` ↔ FE `features/infobases/validation.ts`). Критерии:
> на стенде публикация снимается через UI, приложение и default.vrd/web.config удалены из
> IIS, статус ИБ «не опубликована», аудит-запись создана; freeze-тест enum и parity-тесты
> зелёные. `OPERATIONS.md`/`SECURITY.md` НЕ трогать (перепишет D1). Самая крупная задача
> трека. Полная постановка — `.claude/plans/audit-fix-prerelease.md` (MLC-113); finding —
> `audit/2026-06/MASTER-REPORT.md`/`A4-*` (UX-43). Исполнитель — субагент (opus, worktree);
> куратор ревьюит, гонит тесты, вливает, ставит следующую задачу трека.
