...
- `MLC-091` (ST-E) · Docs · S · **Done (2026-06-10)** — **финальный док-PR этапа 2**. Новый
  **ADR-28 «Single-host topology»** (+ Update-нота в ADR-17: поле SQL-сервера промотировано —
  ...
  single-host (топология, нет `Infobase.DatabaseServer`, ключ `Sql.Server`, discovery без `server=`),
  +попутно 05 (форма без сервера, фильтр MLC-090). Каталог 04 §4 = 24 ключа, синхрон с
  `SettingDefinitions` ([Doc divergence] `Performance.Recording*` закрыт); discovery Swagger чист
  ещё с MLC-087. Мелочи: поллинг сеансов «~15s»→«~5s» (05 §2/06 §1, факт 5_000/MLC-044); «Отключить
  администратора»→«Отключить пользователя» (06 §12/§7). **Хвост-тест полноты:** grep по `backend/src`
  `DatabaseServer|Cluster.Server|sql-instances|server=` → только легитимные (`DatabaseBackup.*`-снимок,
  `sql-instances` пикер /settings, комментарий в `WebinstArgs`); ноль `Infobase.DatabaseServer`/
  использования ключа/`?server=`. Секцию трека в архив **не переносил** — закрытие трека за куратором.
