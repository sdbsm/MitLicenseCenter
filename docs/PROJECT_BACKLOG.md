...
> **`MLC-114` — Пакет валидации/наблюдаемости backend (BE-03 + BE-05 + BE-10 + UX-01-остаток).**
> Четыре независимых мелких фикса (все «Низкая» сложность), одна сессия:
> (1) **BE-03:** runtime-валидация диапазона `MaxConcurrentLicenses` — отрицательное/ноль
> сейчас тихо отключает enforcement тенанта; правило в ручном хелпере валидации (DataAnnotations
> в minimal API в runtime не прогоняются — см. CLAUDE.md «Гоча»); если правило касается
> инфобаз — parity обе стороны.
> (2) **BE-05:** per-item try/catch в `PublicationStatusRefreshJob` — одно исключение не должно
> обрывать обновление остальных публикаций; ошибка по item — в лог.
> (3) **BE-10:** аудит неудачных попыток входа — НОВЫЙ замороженный int в `AuditActionType`
> (число не переиспользовать! freeze-тест покрывает автоматически; FE-зеркало в
> `features/audit/types.ts` union + `AUDIT_ACTION_TYPES` + i18n `audit.actions.*` — иначе
> парити-тест `auditActionTypes.test.ts` красный, урок MLC-113), без записи введённого пароля.
> (4) **UX-01-остаток:** валидация символов логина на BE и FE (пробел в логине сейчас даёт
> 500 вместо 400) — понятное сообщение об ошибке в форме.
> Критерии: тест на каждый из четырёх пунктов; 500→400 на логине с пробелом; freeze-тест enum
> зелёный. `OPERATIONS.md`/`SECURITY.md` НЕ трогать (перепишет D1). Полная постановка —
> `.claude/plans/audit-fix-prerelease.md` (MLC-114); findings — `audit/2026-06/MASTER-REPORT.md`.
> Исполнитель — субагент (opus, worktree); куратор ревьюит, гонит тесты, вливает, ставит
> следующую задачу трека.
