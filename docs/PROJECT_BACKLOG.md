...
**Нет активной задачи.** Патч `0.5.5-beta` (`MLC-162` — фикс обнаружения службы RAS) **выпущен**;
индекс — `PROJECT_BACKLOG_ARCHIVE.md`. Фикс наземно проверен на dev-сервере: реестровый детектор
находит существующую службу `1C:Enterprise 8.5 Remote Server` (Running) — там, где sc-перебор
давал ложное «не зарегистрирована». **Остаётся за владельцем:** убедиться в UI обновлённой панели
(блок RAS → «ок»/«устарела»), и при желании прогон действий (регистрация/обновление/запуск).
...
**Следующий свободный номер: `MLC-163`.** Куратор инкрементирует при выдаче; выдавать занятые
