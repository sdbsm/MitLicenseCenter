...
**`MLC-228` · Канонизация трека «Расследование производительности» (docs-only).**
Активен трек **версии 1.2** — `release/1.2` (ветка; `main` заморожен на теге `v1.1.4` = точка отката).
Задача: ADR «Производительность = Наблюдение + Расследование» и ADR «Политика безопасного сбора ТЖ» в
`DECISIONS.md`; перенос ключевого из базы знаний в present-tense канон (`02`/`04`/`05`); трек в `ROADMAP.md`.
...
- **0 Канонизация/стенд:** `MLC-228` (ADR+канон, ← `NEXT TASK`), `MLC-229` (снять ⚠ на стенде 8.5).
...
**закрыт и выпущен в `v1.1.4`** (2026-06-19). Активного трека нет; следующую работу ставит куратор
постановочным PR.

**Следующий свободный номер: `MLC-229`.** (`MLC-228` выдан как `NEXT TASK` — канонизация трека
«Расследование производительности», 1.2. Номера `MLC-229..243` **зарезервированы активным треком** —
выдавать по одной согласно `research/perf-investigation/80_TRACK_PLAN.md`. `MLC-227` — отзыв
SCM-зависимости службы от SQL, закрыт в `v1.1.4`; `MLC-222..226` — хотфиксы трека «Сервер».)
