...
| `MLC-109` | Security | P1 | High | Немедленный отзыв доступа: SecurityStampValidator (1–5 мин) + `UpdateSecurityStampAsync` в disable/reset/смене роли; self-смена пароля не разлогинивает (SEC-01) | **NEXT** |
...
> **`MLC-109` — Немедленный отзыв доступа при disable/reset/смене роли (SEC-01).**
> Подключить SecurityStampValidator к cookie-пайплайну (`OnValidatePrincipal`), интервал
> проверки 1–5 мин; вызывать `UpdateSecurityStampAsync` в disable / reset-password / смене
> роли. Критерии (тесты обязательны): отключённый пользователь с активной кукой теряет
> доступ в пределах интервала; Admin→Viewer теряет админ-права без пере-логина; регрессия —
> self-смена пароля НЕ разлогинивает свою сессию (`RefreshSignInAsync`). Полная постановка —
> `.claude/plans/audit-fix-prerelease.md`; findings — `audit/2026-06/MASTER-REPORT.md` (A2·SEC-01).
