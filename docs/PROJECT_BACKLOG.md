...
> **`MLC-104` — Backend осведомлён о Windows-службе (корректива к MLC-100).** `Program.cs` без
> `UseWindowsService()` → служба не сигналит SCM `SERVICE_RUNNING`, `sc start` падает (1053).
> Блокирует тест-инсталл установщика. Канон: правка **ADR-31** + `OPERATIONS`. План —
> `.claude/plans/mlc-104-windows-service-aware.md`. (`MLC-101` мастер — следующая после неё.)
...
- **`MLC-104`** · Backend · P1 · **корректива к MLC-100** · Files: `Program.cs`,
  `Directory.Packages.props`, `Web.csproj`, канон ADR-31/`OPERATIONS`. `builder.Host.UseWindowsService()`
  + пакет `Microsoft.Extensions.Hosting.WindowsServices` — без него `sc start` падает (1053), служба
  не встаёт (блокер тест-инсталла). No-op в dev. План — `mlc-104-windows-service-aware.md`. Status: **Approved (NEXT)**.
