...
**`MLC-214` · Frontend · P2 · Экран «Сервер» + вкладка «Службы» + индикатор здоровья.** Новый раздел
меню `/server` (группа [Мониторинг], меню 7→8 — добавить пункт в `components/layout/Sidebar`; Viewer
видит, Admin управляет — паттерн «Параметров»/«Пользователей»). Экран с вкладками (образец
`PerformancePage` + `PerformanceDrillDown`, `Tabs` `forceMount hidden`); в этой задаче — **вкладка
«Службы»** поверх `GET /api/v1/server/status`: список серверов 1С (`ragent`, имя/версия/состояние) +
сводки RAS/SQL/IIS + **общий индикатор здоровья** узла (светофор по `Overall` Healthy/Degraded/Down/
Unknown). Admin-действия **только над сервером 1С**: старт/стоп/рестарт через
`POST /api/v1/server/onec/{start,stop,restart}` — стоп/рестарт с диалогом подтверждения (да/нет,
ADR-55; `confirm:true` в теле), три состояния операции (идёт «может занять до N с» → успех →
ошибка из `SERVER_OPERATION_FAILED`/detail). Деградация адаптеров (`Available:false`/`Error`) — честная
плашка «не удалось получить статус», не падение. Плашка **«здоровье сервера» на «Обзоре»** со ссылкой
сюда (образец `dashboard/HostHealthCard`). RAS/SQL/IIS на вкладке «Службы» — **только наблюдение**
(управление RAS — в Параметрах, IIS — переедет в MLC-215; SQL — без управления). **Zod-parity на новых
границах** `/api/v1/server/*` (`lib/apiSchema`, nullable = `omittable()`/`.nullish()` — API опускает
null-поля; enum'ы строкой). **Приёмка:** компонент/parity-тесты (рендер статусов и светофора, ролевой
доступ, confirm-диалог, omit-null Zod, обработка `Available:false`); локальный `build.ps1` зелёный.
Якоря-образцы — в секции трека ниже.

См. трек «Сервер» ниже — полная цепочка `MLC-212..219` (`MLC-212`, `MLC-213` закрыты).
...
- **`MLC-214`** (NEXT) · FE · экран «Сервер» + вкладка «Службы» + индикатор здоровья (полная постановка —
  в «NEXT TASK» выше).
- **`MLC-214`** · FE · экран «Сервер» + вкладка «Службы» + общий индикатор + пункт меню `/server`
  (Viewer видит, Admin управляет) + плашка «здоровье сервера» на «Обзоре»; Zod-parity на новых границах.
- **`MLC-215`** · FE+BE · переезд управления IIS в вкладку «IIS» нового экрана; удаление вкладки «IIS»
  из «Баз» (`InfobasesPage`). Контракты `/api/v1/iis/*` не меняются.
...
(`MLC-212..219`, релиз 1.1.3) — постановка выше (ADR-54/55); `MLC-212`/`MLC-213` закрыты (2026-06-19), NEXT = `MLC-214`.
