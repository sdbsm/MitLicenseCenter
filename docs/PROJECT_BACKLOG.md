...
> _Не назначена._ `MLC-085` + `MLC-086` закрыты 2026-06-10 (финальная сессия трека, два PR:
> код / канон). Все задачи этапа 1 UX-трека (`MLC-081..086`) выполнены — закрытие трека
> (перенос секции в архив) делает куратор. Тег `v1-pre-ux-redesign` стоит на main.
