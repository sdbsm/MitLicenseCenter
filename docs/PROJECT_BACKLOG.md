...
### MLC-141 · Упрощение диалогов подтверждения · Frontend · P3 · ✅ Done (2026-06-14, PR #207)

Type-to-confirm убран из 4 диалогов дифференцированно по обратимости (**ADR-45**, уже на main):
обратимые (публикация/снятие) → простое «да/нет»; необратимые (удаление базы/клиента) → «да/нет»
с явным предупреждением (`irreversibleWarning`), барьер-диалог сохранён; чекбокс «снять публикацию
из IIS» (MLC-113) сохранён. API не менялся (`confirm:true` как раньше), parity не затронут.
**Гоча:** постановка не перечисляла `DeleteTenantDialog.test.tsx`, но он тоже требовал снятия ввода
(textbox) — выровнен куратором при сведении.

### MLC-142 · Полировка «Быстродействия» (бывш. MLC-073) · Frontend/Backend · P3 · ✅ Done (2026-06-14, PR #205)

`SOS_WORK_DISPATCHER` добавлен в `BenignWaitTypes` (`SqlPerformanceProbe`, +тест); CPU-гейдж host
явно подписан тултипом «мгновенный снимок WMI», таблица атрибуции — тултипом «усреднено за интервал
поллинга ~5 с» (взаимные перекрёстные ссылки снимают мнимое расхождение). Вариант «явная подпись»
выбран над сглаживанием — без риска для числовой логики. Канон `/performance` не описывает базу
времени/benign-список — правок docs не потребовалось.

### MLC-143 · Графики на дашборде (recharts) · Frontend · P3 · ✅ Done (2026-06-14, PR #206)

Карточки топ-клиентов (прогресс-бары) → горизонтальная recharts-`ComposedChart` (паттерн из
`LicenseUsageChart`/`RecordingHostChart`): цвета баров по severity квоты (`quota.ts`, как StatusBadge),
лимит-`ReferenceLine` при едином лимите. Имена клиентов остаются кликабельными ссылками на
`/tenants/:id` (инвариант MLC-085) — вынесены в `TopTenantsLegend` рядом с SVG (recharts не держит
ссылки в svg-лейблах). Канон 05_FRONTEND обновлён.

### MLC-144 · Движок таблиц (бывш. MLC-062) · Frontend · P2 · **NEXT TASK** · трек, последним
