...
- **Трек «Нераспределённые базы: discovery-first добавление»** — `MLC-092..094` 3/3
  ...
  `glittery-floating-turtle.md` (локальный, у куратора). Слепая зона (б) — обратный дрейф —
  остаётся обязательной (`ROADMAP.md`).

---

## NEXT TASK

> **`MLC-095` + `MLC-096` → `MLC-097` — единственная сессия трека «Обратный дрейф
> панель↔кластер»** (постановки — в секции трека ниже). **Два PR:** сначала код
> `MLC-095`+`MLC-096` (один PR, коммиты раздельно), после его вливания — `MLC-097`
> (канон отдельным PR). Перед стартом: `git fetch` + FF, ветка от `main`. Миграций в
> треке НЕТ. Финал — e2e на стенде по чек-листу плана трека
> (`C:\Users\andre\.claude\plans\glittery-floating-turtle.md`, часть 2, «Проверка»).

## Трек «Обратный дрейф панель↔кластер» — открыт (2026-06-11)
...
- Серверный фильтр таблицы по «не найдена» — **не делаем** (отложенная опция).
- Валидация инфобаз, каталог настроек, enum'ы аудита — не меняются.

**Ограничения трека:** панель полностью рабочая после каждой задачи; объём строго по
постановке; **миграций НЕТ** (появилась нужда — стоп, доклад куратору: план неверен);
обе роли проверяются; live-прогон на стенде; кандидаты — записью в реестр, не в тот же ход.

**Темп исполнения:** одна сессия — `MLC-095`+`MLC-096` (один PR кода, коммиты раздельно,
прецедент 087+088) → после вливания `MLC-097` (PR канона). Жизненный цикл реестра — на задачу.

**Задачи трека (исполнять по порядку):**

- `MLC-095` (RD-A) · Backend · S · **Approved** — **`MissingItems` в ответе unassigned**.
  `UnassignedInfobasesContracts`: + `MissingItems` (DTO `{ InfobaseId, TenantName, Name,
  ClusterInfobaseId }`); `GetUnassignedAsync`: при `Available:true` — обратный diff
  (записи `Infobases`, чьих `ClusterInfobaseId` нет в снапшоте; join имени клиента — образец
  `infobaseMap` в `ReconciliationJob`), при `Available:false` — пустой список; кэш/`refresh`
  без изменений. Тесты по образцу тестов слайса: обратный diff; пусто при `Available:false`;
  база «и в панели и в кластере» не попадает никуда; запись с чужим UUID — в `MissingItems`
  и не в `Items`.
- `MLC-096` (RD-B) · Frontend · S/M · **Approved** — **баннер + метка + диалог**. Zod-схема
  unassigned: + `missingItems` (поля non-null; имена camelCase — сверить с фактическим JSON);
  `MissingInfobasesBanner` (danger, слот рядом с `UnassignedBanner`, тот же query — без
  второго запроса) + диалог («Удалить» → существующий delete-флоу + инвалидация); метка в
  `InfobaseRow` по membership-набору `Set<clusterInfobaseId>` со страницы (Viewer /
  `available:false` → набор пуст). Тесты Vitest: нет баннера/меток при `available:false`,
  пустом `missingItems`, для Viewer; метка по membership; диалог → удаление. i18n `ru.json`.
- `MLC-097` (RD-C) · Docs · S · **Approved** — **канон отдельным PR после вливания кода**.
  **ADR-29 — Update-нота** (направление (б) реализовано тем же наблюдательным механизмом;
  нового ADR не нужно); канон `04` (форма ответа + условие `Available`), `05` §3.3 (метка,
  красный баннер, диалог), `06` §12 (фразы «Не найдена в кластере», «Показать»);
  `ROADMAP.md` — пункт «Дрейф панель↔кластер» **закрыт полностью** (оба направления,
  треки `MLC-092..094` + `MLC-095..097`), сжать до строки-факта. Present-tense, без changelog.
...
`MLC-001..094` закрыты (`MLC-095..097` — активный трек «Обратный дрейф»; кроме
...
**Следующий свободный номер: `MLC-098`.** Куратор инкрементирует при выдаче; выдавать занятые
