...
**`MLC-163` · Backend · P1 · High · Module: Publications/Jobs** — фикс: «Проверить публикацию»
падает 500 (`DbUpdateConcurrencyException`); фоновое обновление статусов молча сломано с `0.5.0`.
Патч `0.5.6-beta`.

- **Дефект (подтверждён на dev-сервере, Event Log):** `POST /api/v1/publications/{id}/check` →
  `PublicationStatusRefreshJob.RefreshOneAsync`→`ProcessOneAsync` бросает
  `DbUpdateConcurrencyException` («ожидалось 1 строка, обновлено 0»). **Первопричина:**
  `ProcessOneAsync` грузит снимок публикации проекцией БЕЗ `RowVersion` и аттачит probe-сущность
  с пустым токеном → после MLC-151 (concurrency-токен `Publication.RowVersion`) EF строит
  `UPDATE … WHERE Id=@id AND RowVersion=@token`, токен пустой → 0 строк → исключение **всегда**.
  Регрессия с релиза `0.5.0` (MLC-151): `RefreshAllAsync` (фон) гасит per-item и молча не
  обновляет статусы; `RefreshOneAsync` (ручная «Проверить») пробрасывает → 500. К RAS-работе
  (MLC-159..162) отношения не имеет — другая подсистема.
- **Фикс:** добавить `RowVersion` в проекцию `StatusSnapshot` и выставлять `probe.RowVersion =
  snapshot.RowVersion` — тогда токен совпадает, targeted-апдейт `LastCheck*` проходит (1 строка).
  Сделать `RefreshOneAsync` устойчивым к **реальному** редкому конфликту (`DbUpdateConcurrencyException`
  при параллельном изменении строки) — поймать как benign (лог + выход), чтобы ручная проверка не
  отдавала 500; статус подтянется следующим циклом. (НЕ `ExecuteUpdate` — InMemory-тесты его не
  поддерживают; держим `SaveChanges`-путь.)
- **Тест:** регрессионный — публикация с непустым `RowVersion`, `RefreshOneAsync`/`CheckAsync`
  проходит и пишет `LastCheck*` (раньше бросал). Покрыть и benign-конфликт, если воспроизводимо.
- **Проверка куратором после merge:** живой `POST /publications/{id}/check` на dev-сервере (8080)
  → HTTP 200 + свежий статус (не 500).
- **Канон:** версия `0.5.6-beta` + ROADMAP. ADR не нужен (баг реализации). Спека — этот пункт.
...
`MLC-001..162` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
и патч `0.5.5-beta` (`MLC-162` — фикс обнаружения службы RAS через реестр) **завершены** — индексы
в `PROJECT_BACKLOG_ARCHIVE.md`.
`MLC-146` — уборка мёртвого кода после MLC-144. **Нет активного трека.** Полные отчёты — в
git-истории (коммиты `MLC-NNN:`/PR).

**Следующий свободный номер: `MLC-164`.** (`MLC-163` выдан фиксу публикаций `0.5.6-beta`.)
