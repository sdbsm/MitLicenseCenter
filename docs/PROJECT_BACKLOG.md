...
> **`MLC-137` — R12d: терминология и тексты UI (UX-кластер D).**
> Frontend-only · P3 · Small-Medium. Четвёртый кластер R12. Полировка единообразия текстов (RU-only locked).
> **Audit-first (урок MLC-133):** сначала перечисли фактические отклонения, потом правь; источник
> канонических терминов — глоссарий `docs/01_OVERVIEW.md` + `docs/06_UI_GUIDE.md`. Находки (UX-12/13/30/32/33):
> - **UX-12/13:** разнобой терминов «клиент/арендатор» и «сеанс/сессия» — привести к одному (канон —
>   глоссарий; вероятно «клиент» и «сеанс»). Свип `frontend/src/i18n/ru.json` + хардкод-строк.
> - **UX-30:** английские имена ролей (`Admin`/`Viewer`) в русских сообщениях/таблицах вместо переведённых
>   (`users.roles.*` уже есть) — найти места, где роль рисуется сырой строкой.
> - **UX-32:** «recycle» (IIS app-pool) без перевода в UI.
> - **UX-33:** непоследовательная пунктуация (точки в конце подсказок/тостов, кавычки) — выровнять по
>   преобладающему стилю.
> Объём правок — только тексты (i18n + редкий хардкод); логику/контракты/parity не трогать. Обновить
> ассерты тестов, завязанных на изменённый текст. Канон 06 (термины) при необходимости.
> Критерии готовности: перечень отклонений в отчёте; термины единообразны; `build.ps1` зелёный.
> Исполнитель — субагент (worktree); модель — Sonnet (Small-Medium, текстовый свип).
> _Состав R12 далее: R12e (доступность WCAG AA, aria-label), R12f (онбординг). Источник — `ROADMAP.md`
> R12; первоисточник — `TECH-DEBT.md` UX-кластер D._
...
`MLC-001..136` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-137` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-138`.** Куратор инкрементирует при выдаче; выдавать занятые
