...
> **`MLC-098` — Бэкенд сам отдаёт SPA (same-origin).** Трек «GUI-установщик (Inno Setup)»,
> задача 1/6. Канон в составе задачи: **ADR-30** + `04_INFRASTRUCTURE`/`OPERATIONS`. Детальный
> план — `C:\Users\andre\.claude\plans\zesty-tinkering-fog.md` (одобрен куратором 2026-06-11).
...
- **`MLC-098`** · Backend/Frontend · P2 · Module: Web hosting · Files: `Program.cs`,
  `SpaFallback.cs` (new), `MitLicenseCenter.Web.csproj`, `Web/SpaFallbackTests.cs` (new),
  `.gitignore`, канон `DECISIONS`/`04`/`OPERATIONS`. Kestrel раздаёт собранный SPA из `wwwroot`
  same-origin с API (`UseStaticFiles` + history-fallback; `/api`,`/hangfire` зарезервированы →
  неизвестный `/api/*` отдаёт 404, не `index.html`). MSBuild-таргет кладёт `frontend/dist` в
  опубликованный `wwwroot` (publish самодостаточен; флаги `SkipSpaBuild`/`PrebuiltSpaDist`).
  **ADR-30.** План — `zesty-tinkering-fog.md`. Status: **Approved (NEXT)**.
