...
> **`MLC-107` — Установщик: надёжное создание службы + чистый конфиг при переустановке.** Follow-up к
> «GUI-установщик». Баги (стенд, лог установки): `sc create … obj="sa"` → 1057, установка завершилась
> «успехом» **без службы** (результат `sc` не проверяется); `appsettings.Production.json` не удаляется
> при деинсталле и не перезаписывается при переустановке (залипает старый). Фикс: создание службы в
> `[Code]` с проверкой `rc` + внятная ошибка (особенно 1057 «SQL-логин в Windows-режиме»); чистка/
> перезапись конфига. Только установщик. Канон: **ADR-31** + `OPERATIONS`. План —
> `.claude/plans/mlc-107-installer-service-robustness.md`. Status: **Approved (NEXT)**.
...
**Следующий свободный номер: `MLC-108`.** Куратор инкрементирует при выдаче; выдавать занятые
