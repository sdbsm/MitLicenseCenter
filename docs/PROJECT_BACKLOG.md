...
> **`MLC-117` — Публикация не видит заданный `OneC.RAS.Endpoint` до пересохранения настройки
> (follow-up ВМ-проверки 2026-06-13).** Backend · P1 · High ·
> `Infrastructure/Publishing/WebinstArgs.cs` + путь чтения настроек.
...
> сценария; понятная ошибка остаётся только при реально пустом значении. Исполнитель —
> субагент (opus, worktree); куратор ревьюит, вливает.
>
> После закрытия: куратор пересобирает Setup.exe, владелец повторяет апгрейд-проверку
