...
> _Нет активной задачи._ Последняя закрыта — `MLC-105` (см. индекс «Завершённые треки»,
> строка «Вне трека (follow-up к „GUI-установщик")»). Следующую `NEXT TASK` ставит куратор.
