...
### MLC-140 · Dependabot · Maintainability · P2 · ✅ Done (2026-06-14, PR #203)

15 Dependabot-PR сведены в один сводный PR. **Влито:** Hangfire-семейство (Core/AspNetCore/
SqlServer) → 1.8.23; tailwindcss 4.3.1, date-fns 4.4.0, @hookform/resolvers 5.4.0, prettier 3.8.4;
5 GitHub-Actions (checkout v6, cache v5, setup-node v6, setup-dotnet v5, upload-artifact v7);
мажоры Asp.Versioning.Http/Mvc.ApiExplorer 8→10 (выравнивание под .NET 10 — кода менять **не
пришлось**, API-поверхность v8↔v10 стабильна), coverlet.collector 6→10, @types/node 22→25.
**Отложено осознанно:** `FluentAssertions` оставлен на 6.12.2 — **8.x под коммерческой лицензией
Xceed** (7.x — последняя бесплатная Apache-2.0); тащить платную тест-зависимость в коммерческий
продукт нельзя. Миграция с FA — отдельная опция `MLC-145` ниже. `pnpm/action-setup@v4` не трогали
(вне scope — не `actions/*`). Гейт зелёный: 857 BE + 523 FE.

### MLC-141 · Упрощение диалогов подтверждения · Frontend · P3 · **NEXT TASK**
...
`MLC-001..140` закрыты (кроме отложенных `MLC-025/026/027/145` — см. «Открытые опции»; `MLC-062`/`073`
...
`MLC-036` — ROADMAP «RAS Strategy B»). `MLC-141..144` — активный трек релиза `0.4.0-beta` (выше).
Индекс закрытых задач — **`docs/PROJECT_BACKLOG_ARCHIVE.md`** (поиск по `MLC-NNN`); полные
отчёты — в git-истории (коммиты `MLC-NNN:`/PR).

**Следующий свободный номер: `MLC-146`.** Куратор инкрементирует при выдаче; выдавать занятые
