...
(ревизия канона) в основном; R2–R8 закрыты полностью (`MLC-118..124`); **R9 в работе** (кластеры 1–2
закрыты: security-барьер бэкенда — `MLC-125`, чистка поставки/firewall — `MLC-126`; остаётся аккаунт
службы — `MLC-127`); остаётся R11/R12 (R10 — верификационный остаток). Отмечено в ROADMAP значком ✅.
...
> **`MLC-127` — R9c: Аккаунт службы (REL-06 + SEC-05).** Security/Release · P2 · Medium.
...
`MLC-001..126` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-127` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-128`.** Куратор инкрементирует при выдаче; выдавать занятые
