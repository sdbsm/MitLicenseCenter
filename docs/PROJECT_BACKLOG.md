...
> **`MLC-103` — Деинсталляция + полировка установщика (закрытие трека).** Трек «GUI-установщик»,
> задача 6/6 (последняя). Только установщик: keep-data prompt при удалении (удалять ли ключи
> `%ProgramData%`; дефолт — сохранить, предупреждение про бэкап-юнит; БД не трогаем), ярлык меню
> «Пуск» на URL панели, `SetupLogging=yes`. Канон: правка **ADR-31** + `OPERATIONS`. План —
> `.claude/plans/mlc-103-uninstall-polish.md`. После вливания куратор закрывает трек (перенос в архив).
...
- **`MLC-103`** · Installer · P2 · Files: `installer/MitLicenseCenter.iss`, канон ADR-31/`OPERATIONS`.
  Деинсталл-полировка (закрытие трека): keep-data prompt при удалении (удалять ли ключи
  `%ProgramData%\MitLicenseCenter`; дефолт — сохранить + предупреждение про бэкап-юнит; БД не трогаем),
  ярлык меню «Пуск» на `http://localhost:<port>/`, `SetupLogging=yes`. План — `mlc-103-uninstall-polish.md`. Status: **Approved (NEXT)**.
