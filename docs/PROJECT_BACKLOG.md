...
10. `MLC-071` — Frontend: Recording UI (кнопка старт/стоп + индикатор «идёт запись», список расследований,
    просмотр = график host во времени + топ-виновники за период + экспорт по образцу `features/reports/
    export/`). Фаза 4 (завершает). **Следующий по порядку.**

---

## NEXT TASK

> **`MLC-071` — Frontend: Recording UI** (трек «Анализ быстродействия 1С», Фаза 4 — завершает;
> `MLC-070` backend Done 2026-06-09).
> **Модель** (ADR-26): live-панель и backend Recording готовы. Объём — UI записи на `/performance` (ниже
> трёх live-источников): (1) кнопка **старт/стоп** + индикатор «идёт запись» (старт/стоп/удаление = `Admin`,
> список/просмотр = `Viewer`); 409 `RECORDING_ACTIVE` на повторный старт / удаление идущей записи. (2) Список
> расследований (`GET /performance/recordings`: id, начало/конец, статус `Active`/`Stopped`/`Interrupted`,
> `startedBy`, `stopReason`, число сэмплов). (3) Просмотр записи (`GET /performance/recordings/{id}`: ряд
> сэмплов = график host во времени + топ-виновники 1С/SQL за период; JSON-поля уже десериализованы бекендом
> в те же формы, что live — `processGroups`/`oneC`/`sql`) + экспорт по образцу `features/reports/export/`.
> (4) `DELETE …/{id}`. Zod-граница на ответах (урок [[api-omits-null-fields]] — nullable через `omittable()`).
> Тесты по образцу `features/reports`/`features/performance`.
>
> Полная спека — `C:\Users\andre\.claude\plans\spicy-discovering-torvalds.md` (Фаза 4, `MLC-071`).
> Зависит от `MLC-070` (Done). Оценка M. Дальше — Фаза 5 (UI-холистик) — вне трека.
>
