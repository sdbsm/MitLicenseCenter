...
> **`MLC-121` — R5: Офлайн-UX и обратная связь.** Frontend (+мелкий BE-текст) · P2/P3 · Medium.
> Зависимость: R2 (`MLC-118`) по формам с inline-ошибками — закрыта. Цель — внятная обратная
> связь при сетевых сбоях/недоступности и единый паттерн ошибок вместо generic-тостов. Состав
> (находки `audit/2026-06/TECH-DEBT.md`, кластер UX/FE):
> - **UX-03 + FE-05:** различать 401 (протух сеанс → редирект/релогин) и сетевую ошибку (нет
>   связи с бэкендом); глобальный индикатор состояния соединения; живой текст `errors.network`
>   вместо немого провала; логировать `ApiSchemaError` (Zod-граница отвергла ответ) отдельно от
>   сетевых — диагностируемость FE↔BE-расхождений.
> - **UX-04:** единый паттерн **inline-ошибок** в формах вместо разрозненных generic-тостов
>   (опереться на барьер `MLC-118`: 400 ValidationProblem с ошибкой поля уже приходит — связать
>   ключи полей с inline-выводом).
> - **UX-17:** RAS-карточка дашборда — внятная подсказка при недоступности кластера (сейчас
>   нулевое/немое состояние), завязать на `ras.healthy`/`consecutiveFailures` снапшота.
> - **UX-44 (хвост):** понятный текст ошибки публикации. Функциональный корень устранён в
>   `MLC-117` (публикация работает сразу после установки) — остаётся **только полировка текста**
>   сообщения об ошибке (без правок флоу публикации).
> Критерии готовности: сетевой сбой и недоступность RAS дают пользователю понятный сигнал (не
> немой провал/generic-тост); 401 отличается от сети; `ApiSchemaError` логируется обособленно;
> формы используют inline-ошибки на 400-ValidationProblem; **parity BE↔FE и canon (05/06)
> обновлены в том же PR**, если меняются контракты/тексты; `build.ps1` зелёный. Исполнитель —
> субагент (opus, worktree); модель выбирает куратор. Куратор ревьюит, вливает, ставит R6 в
> `NEXT TASK`. _Источник детализации — `ROADMAP.md` R5; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..120` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-121` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-122`.** Куратор инкрементирует при выдаче; выдавать занятые
