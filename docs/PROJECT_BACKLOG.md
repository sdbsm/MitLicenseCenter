...
Полные отчёты и постановки по уже **закрытым** задачам (MLC-001..020) вынесены в
...
Текущий реестр (MLC-021..028) — результат **анализа технического долга и поддерживаемости**
...
### MLC-021 — Web-хелперы: uniqueness-backstop + билдер описаний аудита
- **Category:** Maintainability / Backend (дублирование внутри Web-слоя)
- **Priority:** P2 · **Severity:** Medium
- **Module:** Web / Endpoints
- **File(s):** `backend/.../Endpoints/InfobasesEndpoints.cs` (Create/Update/Reassign — блоки
  `try { SaveChanges } catch (DbUpdateException …)` и `audit.LogAsync(...)` с инлайн-строками);
  `TenantsEndpoints.cs`; `PublicationsEndpoints.cs`
- **Description:** В каждом мутирующем endpoint повторяются: резолв initiator
  (`User.Identity?.Name ?? "unknown"`), почти дословные `catch (DbUpdateException) when
  (DbUniqueViolation.Identify…)` блоки, ручная сборка русских описаний аудита. Это следствие
  вертикальных срезов (ADR-20), но дублирование живёт **внутри одного транспорта** — убирается
  без use-case-слоя.
- **Impact:** Новая аудируемая сущность = копирование всего паттерна; разрозненные строки
  аудита → дрейф формулировок, их нельзя протестировать как единицу. **Cost if ignored:** M.
- **Recommendation:** Тонкие Web-хелперы (не нарушая ADR-20): `SaveWithUniquenessBackstop(...)`,
  билдер/каталог шаблонов описаний аудита, extension для initiator. **НЕ** вводить Application
  use-case слой — это отдельная отложенная опция `MLC-011(a)` со своим триггером. **Cost to fix:** S–M.
- **Status:** Open
...
1. **MLC-021** — высокий: сокращает каждый будущий мутирующий endpoint, централизует
   формулировки аудита (S–M). → **NEXT TASK**.
2. **MLC-022** — высокий: убирает тихий дрейф правил при росте форм (M).
...
> **MLC-021 — Web-хелперы: uniqueness-backstop + билдер описаний аудита.**
> Высокий ROI: сокращает каждый будущий мутирующий endpoint, централизует формулировки
> аудита и резолв initiator, убирает дублирование внутри Web-слоя без введения
> use-case-слоя (ADR-20). Объём S–M.
