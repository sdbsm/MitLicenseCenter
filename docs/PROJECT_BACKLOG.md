...
2. `MLC-058` — Раздел «Администраторы» (Backend API + Frontend UI в одном заходе): эндпоинты `/admins`
   ...
   `features/tenants/`, admin-only роут + пункт сайдбара, показ сгенерированного пароля. BE+FE, объём M.
...
> RAS Strategy B, перф PERF-08+) остаются gated на триггеры — анализ 2026-06-08 подтвердил, что триггеры
> не сработали, по умолчанию не берутся.
