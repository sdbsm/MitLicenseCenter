...
**Нет активной задачи.** `MLC-173` (Трек 1 рефакторинга установщика — разрезание секции `[Code]`
на `#include`-модули) **влит** (PR #280, ISCC зелёный, байт-в-байт). **Следующий шаг — Трек 2**
(редизайн GUI мастера: явный выбор провижининга вместо «пусто = Integrated», укрупнение страниц,
страница-итог; родные контролы Inno — не кастомная отрисовка, прецедент MLC-172). Карту страниц
фиксирует куратор постановочным PR; версия → `0.7.0-beta`. Спека — план-файл `atomic-coalescing-wave.md`.

**За владельцем:** живая проверка `0.6.6-beta` на стенде по `MLC-170/171/172`
(virtual account `NT SERVICE\MitLicenseCenter`, провижининг Integrated Security / SQL-логин `sa`,
негатив с откатом, апгрейд, ручная миграция B→A `OPERATIONS.md §8`). Актуальный релиз —
**`0.6.6-beta`** (`0.6.4`/`0.6.5` отозваны).
...
`MLC-001..174` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
**Следующий свободный номер: `MLC-175`.** (`MLC-174` выдан чистке кода установщика — помощник запуска PS + dedup.)
