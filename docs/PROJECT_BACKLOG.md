...
> **`MLC-083` + `MLC-084` — объединённая сессия** (решение пользователя + куратора 2026-06-10:
> остаток трека исполняется парами, см. примечание в секции трека). Обе постановки — ниже.
> Один PR, коммиты раздельно по задачам (`MLC-083: …` / `MLC-084: …`). Перед стартом:
> `git fetch` + FF, ветка от `main`. Тег `v1-pre-ux-redesign` стоит на main.
...
- `MLC-083` (UX-C) · Frontend · XS–S · **NEXT TASK (сессия с `MLC-084`)** — **/settings:
  расформировать «Значения по умолчанию»** (§4.3, §5).
  ...
  по-прежнему без UI.
- `MLC-084` (UX-D) · Frontend · S · **NEXT TASK (сессия с `MLC-083`)** — **профиль в топбар +
  перегруппировка меню** (§3.3, §3.5).
