...
**`MLC-176` · Frontend+Backend · P2 · Medium · Updates · `IN PROGRESS`** — **проверка обновлений
через GitHub Releases**. Панель ненавязчиво сигнализирует о вышедшей версии: глобальный баннер
«Доступна версия X.Y.Z» (виден **всем** ролям) + ссылка «Открыть релиз» на GitHub (всем) + кнопка
«Скачать установщик» (только **Admin**) + «Проверить сейчас» (Admin, карточка в «Параметрах»).
Запуск установщика — **руками админа через UAC** (браузер не запускает `.exe`, служба под IIS
неинтерактивна); бэкенд только отдаёт URL — скачивает браузер.

- **Источник:** `GET api.github.com/repos/{repo}/releases/latest`, анонимно (public repo). Владелец
  публикует релизы вручную (тег `vX.Y.Z` + ассет `Setup.exe`). Ленивая проверка через `IMemoryCache`
  (паттерн `DashboardEndpoints`), TTL из настройки; недоступность GitHub → `CheckAvailable=false`
  (HTTP 200, не 500) с коротким TTL.
- **Бэкенд:** свой semver-компаратор (`Domain/Updates`, без новых зависимостей); порт
  `IGitHubReleaseClient` (Application) + первый в проекте типизированный `HttpClient` (Infrastructure,
  обяз. `User-Agent`); эндпоинты `GET /updates/status` (Viewer) + `POST /updates/check-now` (Admin);
  настройки `Updates.Repository`/`CheckIntervalHours`/`Enabled` в `SettingDefinitions`. **Аудит не
  пишем** (enum заморожен).
- **Фронт:** фича `features/updates/` (Zod `.nullish()` для nullable — урок omit-null); `UpdateBanner`
  в `AppShell`; «Проверить сейчас» в Settings (Admin).
- **Канон в том же PR (исполнитель):** **ADR-50** (канал обновлений; UAC-ручная установка; без
  автозапуска/подписи/changelog-рендера; ортогонально ADR-28) + present-tense правки `02`/`04`/`05`
  + `DEVELOPMENT` (требование к релизу: тег + ассет `Setup.exe`).
- **Вне scope:** авто-скачивание/запуск бэкендом, проверка подписи/хеша, рендер changelog в панели,
  приватные репо/PAT, фоновый hosted-service, новые аудит-коды.
- **Без сертификата подписи** (его нет) — целостность за HTTPS-каналом GitHub; SmartScreen-предупреждение
  при запуске неподписанного `.exe` — ожидаемо и приемлемо (доверенный админ на своём хосте).

Спека — локальный план-файл `vivid-waddling-sedgewick.md` (у куратора). Гейт — `scripts/build.ps1`.
...
**Следующий свободный номер: `MLC-177`.** (`MLC-176` выдан проверке обновлений через GitHub Releases.)
