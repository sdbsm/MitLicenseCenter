...
> **`MLC-112` — Страховка апгрейда установщика (REL-02).**
> Finding A6·REL-02 (High): апгрейд заменяет файлы до миграций без какой-либо страховки,
> отката нет. `MLC-107` уже добавил MsgBox+RaiseException на провал `sc create` и
> предупреждение на провал `sc start` — исполнителю сверить текущее поведение `.iss` с
> findings и закрыть ОСТАТОК. Объём (только `installer/MitLicenseCenter.iss`): (1) на
> апгрейде, ДО замены файлов — обязательный экран «сделайте бэкап БД + key ring»
> (показывается только при апгрейде, не на чистой установке); (2) провал старта службы в
> финале апгрейда — явный экран ОШИБКИ мастера (не «успех»), со ссылкой на Event Log:
> проверить, что текущее «предупреждение» из MLC-107 действительно неотличимо от успеха, и
> если да — усилить до ошибки. Критерии: ручной прогон апгрейда с искусственно сломанным
> стартом службы завершается экраном ошибки, не «успехом»; экран бэкапа показывается только
> при апгрейде. `OPERATIONS.md`/`SECURITY.md` НЕ трогать (перепишет D1); правка ADR-31, если
> прямо нужна. Полная постановка — `.claude/plans/audit-fix-prerelease.md` (MLC-112);
> finding — `audit/2026-06/MASTER-REPORT.md` (A6·REL-02). Исполнитель — субагент (opus,
> worktree); куратор ревьюит, вливает, ставит следующую задачу трека.
