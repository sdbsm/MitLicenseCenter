...
Уже закрыто пред-релизным треком и этапом D1: **R1** (security-барьер) полностью; **R10**
(ревизия канона) в основном; R2–R9 закрыты полностью (`MLC-118..127`; R9 — три кластера
`MLC-125`/`MLC-126`/`MLC-127`); остаётся R10-остаток (верификация → `MLC-128`), R11, R12.
...
> **`MLC-128` — R10-остаток: верификация (REL-11 / DOC-10 / BE-15 / FE-17-18).** Maintainability/Docs · P3 · Small.
> Завершает R10 (основное закрыто этапом D1). Сборная зачистка мелких хвостов; затрагивает немного
> продуктового кода (DOC-10 — один EventId; BE-15 — комментарий) + FE-удаление сироты + канон. Замороженные
> enum аудита/parity не трогаем; EventId логов — НЕ аудит-enum, не заморожены.
> Состав (детализация — `ROADMAP.md` R10-остаток; находки `TECH-DEBT.md`):
> - **DOC-10:** коллизия `EventId = 1002` — его используют **оба** `IdentitySeeder` (admin с заданным
>   паролем, Warning) и `SettingsSeeder` (засеяно N параметров, Information). Дать `SettingsSeeder`
>   отдельный EventId (предложение: диапазон по компоненту — Identity `100x`, Settings `110x` →
>   `SettingsSeeder = 1100`; добавить короткий комментарий-конвенцию в обоих seeder'ах). **Doc-driven:**
>   в `OPERATIONS.md` §3 таблица EventId (строки ~229-236) показывает 1002 для обоих и примечание «различать
>   по уровню» — обновить таблицу под новый EventId и убрать примечание о коллизии в **том же PR**.
> - **BE-15/DOC-32:** стейл-комментарий в шапке `SystemProcessRacRunner.cs` (строки ~10-11) утверждает
>   «UTF-8 кодировка... rac.exe пишет UTF-8 без BOM, верифицировано в ADR-3.3» — это противоречит телу
>   (строки 16-23: фактически OEM/CP866-декод). Привести шапку в соответствие с реальностью (OEM/CP866).
>   Поведение НЕ меняем (тест BE-12 сторожит декод). Если ADR-3.3 в `DECISIONS.md` несёт ту же стейл-формулировку
>   «UTF-8» — синхронизировать (это канон). Не переписывать декод.
> - **FE-17/18:** удалить сироту `frontend/src/components/layout/ComingSoonPage.tsx` (подтверждено: только
>   самоссылки, нет импортов) + мёртвый i18n-ключ `comingSoon` (`i18n/ru.json:22`). Проверить, что нет других
>   очевидно-мёртвых ключей с нулём ссылок (консервативно: удалять только с нулём строковых ссылок — i18n
>   может вызываться динамически).
> - **REL-11:** верифицировать полноту раздела логов + runbook в `OPERATIONS.md` (§3 «Логи и диагностика»,
>   §4 «Диагностика типовых отказов», аварийный runbook уже есть после D1). Это **проверка**, не переписывание;
>   единственная обязательная правка — таблица EventId (см. DOC-10). Существенных дыр не добавлять; если найдена
>   реальная дыра — заполнить минимально.
> Критерии готовности: EventId не коллидирует (код + таблица OPERATIONS синхронны); шапка `SystemProcessRacRunner`
> описывает OEM/CP866 (не UTF-8); `ComingSoonPage` + ключ `comingSoon` удалены, FE собирается; раздел логов
> верифицирован; **canon правится в том же PR**; `build.ps1` зелёный. **После MLC-128 R10 закрыт полностью.**
> Дальше — R11 (масштабируемость списков), R12 (полировка UX). Исполнитель — субагент (worktree); модель выбирает куратор.
> _Источник детализации — `ROADMAP.md` R10-остаток; первоисточник — `TECH-DEBT.md`._
> Завершающий, третий кластер R9 (кластеры 1–2 закрыты `MLC-125`/`MLC-126`). Закрывает «установка
> прошла, а половина продукта не работает» в режиме A и фиксирует привилегии службы.
> Состав (находки `audit/2026-06/TECH-DEBT.md` / `A2-SECURITY.md` SEC-05 / `A6-RELEASE.md` REL-06;
> детализация — `ROADMAP.md` R9 кластер 3):
> - **REL-06 (основное, выполнимо):** в режиме A (Windows-аккаунт под службой) — явный экран/инфо-блок
>   мастера с **требованиями к аккаунту**: доступ к SQL (логин + права на целевую БД; «Проверить
>   подключение» идёт под админом-установщиком, не под сервис-аккаунтом — зелёное ≠ служба сможет) и
>   для IIS-функций членство в локальных Administrators **или** read на `%windir%\system32\inetsrv\config`
>   (иначе все публикации стартуют в «Ошибка проверки»). Пост-установочный чек-лист — в `OPERATIONS`/`INSTALL`.
>   Текст/инфо-страница `.iss` + канон; продуктовый код не трогаем.
> - **SEC-05 (фиксация риска):** служба в режиме B = LocalSystem, в режиме A = аккаунт в Administrators —
>   привилегии выше реальной потребности (фактически нужен read на `inetsrv\config` + SQL-доступ). Точный
>   минимум (узкий ACE на `inetsrv\config` / членство в `IIS_IUSRS` вместо Administrators) требует
>   **runtime-трассировки** на стенде — её куратор/субагент автономно не делает. Поэтому SEC-05 закрывается
>   **принятым-риском** в `DECISIONS.md` (ADR или binding-нота) + раздел «Service privileges» в `SECURITY.md`;
>   полный переход на low-priv аккаунт с узким ACE — отложенный follow-up (триггер: проведена runtime-трасса
>   чтений `ServerManager`/`inetsrv\config`).
> Критерии готовности: режим A снабжён явными требованиями к аккаунту в мастере + чек-листом в каноне
> (REL-06); привилегии службы и принятый риск SEC-05 зафиксированы в `DECISIONS.md` + `SECURITY.md`;
> **canon правится в том же PR**; `build.ps1` зелёный. Замороженные enum аудита/parity не затрагиваются.
> **Гоча (биллинг):** CI красный — гейт локальный (`build.ps1`); живая ISCC-компиляция `.iss` — за
> владельцем. Исполнитель — субагент (worktree); модель выбирает куратор.
> **После MLC-127 R9 закрыт полностью.** Дальше — R10-остаток (REL-11/DOC-10/BE-15/FE-17-18), R11, R12.
> _Источник детализации — `ROADMAP.md` R9 кластер 3; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..127` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-128` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-129`.** Куратор инкрементирует при выдаче; выдавать занятые
