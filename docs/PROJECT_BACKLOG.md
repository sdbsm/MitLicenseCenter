...
**`MLC-149` · Версия панели в подвале сайдбара** — трек «Релиз 0.5.0-beta» (полная постановка в
одноимённом разделе ниже). Одна задача за сессию; после закрытия куратор сдвигает NEXT TASK на
следующую по порядку трека (149→148→147→027→150→151→152).
...
**Принятые в работу** задачи по итогам приёмки `0.4.0-beta` (2026-06-14) + запросы владельца.
Минор (security + bug + фичи + служебное), **одним релизом** (решение владельца). Обзор —
`ROADMAP.md`; полная спека трека (развилки, паттерны-зеркала, файлы) — план-файл
`.claude/plans/synthetic-seeking-hedgehog.md` (у куратора). **Strategy B отклонена** — см.
`ROADMAP.md` п.2. Порядок исполнения — от изолированного к рискованному (149→148→147→027→150→151→152).
Бамп `<Version>`→`0.5.0-beta` (`backend/Directory.Build.props`) — **при закрытии трека**, не сейчас.

### MLC-149 · Версия панели в подвале сайдбара · Frontend · P2 · S · ← NEXT TASK

**Задача.** Установленная панель нигде в UI не показывает версию (только «Программы и компоненты» /
`/api/v1/health`) — после апгрейда владелец не видит, накатилось ли. Подвал сайдбара рисует
приглушённую строку версии.
**Подход.** Источник — рантайм-версия бэкенда через анонимный `GET /api/v1/health`
(`HealthEndpoints.cs:21-34`). Новый хук `features/health/useHealth.ts` (образец
`features/dashboard/useDashboardSummary.ts`) + `SidebarFooter` (уже экспортирован
`components/ui/sidebar.tsx`) в `components/layout/Sidebar.tsx`. **Дрейф:** `frontend/package.json`
уже несёт `"version"` — в UI его не показывать (источник правды = `Directory.Build.props`).
**Готово.** В подвале сайдбара видна версия установленной панели; при недоступном `/health` строка
не ломает шелл; `build.ps1` зелёный.

### MLC-148 · Холодный старт: /sessions пуст + «обновлено ~2000 лет назад» · Backend/Frontend · P2 · Bug · S

**Симптом (стенд, 2026-06-14).** Сразу после установки/рестарта `/sessions` пуст и «Обновлено
`<огромное число>` назад», хотя сеанс есть (виден на `/performance`); самоисцеляется ~60 с.
**Первопричина.** `ActiveSessionSnapshotStore` инициализирован `DateTime.MinValue`
(`.../Jobs/ActiveSessionSnapshotStore.cs:8`); джоба `cold-snapshot` (CRON `* * * * *`,
`Program.cs:360-363`) на свежем старте срабатывает с задержкой; `MinValue` рендерится `RelativeTime`
(`components/ui/RelativeTime.tsx`) как «~2000 лет назад». Не регрессия `MLC-144a`.
**Фикс (оба малых).** (1) прогрев: разово `BackgroundJob.Enqueue` `cold-snapshot` при старте;
(2) защита: при `MinValue` показывать «ещё не обновлялось» — **holistically** в `RelativeTime`
(другие страницы тоже через него).
**Готово.** На свежем старте `/sessions` либо сразу с сеансами, либо честная метка «ещё не
обновлялось»; прочие страницы с `RelativeTime` защищены; parity; `build.ps1` зелёный.

### MLC-147 · xlsx (SheetJS) — устранить 2 high-уязвимости · Security · P2 · S

**Задача.** `pnpm audit` на `0.4.0-beta`: 2 **high** в `xlsx` 0.18.5 — Prototype Pollution
(`GHSA-4r6h-8v6p-xvw6`, фикс ≥0.19.3) и ReDoS (`GHSA-5pgg-2g8v-p4x9`, фикс ≥0.20.2); фикс-версии
**не в npm** (только CDN SheetJS). Риск низкий: `xlsx` — только **экспорт** (`toXlsx`/`recordingXlsx`/
`export-xlsx`), не парсинг недоверенного ввода. Пред-существующее (трек экспорта `MLC-051/052`).
**Варианты (выбрать).** (a) CDN-версия SheetJS (pin `cdn.sheetjs.com/xlsx-0.20.x/...tgz`, тот же
API) — **рекомендация**; (b) `exceljs`; (c) принятый риск в `SECURITY.md` + исключение `pnpm audit`.
**Готово.** Уязвимости устранены **или** осознанно приняты записью в `SECURITY.md`; `build.ps1`
зелёный.

### MLC-027 · Разбить `i18n/ru.json` на per-feature файлы · Frontend/Maintainability · P3 · S

**Задача.** Все надписи UI — один плоский `frontend/src/i18n/ru.json` (1208 строк, 44 top-level
ключа). Триггер «> 1000 строк» сработал. (Перенесена из «Открытых опций».)
**Подход (держит S).** Разнести исходник на `i18n/ru/<feature>.json`, но в `i18n/index.ts` **слить
в один namespace** `translation` — 30+ `useTranslation()` call-sites не трогаются (ключи 1:1).
Полноценные i18next-namespaces (с миграцией call-sites) **не делаем** (M-объём без выгоды при RU-only).
**Готово.** `ru.json` разнесён, собирается в тот же словарь, ни один ключ не потерян; UI идентичен;
`build.ps1` зелёный.

### MLC-150 · Серверный фильтр «Базы» по «не найдена в кластере» · Backend/Frontend · P2 · S–M

**Задача.** Метка «Не найдена в кластере» (обратный дрейф) на «Базах» есть, но фильтр по ней не
серверный; у владельца ~100 баз (среднее по инсталляциям) — серверный фильтр держит список быстрым.
**Подход.** BE: параметр `notInCluster` в `GET /infobases` (`InfobasesEndpoints.cs:38-124`), источник
«missing» — снапшот `UnassignedInfobasesCache` (TTL 60 с): `clusterIds` HashSet,
`Where(x => !clusterIds.Contains(x.ClusterInfobaseId))`. **Развилка:** при недоступном RAS
(`Available==false`) не путать «нет missing» с «не знаем» — честная ошибка/флаг (не ложный «0»).
FE: параметр в `useInfobases.ts` + чек в тулбаре `InfobasesPage.tsx`; Zod-parity (`types.ts`).
**Готово.** Серверный фильтр работает; недоступность RAS — честно; parity; `build.ps1` зелёный.

### MLC-151 · Optimistic concurrency на Infobase/Publication · Backend/Frontend · P2 · M

**Задача.** Защита от затирания при одновременной правке (rowversion) есть только у `Tenant`
(MLC-136). Распространить на Infobase и Publication.
**Подход — зеркало `Tenant` (MLC-136, R12c):** `byte[]? RowVersion` в домене + `.IsRowVersion()`
(`AppDbContext.cs`) + токен в контрактах (omit-null) + `OriginalValue` →
`catch DbUpdateConcurrencyException` → 409 (новые коды по образцу `TENANT_CONCURRENCY_CONFLICT`,
`Problems.cs`) + аддитивная миграция (образец `..._MLC136TenantRowVersion`) + тесты-зеркало
`TenantConcurrencyTests.cs` + FE `omittable(z.string())` и обработка 409.
**Развилка.** Publication обновляется внутри агрегата Infobase
(`UpdateInfobaseRequest.Publication`) — по умолчанию токен только на Infobase (корень агрегата);
отдельный токен Publication — лишь если есть её самостоятельный update-путь.
**Готово.** Конкурентная правка со устаревшим токеном → 409 + внятное сообщение; backward-compat
при `null`; миграция аддитивна; parity; `build.ps1` зелёный.

### MLC-152 · Low-priv аккаунт службы (ADR-44) · Security/Backend/Installer · P2 · M–L · 2 фазы

**Задача (SEC-05, ADR-44).** Служба работает под `LocalSystem` / аккаунтом в Administrators — прав
больше потребности. Триггер ADR-44 (runtime-трасса) активирован.
**Фаза 1 — разведка (нужно участие владельца).** Procmon-трасса `MitLicenseCenter.Web.exe`
(+`iisreset.exe`) на стенде при всех сценариях (публикация/снятие/recycle/start/stop пула и сайта/
iisreset/бэкап SQL/DMV перфа/bootstrap) → документ минимально нужных прав:
`inetsrv\config` (точки `ServerManager` — `OneCIisLifecycleService.cs:44-201`), `W3SVC`/`WAS`,
SQL-роли (BACKUP/VIEW SERVER STATE/миграции), ACL key-ring + `appsettings.Production.json`.
**Фаза 2 — сузить.** Выделенный low-priv аккаунт (`SeServiceLogonRight` + узкий read-ACE / `IIS_IUSRS`
вместо Administrators; точечные SQL-роли). Правки `installer/MitLicenseCenter.iss`; ADR-44 →
реализовано; `SECURITY.md` §6/§9, `OPERATIONS`/`INSTALL`.
**Риск.** Недостаточный ACE ломает функции **молча**/на старте — фаза 1 обязательна; `.iss` через
`build-installer.ps1` (ISCC) до merge; живой прогон на ВМ/стенде — за владельцем.
**Готово.** Служба под low-priv аккаунтом с минимально нужными правами; все функции подтверждены на
стенде; ADR-44 → реализовано; канон безопасности обновлён; ISCC-компиляция зелёная.
...
`MLC-001..146` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
`MLC-036` — ROADMAP «RAS Strategy B», отклонена). Трек релиза `0.4.0-beta` (`MLC-140..144`)
**завершён** — индекс в `PROJECT_BACKLOG_ARCHIVE.md`. `MLC-146` — уборка мёртвого кода после MLC-144.
**Активен трек «Релиз 0.5.0-beta»** (`MLC-147`/`148`/`149`/`027`/`150`/`151`/`152`) — см. одноимённый
раздел; NEXT TASK = `MLC-149`. Полные отчёты — в git-истории (коммиты `MLC-NNN:`/PR).

**Следующий свободный номер: `MLC-153`.** Куратор инкрементирует при выдаче; выдавать занятые
