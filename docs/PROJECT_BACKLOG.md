...
> **`MLC-062` — Движок таблиц `@tanstack/react-table`** (постановка пользователя; запускается в **plan**-режиме
> — исполнитель сам исследует и предлагает детальный план). Перевести списковые таблицы панели (клиенты,
> базы, публикации, сессии, аудит, пользователи) с ручной вёрстки на готовый движок таблиц, чтобы получить
> производные UX из дизайн-канона `06_UI_DESIGN.md` §6: **меню видимости колонок**, **density-toggle**
> (компактно/комфортно, в localStorage), **сериализация фильтров в URL** (шаринг отфильтрованного вида
> ссылкой). Снимает пункт «не построено в v1» из `ROADMAP.md` / `06`. Жёсткие рамки: поведение существующих
> таблиц **1:1** (колонки/сортировка/серверная пагинация/polling/фильтры сохраняются), `StatusBadge` и
> code-splitting не ломать, RU-only. Объём M–L — исполнитель в плане предлагает фазирование (например, ввести
> движок на одной таблице, затем раскатать). Детальную спеку формирует план исполнителя.
>
