...
**Нет активной задачи.** Трек «Сервер» завершён полностью (`MLC-212..222`, релиз **1.1.3**,
...
и планов обслуживания SQL с детализацией по задачам. Все `MLC-212..222` Done и заархивированы (индекс —
...
- **Трек «Сервер»** — `MLC-212..222` (2026-06-19, выпущен в `v1.1.3`, **ADR-54/55/56**): раздел меню
...
(`MLC-212..222`, **ADR-54/55/56**) **завершён и выпущен в `v1.1.3`** (2026-06-19) — индекс в
`PROJECT_BACKLOG_ARCHIVE.md`. Активного трека нет; следующую работу ставит куратор постановочным PR.

**Следующий свободный номер: `MLC-223`.** (`MLC-222` — хотфикс DI-lifetime трека «Сервер», см. архив.)
