...
5. `MLC-066` — **Done (2026-06-08)** — Backend: 1С-сеансы/процессы «кто грузит». `IClusterClient`
   ...
   «Закрыто» ниже. `MLC-067` (frontend экран «нагрузка по сеансам») — следующая.
6. `MLC-068` / `MLC-069` — SQL DMV realtime (backend адаптер по ADR-20 + frontend). **Контур.**
7. `MLC-070` / `MLC-071` — Recording: запись по требованию (backend сущности+сервис+API, миграция;
...
> **`MLC-067` — Frontend: экран «нагрузка по сеансам/процессам»** (трек «Анализ быстродействия 1С»,
> Фаза 2; следующий по зафиксированному в плане порядку `066→067` — **финальное выставление за куратором**).
> Backend готов на `MLC-066`: `GET /api/v1/performance/onec-sessions` (Viewer) отдаёт `OneCLoadSnapshot`
> (сеансы с perf-полями `OneCSessionLoad` + рабочие процессы `OneCProcessLoad`), контракт сверить с
> `Performance/OneCLoadSnapshot.cs` + `Clusters/ClusterModels.cs`. Объём (из плана, Фаза 2): экран
> «кто грузит внутри 1С» в `features/performance/` — топ по `cpu-time-current`/`duration-current`,
> подсветка заблокированных (`blocked-by-dbms`/`-ls`≠0), зависших/молчащих (`last-active-at`),
> `available-perfomance` процессов; **отдельно** от `/sessions` (там задача — kill). Паттерны: polling 5с
> (`refetchInterval`/`placeholderData: prev`), **Zod-граница** на ответ (критично, MLC-016), i18n
> `performance.*`, плотная таблица. Семьи/measuring-честность как на `MLC-065`.
>
> Полная спека — `C:\Users\andre\.claude\plans\spicy-discovering-torvalds.md` (Фаза 2, `MLC-067`).
> Зависит от 066 (Done). Оценка S–M. Дальше — Фаза 3 (SQL DMV, `MLC-068/069`).
>
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 054, 055, 056, 057, 058, 059, 060, 061, 063, 064, 064a, 065, 066) — индекс
