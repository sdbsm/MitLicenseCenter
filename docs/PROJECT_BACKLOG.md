...
**`MLC-173` · Maintainability · P3 · Medium · Module Installer · File `installer/MitLicenseCenter.iss`** —
рефакторинг установщика (Трек 1 из 2): разрезать монолитную секцию `[Code]` (~1600 строк) на
`#include`-модули под `installer/include/` — 8 файлов в текущем текстовом порядке: `code-declarations`
(const/type/external + var) / `code-input-accessors` / `code-service-control` / `code-strings-connstr` /
`code-config-acl` / `code-service-sql` / `code-sql-test` / `code-wizard`. Главный `.iss` сохраняет
директивные секции (1–150) + наглядную карту `#include` под `[Code]`. **Поведение байт-в-байт без
изменений** (ISPP `#include` — дословная вставка строк; препроцессорный вывод идентичен монолиту).
Инвариант: конкатенация include-файлов == исходные строки 151–1748 побайтно; include — UTF-8 c BOM,
без повторного `[Code]`-заголовка. Канон `INSTALL`/`DEVELOPMENT` правится в **том же PR** (doc-driven).
`build-installer.ps1` и CI не трогаются (путь `#include` резолвится относительно главного `.iss`).
Гейт — ISCC-компиляция (`build-installer.ps1 -SkipPublish`) + байт-сверка. **Версию не бампим**
(поведение идентично, остаётся `0.6.6-beta`). Подготовка к Треку 2 (редизайн GUI). Спека — план-файл
`atomic-coalescing-wave.md`.

**За владельцем (не блокирует MLC-173):** живая проверка `0.6.6-beta` на стенде по `MLC-170/171/172`
...
`MLC-001..172` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
