...
4. `MLC-078` · Frontend · P2 · **NEXT TASK** — Frontend + e2e: кнопка «Сделать бэкап» на карточке инфобазы
   (Viewer) + диалог списка/статуса с поллингом 5с + тост 409, Admin-удаление; секция настроек бэкапа на
   `/settings`; i18n; present-tense `04`/`05`/`OPERATIONS`. **Live-verify на стенде обязателен**
   (урок [[api-omits-null-fields]]). Последняя задача трека.

---

## NEXT TASK

> **`MLC-078`** — Frontend + e2e трека «Резервное копирование баз SQL» (см. трек выше, последняя задача):
> кнопка «Сделать бэкап» на карточке инфобазы (Viewer+Admin), диалог бэкапов с поллингом и статус-бейджами,
> Admin-удаление, секция настроек `Backup.*` на `/settings`, i18n, Zod-граница через `omittable()`
> (nullable-поля `BackupSummary` опускаются на проводе). **Live e2e на стенде обязателен.**
> Полная спека — `.claude/plans/lazy-cuddling-finch.md`.
>
> `[Doc divergence]` с MLC-076 (Recording-ключи в каталоге 04 §4) **закрыт куратором** вариантом (а) —
> таблица дополнена тремя строками `Performance.Recording*`, резолюция отмечена в Done-отчёте MLC-076.
>
