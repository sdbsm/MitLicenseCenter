...
> **`MLC-123` — R7: Устойчивость джобов и службы.** Backend · P3 · Medium. Зависимость: независима.
> Цель — устойчивость recurring-джобов и Windows-службы к сбоям/перезапускам; все правки в
> `Infrastructure/Jobs` и инсталляторе. Состав (находки `audit/2026-06/TECH-DEBT.md`):
> - **BE-19:** TTL-reaper для застрявших `Running`-бэкапов — сейчас застрявший бэкап
>   восстанавливается только при рестарте процесса (retention-джоб его не чистит), и база
>   навсегда выпадает из бэкапов до рестарта панели. Нужен таймаут-репер, переводящий зависший
>   `Running` в `Failed` по TTL.
> - **BE-20:** прокинуть реальный `CancellationToken` во все 5 recurring-джобов (сейчас
>   `CancellationToken.None`) — graceful-прерывание при остановке службы, чтобы snapshot-бакет не
>   терялся при негладком завершении.
> - **BE-21:** осознанный `AutomaticRetry` на retention/идемпотентных джобах (сейчас при устойчивом
>   сбое БД ретраятся дефолтные 10 раз — только шум красными джобами в Hangfire).
> - **REL-22:** явные retry-политики + экспирация `Failed`-джобов (сейчас копятся в Hangfire,
>   оператору трудно отличить норму от постоянного сбоя) + раздел «Джобы и устойчивость» в
>   `OPERATIONS`.
> - **REL-03:** recovery-политика Windows-службы (авто-перезапуск) + зависимость
>   `depend= MSSQLSERVER` в установщике — сейчас после ребута хоста при медленном старте MSSQL
>   служба падает и не перезапускается (панель лежит до ручного вмешательства).
> Критерии готовности: зависший `Running`-бэкап автоматически экспирирует по TTL; джобы
> прерываются по `CancellationToken` при остановке службы; retry-политики джобов осознанные и
> `Failed` не копятся бесконечно; служба авто-перезапускается после сбоя и не стартует раньше SQL;
> **canon обновлён в том же PR** (`OPERATIONS` — раздел устойчивости джобов/службы; ADR при
> изменении операционной модели службы); `build.ps1` зелёный. Затронут enum аудита — не трогать;
> новые аудит-записи (если появятся) — по правилам R3 (`EnlistAudit`/`LogAsync`). Исполнитель —
> субагент (worktree); модель выбирает куратор. Куратор ревьюит, вливает, ставит R8 в `NEXT TASK`.
> _Источник детализации — `ROADMAP.md` R7; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..122` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-123` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-124`.** Куратор инкрементирует при выдаче; выдавать занятые
