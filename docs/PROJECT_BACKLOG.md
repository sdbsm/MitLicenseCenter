...
**Нет активной задачи.** Трек «Сервер» завершён полностью (`MLC-212..226`, релиз **1.1.3**,
**ADR-54/55/56**): новый раздел меню «Сервер» (7→8) с вкладками «Службы»/«IIS»/«Обслуживание» —
статус и жизненный цикл служб стека (сервер 1С/RAS/IIS; SQL только наблюдение), рабочие процессы 1С
(`rphost`) со списком и рестартом, расписание авто-рестартов сервера 1С, мониторинг свежести бэкапов
и планов обслуживания SQL с детализацией по задачам. Все `MLC-212..226` Done и заархивированы (индекс —
...
**Следующий свободный номер: `MLC-227`.** (`MLC-222..226` — хотфиксы трека «Сервер» по приёмке на стенде: DI-lifetime старта, квотирование `sc`, таймаут верификации, полная остановка перед стартом, единицы памяти rphost; см. архив.)
