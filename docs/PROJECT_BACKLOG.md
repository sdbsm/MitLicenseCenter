...
**Трек «Свежесть данных на страницах» (MLC-154..156)** — устранение задержки появления данных.
Аудит выявил скрытый дефект: cold-обход сеансов шёл раз в 60с вместо задуманных 25с (минимум
CRON Hangfire = 1 мин делал `Polling.ColdIntervalSeconds` инертной), отчего /sessions и дашборд
для обычных клиентов отставали до минуты. Спека — `.claude/plans/parsed-mixing-puppy.md`.

- **MLC-155** — `refetchOnWindowFocus: true` (застой админ-списков). **Done** (PR #233).
- **MLC-154** — cold-цикл → `ColdTierPollingService : BackgroundService`, каданс реален (дефолт
  15с), снят мёртвый throttle + Hangfire-регистрация cold; ADR-6.1 Update. **In Progress** (этот PR).
- **MLC-156** — `NEXT`: `LiveControls` (Пауза/Возобновить + «Обновить сейчас») на /sessions и
  «Быстродействии»; на /sessions «Обновить сейчас» = live-форс cold-прогона через эндпоинт
  (сигнал `ColdTierPollingService`); «Обновить сейчас» на админ-списках. Зависит от MLC-154.
...
**Следующий свободный номер: `MLC-157`.** Куратор инкрементирует при выдаче; выдавать занятые
