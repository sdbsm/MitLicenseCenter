...
**Нет активной задачи.** `MLC-170` (служба под virtual service account, ADR-49) **реализован**
(ISCC-компиляция зелёная) — **остаётся за владельцем:** живая проверка на стенде (чистая установка
под virtual account → служба под `NT SERVICE\MitLicenseCenter`, в конфиге нет SQL-пароля, в SQL есть
логин с `sysadmin`, health `ready`, секреты 1С расшифровываются, нет 18456; апгрейд; негатив «админ
не sysadmin» → внятный отказ; ручная миграция B→A по `OPERATIONS.md §8`). Следующую задачу/трек
