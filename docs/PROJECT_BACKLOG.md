...
> Phase 1 (PERF-01..04 = MLC-037/039/038/040) закрыт полностью; из Phase 2 закрыты `MLC-041` и `MLC-043`**
> (PERF-05 кросс-вызовный кэш UUID кластера — спавны ×2 вниз; PERF-07 батч-загрузка публикаций в
> `DriftCheckJob` — N+1→1 на проход; см. архив). Последним выполнен `MLC-043`; остальные пункты перф-плана
> (`compressed-giggling-grove.md`, PERF-06 / PERF-08+) — gated. Остаются только **отложенные опции**
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 043) — индекс
