...
> **`MLC-110` — Защита секретов на диске: ACL + честный ADR-8 (KEYRING-01(в) + SEC-04 + SEC-02).**
> Решение владельца — вариант (в): ключи остаются переносимым plaintext, защита — NTFS ACL.
> Объём: (1) инсталлятор, оба режима установки — ужесточить ACL `%ProgramData%\MitLicenseCenter`
> и `appsettings.Production.json` (только SYSTEM / Administrators / сервис-аккаунт);
> (2) `SettingsStore`: `CryptographicException` (битый/чужой key ring) → понятная ошибка
> оператору, не сырой стектрейс; (3) ADR-8 в `docs/DECISIONS.md` переписать честно: «ключи
> переносимы, plaintext, защита — NTFS ACL; trade-off: переносимость бэкапа важнее at-rest
> шифрования». SECURITY.md/04 НЕ трогать (перепишет D1). Критерии: после чистой установки
> (оба режима) `icacls` каталога keys и конфига показывают только SYSTEM/Administrators/
> сервис-аккаунт; ADR-8 обновлён; тест обработки `CryptographicException`. Полная постановка —
> `.claude/plans/audit-fix-prerelease.md`; findings — `audit/2026-06/MASTER-REPORT.md`
> (A5·DOC-01, A2·SEC-04, A2·SEC-02). Исполнитель — субагент (opus, worktree); куратор
> ревьюит, вливает, ставит следующую задачу трека.
