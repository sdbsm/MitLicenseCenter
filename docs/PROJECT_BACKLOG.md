...
**Нет активной задачи.** `MLC-227` (отзыв SCM-зависимости службы от SQL, пересмотр ADR-40) закрыт и
выпущен в `v1.1.4` (2026-06-19) — индекс в `PROJECT_BACKLOG_ARCHIVE.md`. Активного трека нет.

Следующую задачу/трек ставит **куратор постановочным PR**. Кандидаты — «Открытые опции» ниже и
`ROADMAP.md`.
...
**Следующий свободный номер: `MLC-228`.** (`MLC-227` — отзыв SCM-зависимости службы от SQL, закрыт в `v1.1.4`; `MLC-222..226` — хотфиксы трека «Сервер» по приёмке на стенде: DI-lifetime старта, квотирование `sc`, таймаут верификации, полная остановка перед стартом, единицы памяти rphost; см. архив.)
