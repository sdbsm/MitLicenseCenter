...
> **`MLC-087` + `MLC-088` — объединённая сессия 1 трека «Этап 2: single-host бек-чистка»**
> (постановки — в секции трека ниже). Один PR, коммиты раздельно (`MLC-087: …` / `MLC-088: …`).
> Перед стартом: `git fetch` + FF, ветка от `main`; страховочный тег `v1-pre-singlehost-stage2`
> поставлен куратором (плюс прежний `v1-pre-ux-redesign`); **перед накаткой миграций на стенд —
> бэкап БД** (БД + DPAPI key ring — единый юнит, см. CLAUDE.md).
...
- `MLC-087` (ST-A) · Backend+Frontend · M · **NEXT TASK (сессия с `MLC-088`)** — **SQL-инстанс:
  настройка — единственный источник** (§7 п.2/4/5). Переименовать ключ `Defaults.DatabaseServer`
  → `Sql.Server` (или эквивалент): `SettingDefinitions` + сидер + миграция значения в `dbo.Settings`
  + FE-привязка секции «SQL Server». `GET /discovery/databases` — убрать параметр `server=`
  (сервер из настройки на BE); `GET /discovery/sql-instances` остаётся (пикер /settings).
  Сузить `ISqlInstanceDiscovery`/`SqlDatabaseDiscovery` — без параметра сервера. FE: discovery
  имён БД без server; пикер на /settings не ломать.
- `MLC-088` (ST-B) · Backend+Frontend · M — **удалить `Infobase.DatabaseServer`** (§7 п.1,
  решение «чисто»). Дроп колонки + миграция; всех читателей колонки (grep: публикации/webinst,
  бэкапы, отчёты, перф) переключить на настройку. Контракты POST/PUT инфобазы: убрать поле
  `databaseServer`; FE — убрать скрытое поле формы (`useInfobaseForm`); правила валидации поля —
  снять с **обеих сторон** + parity-тесты. Деталь из `MLC-082`: «правка не мигрирует сервер базы
  молча» — после удаления колонки вопрос исчезает, поведение описать в отчёте.
- `MLC-089` (ST-C) · Backend · S — **удалить ключ `OneC.Cluster.Server`** (§7 п.3): из
...
- `MLC-090` (ST-D) · Backend+Frontend · S — **фильтр «статус публикации»** (принятый кандидат
...
- `MLC-091` (ST-E) · Docs · S — **финальный док-PR этапа 2**: новый ADR «Single-host topology»
...
`MLC-001..086` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
