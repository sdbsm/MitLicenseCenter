...
Текущий реестр (MLC-023..028; MLC-020..022 закрыты) — результат **анализа технического долга и поддерживаемости**
...
3. **MLC-023** — средний-высокий: дешевле расширять главную форму (M); опереться на извлечённый `validation.ts`. → **NEXT TASK**.
...
> **MLC-023 — Декомпозиция InfobaseFormDialog (787 строк).**
> Главная форма продукта — god-component (create+edit, prefill с `touched`-ref, маппинг 409).
> Декомпозировать без смены поведения: `useInfobaseForm`, `PublicationFieldset`,
> `mapConflictToField`. Удобно опереться на уже извлечённый `validation.ts` (MLC-022). Объём M.
