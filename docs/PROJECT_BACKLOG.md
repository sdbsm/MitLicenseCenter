...
> **Нет активной задачи.** Пострелизный трек рефакторинга **R1–R12 закрыт полностью**
> (`MLC-118..139`, 2026-06-13..14) — весь остаточный техдолг пред-релизного аудита отработан
> по итерациям `ROADMAP.md`; `build.ps1` зелёный (857 BE + 523 FE). Индекс закрытых задач —
> `PROJECT_BACKLOG_ARCHIVE.md` (поиск по `MLC-NNN`). Следующую задачу/трек заводит куратор
> постановочным PR. Отложенные опции и инфраструктурные отложенности — ниже и в `ROADMAP.md`.
...
`MLC-001..139` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции»; активной задачи нет — пострелизный трек R1–R12 закрыт полностью; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
