...
> **`MLC-072` — Верификация корректности метрик раздела «Быстродействие»** (диагностика; постановка
> пользователя 2026-06-09). Трек `MLC-063..071` завершён 9/9, но это **follow-up на сомнение в точности**.
> **Повод:** оператор заметил расхождение — `sqlservr` CPU в разделе показывает всплеск, которого **нет** в
> Диспетчере задач. Абсолютные значения метрик трека **независимо с эталоном не сверялись** (live-verify
> проверяла отображение/схему, не точность чисел). **Это research/диагностика — прод-код НЕ менять; выхлоп =
> вердикт куратору** (где расхождение, баг или объяснимо методикой; при подтверждённом баге — отдельный
> фикс-`MLC`).
> **Объём:** на живом **нагруженном** стенде снять ОДНОВРЕМЕННО наш `GET /performance/host` (+`/onec-sessions`,
> `/sql`) и эталоны, сверить по каждой метрике:
> - host CPU%/очередь/RAM/диск-латентность ↔ `Get-Counter` (`\Processor(_Total)\% Processor Time`,
>   `\System\Processor Queue Length`, `\Memory\Available MBytes`, `\PhysicalDisk(_Total)\Avg. Disk sec/Read|Write`)
>   ↔ Диспетчер задач / Resource Monitor;
> - **CPU% процессов/семей — фокус `sqlservr`/`rphost`** ↔ `Get-Counter '\Process(<name>)\% Processor Time'`
>   (делить на число ядер!) ↔ ДЗ. Проверить в `OneCHostMetricsProbe.BuildSamples`: нормировку на
>   `Environment.ProcessorCount` (есть), **временной лаг** (5с-усреднение + FE `placeholderData` держит прошлый
>   кадр → всплеск «висит» дольше ДЗ), рассинхрон метки времени (`now` берётся до `ReadProcesses`), turbo/базовую
>   частоту;
> - rac-метрики сеансов/процессов ↔ `rac session list`/`process list` напрямую; SQL-снимок ↔ прямой запрос к DMV.
> **🔴 Точка синхронизации:** перед сверкой исполнитель ОСТАНАВЛИВАЕТСЯ и ждёт «поехали» — пользователь
> воспроизводит нагрузку (перепроведение/тяжёлый отчёт), как на `MLC-063`. Вердикт + рекомендации — куратору.
>
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 054, 055, 056, 057, 058, 059, 060, 061, 063, 064, 064a, 065, 066, 067, 068, 069, 070) — индекс
