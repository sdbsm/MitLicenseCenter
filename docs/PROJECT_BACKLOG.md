...
> **`MLC-132` — R11d: FE-09 — Zod-валидация на широких границах вместо `payload as T`.**
> Frontend-only · P3 · Medium. Завершающий кластер R11 (a/b/c закрыты `MLC-129..131`). Сейчас рантайм-Zod
> только на 3 критичных границах (ADR-10.1/MLC-016: `currentUserSchema`, snapshot сеансов, списки
> инфобаз/клиентов/host-метрики/paged-бэкапы/записи); остальные ~35 ответов читаются слепым `api<T>()` /
> `payload as T` (raw-касты в ~20 хуках: `useDiscovery` 6, `useIisManagement` 12, `usePublications` 4,
> `useInfobases` 5, `useUsers` 5, `useSettings`, `useReports`/`useLicenseUsage`, `useUnassignedInfobases`,
> `useAuditRetention` и др.). Расширить Zod-покрытие, чтобы дрейф контракта FE↔BE ловился рантаймом, а не
> падал глубже в UI.
> Состав:
> - Для каждого значимого read-ответа без схемы — описать Zod-схему (зеркало DTO) в `types.ts` фичи и
>   подключить через `api(url, { schema })`. **Гоча omit-null** (ADR-32 / память `api-omits-null-fields`):
>   nullable-поля — `omittable()`/`.nullish()`, НЕ `.nullable()` (иначе схема рушится на опущенном null).
>   **Enum-поля строкой** (JsonStringEnumConverter): `z.enum(...).or(z.string().transform(...))` —
>   неизвестное будущее значение НЕ должно ронять список (образец `backupStatusSchema`).
> - Типы выводить `z.infer` (не дублировать интерфейсы вручную, как в существующих фичах).
> - Объём управляемый: исполнитель может идти по фичам; начать с самых рисковых (`useIisManagement`,
>   `useDiscovery`, `usePublications`). Полное покрытие всех read-эндпоинтов — цель; мутации-ответы с
>   тривиальным телом (`null`/echo) можно пропустить, отметив.
> - Канон 05 §5.3 (список критичных границ → расширить) + при необходимости ADR-10.1-нота.
> Не трогать backend/parity-валидацию инфобаз/enum аудита. Критерии готовности: значимые read-границы
> валидируются Zod (omit-null/enum-устойчивость соблюдены); `build.ps1` зелёный; канон обновлён. Исполнитель
> — субагент (worktree); модель выбирает куратор.
>
> **План кластеров R11** (a ✅ `MLC-129`, b ✅ `MLC-130`, c ✅ `MLC-131`):
> - **R11d `MLC-132`** — FE-09: расширение Zod-границ (~35 эндпоинтов). ← _текущий, завершает R11_
...
`MLC-001..131` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-132` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-133`.** Куратор инкрементирует при выдаче; выдавать занятые
