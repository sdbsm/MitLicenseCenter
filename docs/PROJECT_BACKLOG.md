...
3. `MLC-077` · Backend · P2 · **NEXT TASK** — Оркестрация + API: `BackupOrchestrator` + `BackupPumpService` + recovery;
...
> **`MLC-077`** — Backend, оркестрация + API трека «Резервное копирование баз SQL» (см. трек выше):
> `BackupOrchestrator` + `BackupPumpService` (насос: потолок `Backup.MaxParallel` пер-тик + замок-на-базу +
> FIFO; recovery осиротевших `Running`→`Failed/Interrupted` на старте) + `BackupRetentionJob` (recurring,
> TTL) + эндпоинты `/api/v1/backups` (list/detail/start=Viewer, delete=Admin; 409 `BACKUP_ACTIVE`) + DI +
> тесты на `FakeSqlBackupService` (+`BackupGate`). Полная спека — `.claude/plans/lazy-cuddling-finch.md`.
> Дальше `MLC-078` (frontend + e2e, последняя задача трека).
>
