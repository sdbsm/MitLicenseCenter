...
**`MLC-162` · Backend · P1 · High · Module: Ras** — фикс: обнаружение службы RAS не находит
существующую службу (детектор всегда «не зарегистрирована»). Патч `0.5.5-beta`.

- **Дефект (подтверждён на dev-сервере):** на хосте есть валидная служба
  `1C:Enterprise 8.5 Remote Server` (`…\8.5.1.1302\bin\ras.exe …`, Running) — дашборд видит RAS
  живой, а блок в Настройках показывает «не зарегистрирована». Причина — `ScRasServiceManager`
  перебирает службы через `sc query state= all` + `sc qc` **из-под `Process.ArgumentList`**, и
  `sc.exe` (нестандартный парсер опций `key= value`) этот вызов не принимает → энумерация пустая
  → `NotRegistered`. CIM/`Win32_Service` ту же службу находит одним запросом. Юнит-тесты это
  пропустили — они **мокали** `IScProcessRunner` (никогда не дёргали реальный `sc.exe`).
- **Фикс обнаружения:** заменить sc-перебор на **чтение списка служб из реестра**
  (`HKLM\SYSTEM\CurrentControlSet\Services\*\ImagePath`, фильтр по `ras.exe`) + состояние через
  `System.ServiceProcess.ServiceController` — один проход, без спавнов (заодно снимает перф-риск
  из ревью MLC-159). Парсеры версии/порта из `binPath` переиспользовать.
- **Фикс действий:** `sc create/config/start/stop` (`binPath=`/`start=` — тот же `key= value`)
  **проверить против реального `sc.exe`** и собирать командную строку так, как её принимает sc
  (как установщик, raw `Arguments`, не `ArgumentList`, если ArgumentList ломает). НЕ создавать
  реальные службы из автотестов; формат команды — юнит-тестом строки + проверка куратором на
  dev-сервере.
- **Проверка:** куратор после merge прогоняет живой `GET /ras-service/status` на dev-сервере
  (порт 8080) — должен показать существующую службу (`Ok`/`Outdated`), не `NotRegistered`.
- **Канон:** ADR-47 Update-нота (обнаружение через реестр/CIM, не sc-перебор); версия
  `0.5.5-beta` + ROADMAP. Спека — `.claude/plans/ras-service-autoregister.md` (раздел MLC-162).
...
`MLC-001..161` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
(`MLC-154..156`) и трек «Авто-регистрация службы RAS» (`MLC-159..161`, релиз `0.5.4-beta`, ADR-47)
**завершены** — индексы в `PROJECT_BACKLOG_ARCHIVE.md`.
`MLC-146` — уборка мёртвого кода после MLC-144. **Нет активного трека.** Полные отчёты — в
git-истории (коммиты `MLC-NNN:`/PR).

**Следующий свободный номер: `MLC-162`.** Куратор инкрементирует при выдаче; выдавать занятые
