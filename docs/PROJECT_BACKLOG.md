...
(ревизия канона) в основном; R2–R8 закрыты полностью (`MLC-118..124`); остаётся R9/R11/R12
(R10 — верификационный остаток). Отмечено в ROADMAP значком ✅.
...
> **`MLC-125` — R9: Инсталлятор hardening.** Backend/Security/Tooling · P2 · Large.
> Зависимость: часть по ACL — на R1 (закрыт пред-релизным треком); остальное независимо.
> Цель — ужесточить безопасность поставки и инсталлятора. Правки в установщике (`.iss`),
> бэкенде (security-заголовки, rate-limiting), publish/installer-скриптах и каноне.
> Состав (находки `audit/2026-06/TECH-DEBT.md`, детализация — `ROADMAP.md` R9):
> - **SEC-07:** добавить security-заголовки (CSP, `X-Frame-Options`, `X-Content-Type-Options`,
>   `Referrer-Policy`) — backend middleware; учесть SPA same-origin (ADR-30) и `EnableSwagger`.
> - **SEC-08:** rate-limiting на `/auth/login` (ASP.NET RateLimiter) — защита от
>   таргетированного lockout-DoS / перебора; аудит `LoginFailed=108` уже есть.
> - **SEC-05:** опция low-priv аккаунта службы вместо `LocalSystem` (режим B) — оценить
>   совместимость с key ring ACL и SQL-доступом; **enum аудита не трогать**.
> - **SEC-06 / SEC-09:** сузить firewall-правило (порт/профиль) + `localhost` bind по
>   умолчанию (не регресснуть штатный LAN-сценарий с `Urls: http://+:<порт>` — решение
>   оператора).
> - **REL-06:** явные требования к аккаунту службы в режиме A + IIS-права — канон
>   (`OPERATIONS`/`INSTALL`), без кода если только документ.
> - **REL-08:** убрать `*.pdb` / `appsettings.Development.json` / `web.config` из поставки
>   (publish-скрипт или `.iss` Excludes) — sanity-чек состава.
> - **REL-17:** лицензия Inno Setup (баннер «Non-commercial use» — оценить, фиксировать
>   решением, не блокер поставки).
> Критерии готовности: security-заголовки и rate-limiting покрыты тестами; состав поставки
> не содержит pdb/Development.json/web.config; firewall/bind не регрессят штатный сценарий;
> **canon правится в том же PR** (новые/изменённые **ADR** на security-решения — заголовки,
> rate-limiting, аккаунт службы; `OPERATIONS`/`INSTALL`/`SECURITY.md`); `build.ps1` зелёный;
> замороженные enum аудита и parity BE↔FE соблюдены, если затронуты. **Гоча (биллинг):**
> CI красный — гейт локальный (`build.ps1`). **R9 — крупная задача**; куратор вправе разбить
> на несколько подзадач/PR (`MLC-125`, `MLC-126`…) по кластерам (security-заголовки+rate-limit /
> аккаунт службы+firewall / чистка поставки), если объём одного PR слишком велик. Исполнитель —
> субагент (worktree); модель выбирает куратор. Куратор ревьюит, вливает, ставит следующую
> итерацию в `NEXT TASK`.
...
`MLC-001..124` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-125` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-126`.** Куратор инкрементирует при выдаче; выдавать занятые
