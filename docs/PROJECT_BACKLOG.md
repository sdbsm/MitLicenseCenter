...
5. `MLC-066` / `MLC-067` — 1С-сеансы/процессы: кто грузит (backend rac-расширение + frontend). **Контур.**
...
> **Нет активной задачи — ждёт куратора.** Последняя закрытая — `MLC-064a` (Done 2026-06-08, доработка
> Фазы 1, см. индекс прогресса выше). Следующая по треку — Фаза 2 `MLC-066` (backend 1С-сеансы «кто
> грузит»: расширить маппинг сеанса perf-полями + новая команда `IClusterClient.ListProcessesAsync`,
> состав подтверждён разведкой `MLC-063`; полная спека — `C:\Users\andre\.claude\plans\spicy-discovering-torvalds.md`).
> `NEXT TASK` выставляет куратор.
>
> **Отложенные опции**
> (`MLC-025/026/027/028/011(a)` + Phase 3–4 рефакторинг-трека / `MLC-036` RAS Strategy B, перф PERF-08+,
> `MLC-062` движок таблиц `@tanstack/react-table` → будущий UI-холистик-трек / Фаза 5 этого трека)
> остаются gated на триггеры — по умолчанию не берутся.
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 054, 055, 056, 057, 058, 059, 060, 061, 063, 064, 064a, 065) — индекс
