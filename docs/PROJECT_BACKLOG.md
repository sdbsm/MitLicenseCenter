...
> **`MLC-120` — R4: Тестовые слепые зоны.** Testing · P3/P4 · Low · backend + frontend.
> Чистая safety-net-итерация (без функциональных правок поведения): закрыть остаток R4
> (FE-01 «тесты ChangePasswordForm + ForcePasswordChange» уже сделан в `MLC-115`):
> - **BE-09:** `InfobaseValidationRules.AppendPublicationFieldErrors` и
>   `PublicationsEndpoints.UpdateAsync` — без поведенческих тестов; это единственный runtime-барьер
>   валидации публикаций (см. гочу `MLC-118`). Добавить поведенческие тесты (валид/невалид по длине
>   и метасимволам → 400 с ошибкой поля, не 500; happy-path → 200).
> - **BE-12:** CP866/OEM-декод `SystemProcessRacRunner` вне тест-контура; маркер идемпотентности kill
>   на русском литерале. Вынести декод в тестируемую единицу (или протестировать существующую) —
>   юнит-тест декодирования OEM/CP866 + маркера, чтобы регрессия (в т.ч. по стейл-комментарию BE-15
>   «UTF-8») ловилась. **Декод не менять — только покрыть тестом; поведение 1:1.**
> - **BE-14:** freeze-тест `AuditActionType` покрывает 11 из 43 членов — расширить до **полного**
>   (все объявленные имя↔int, включая заморожённые исторические и `LimitChanged=201`), чтобы случайная
>   смена незакреплённого числа падала. (В `MLC-119` добавлены частичные ассерты 200/201 — заменить
>   полной таблицей.)
> - **BE-24:** хрупкие тесты — timezone-зависимый assert (привязать к фиксированному offset/UTC),
>   `Task.Delay`-синхронизация (заменить на `TaskCompletionSource`/детерминированный сигнал),
>   smoke-пустышка. Убрать маскировку регрессий.
> - **FE-11:** kill-сессий, LoginPage, IIS-подсекция без тестов; схемы не прогоняются в хуках с полным
>   моком `api`. Добавить тесты этих флоу с прогоном Zod-схем.
> - **FE-19:** `StatusBadge.test.tsx` ассертит Tailwind-классы (ложно-красный при дизайн-рефакторе);
>   нет wire-fixture-теста для `dashboard/summary` (nullable-поля без схемной защиты). Переписать на
>   поведенческий ассерт + добавить wire-fixture с omit-null (урок `api-omits-null-fields`/MLC-067/071).
> Детали находок — `audit/2026-06/TECH-DEBT.md` (BE-09/12/14/24, FE-11/19) и `A1`/`A3`. Критерии
> готовности: перечисленные слепые зоны покрыты; **только тесты — продакшн-поведение не меняется**
> (если по ходу всплывает реальный дефект — не чинить в этой задаче, завести запись куратору);
> замороженные int `AuditActionType` не трогать; `build.ps1` зелёный. Исполнитель — субагент (opus,
> worktree); модель выбирает куратор. Куратор ревьюит, вливает, ставит следующую итерацию (R5) в `NEXT TASK`.
...
`MLC-001..119` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-120` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-121`.** Куратор инкрементирует при выдаче; выдавать занятые
