...
> **Активных задач в бэклоге нет.** Пред-релизный аудит А0–С1 и весь трек фиксов
> (`MLC-108..117`, включая живую ВМ-проверку установщика) закрыты; документация переписана
> с нуля от кода (канон v2, этап D1) и вновь является источником правды; продукт лицензирован
> (MIT). Канон `docs/` снова надёжен полностью. Пострелизный roadmap рефакторинга (R1–R12)
> перенесён в `docs/ROADMAP.md` «Backlog / deferred»; артефакты аудита — `audit/2026-06/`.
>
