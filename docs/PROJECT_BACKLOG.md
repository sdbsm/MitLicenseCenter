...
(ревизия канона) в основном; частично R2/R3/R4/R5/R7/R8 (отмечено в ROADMAP значком ✅).
Номера задачам присваиваются по мере выдачи (бэклог тонкий — одна `NEXT TASK` за раз).

## NEXT TASK

> **`MLC-118` — R2: Валидационный барьер (BE+FE, parity).** Backend+Frontend · P3 · Medium ·
> `Endpoints/Shared/InfobaseValidationRules.cs` ↔ `features/infobases/validation.ts` (+ зеркало FE).
> Закрыть остаток R2 (BE-03 уже сделан в `MLC-114`):
> - **BE-04 / FE-16:** max-длины строковых полей ловятся только nvarchar БД → 500 вместо 400
>   на мутационных эндпоинтах. Ввести проверку длины в общих хелперах валидации (обе стороны).
> - **SEC-11 / SEC-12 / UX-11:** `VirtualPath`, `PhysicalPathOverride`, `DatabaseName` без
>   проверки path/connstr-метасимволов — добавить валидацию символов и длины (обе стороны).
> - **BE-07 / SEC-13:** символы `;`/`=` в `Infobase.Name` уходят в connstr webinst —
>   запретить/экранировать (риск инъекции уровня парсера 1С, Admin-only).
> Детали находок — `audit/2026-06/TECH-DEBT.md` (строки BE-04/07, SEC-11/12/13, UX-11) и
> `A1/A2/A4`. Критерии готовности: невалидный ввод (длина/символы) даёт **400 с понятной
> ошибкой поля**, не 500 и не сырую ошибку БД; правила добавлены синхронно BE↔FE; parity-тесты
> зелёные; новых эндпоинтов/миграций нет. Замороженные enum не трогать.
> Исполнитель — субагент в worktree; **модель выбирает куратор по оценке сложности**
> (любая, без ограничений). Куратор ревьюит, вливает, ставит следующую итерацию (R3) в `NEXT TASK`.

**Отложенные опции** — см. «Открытые опции» ниже (`MLC-025/026/027`, `MLC-062`, `MLC-073`);
прочие инфраструктурные отложенности несёт `ROADMAP.md` «Backlog / deferred».
...
**Следующий свободный номер: `MLC-119`.** Куратор инкрементирует при выдаче; выдавать занятые
