...
**`MLC-170` (Security / Installer) — служба под virtual service account, отказ от SQL-аутентификации.**
Постановка зафиксирована в **ADR-49** (+ форвард-ноты ADR-31/ADR-44). Цель: **убрать SQL-пароль
с диска** — служба `MitLicenseCenter` переходит с LocalSystem+SQL-логин на **`NT SERVICE\MitLicenseCenter`**
(по умолчанию) либо именную Windows-учётку/gMSA (домен) с `Trusted_Connection` к локальному SQL.
Привилегии не снижаем (ADR-44): `sysadmin` + локальный админ остаются, меняется только идентичность.
**Объём (PR исполнителя):** только `installer/MitLicenseCenter.iss` + present-tense правки
`SECURITY.md`/`INSTALL.md`/`OPERATIONS.md` и приведение текста ADR-31/ADR-44 к факту (статус «изменён»)
в том же PR. Ключевое: режимы → `ACCT_VIRTUAL`/`ACCT_NAMED` (режим B убран); `BuildConnString` всегда
`Trusted_Connection`; `sc create/config obj= "NT SERVICE\…"` без `password=` (для gMSA — явный чекбокс,
для обычной доменной учётки — с паролем); **новое** — авто-создание SQL-логина (`CREATE LOGIN … FROM
WINDOWS` + `sysadmin`, идемпотентно, hard-fail при rc≠0, предусловие — установщик-админ = `sysadmin`);
добавление учётки в локальных Администраторов (`Add-LocalGroupMember -SID S-1-5-32-544`, best-effort);
грант ACL учётке (`icacls` по имени `NT SERVICE\…`); **рефактор порядка** `ssPostInstall` — учётка/SID
появляются только после `sc create`, поэтому ACL + провижининг логина переезжают на момент **после**
регистрации службы, а `sc start` — **последним**. Миграция уже стоящих режим-B инсталляций — **ручная
процедура** в `OPERATIONS.md` (установщик не авто-мигрирует). Бэкенд **не трогаем**. **Гейт:** `.iss`
обязан компилироваться ISCC (`scripts/build-installer.ps1`) ДО merge; живая проверка на стенде
(чистая установка + апгрейд + негатив «админ не sysadmin» + ручная миграция) — за владельцем. Полная
спека — план-файл `gleaming-sparking-cat.md` (локальный, у куратора). **Постановочный PR (ADR-49 +
бэклог) вливается ДО PR исполнителя.**
...
`MLC-001..169` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
**Следующий свободный номер: `MLC-171`.** (`MLC-170` выдан задаче «virtual service account», ADR-49.)
