...
**`MLC-160` · Frontend · P2 · Medium · Module: settings** — блок диагностики службы RAS +
лечение в Настройках (поверх готового BE-контракта MLC-159).

- **Что:** блок состояния службы RAS в секции «Подключение к 1С/RAS» (`SettingsPage`),
  Admin-only. 4 состояния по `state`: `Ok` (зелёный бейдж, кнопок нет) / `NotRegistered`
  («Зарегистрировать службу RAS» → `register`) / `Outdated` («Обновить службу на платформу
  X.Y.Z» → `update`) / `Stopped` («Запустить» → `start`). Статус — только через `StatusBadge`.
- **Диалоги-подтверждения** (стиль ADR-45 — действие обратимо, простое «да/нет»): показывают
  человеческое описание (служба / платформа / порт из `target`) **и** точную команду
  `commandPreview` (поле `sc …` из ответа). Если `targetReady=false` — показать `issue`
  (нет `ras.exe` / не выбрана платформа), действие заблокировать.
- **BE-контракт (MLC-159, готов):** `GET /api/v1/ras-service/status` → `{ state, service?,
  target?, commandPreview?, targetReady, issue? }` (camelCase, null-поля **опускаются** —
  Zod `.nullish()`, гоча `api-omits-null-fields`); `POST /api/v1/ras-service/{register|update|
  start}` → `{ state, serviceName }` (200) либо `409` ProblemDetails (`RAS_SERVICE_OPERATION_FAILED`,
  `detail` — человеческое сообщение). Всё Admin-only.
- **Перф/UX (из ревью MLC-159):** `/status` дёргать **лениво** — по раскрытию секции/кнопке
  «Проверить», НЕ на каждом заходе в Настройки; короткий кэш на ответ. (Причина — обнаружение
  перебирает все службы Windows.)
- **Parity + тесты:** Zod-схемы статуса/ответов + **omit-null** схема-тест; тесты диалогов;
  i18n. Канон — **ADR-47**; спека — `.claude/plans/ras-service-autoregister.md` (раздел MLC-160
  + блок «ОТКРЫТО для MLC-160»).

> Трек «Авто-регистрация службы RAS» (`MLC-159..161`, релиз `0.5.4-beta`). Порядок:
> `MLC-159` ✅ → **`MLC-160`** (FE: блок в Настройках) → `MLC-161` (FE: сигнал дашборда +
> бамп версии). Архитектура — ADR-47 + `ROADMAP.md` (релиз `0.5.4-beta`).

### Трек «Авто-регистрация службы RAS» — `MLC-159..161` (релиз `0.5.4-beta`)

- `MLC-159` — **Done** (PR #241, `15c8a66`). BE: порт `IRasServiceManager`/`ScRasServiceManager`
  поверх `sc.exe`, обнаружение по binPath с `ras.exe` (ФС не трогает → удалённая папка не
  роняет диагностику), 4 состояния, register/update/start (update перенастраивает ту же
  службу), эндпоинты `/ras-service/*` (Admin), аудит 600/601/602 (freeze + FE-зеркало).
  Команда `ras.exe cluster --service --port=<p> localhost:1540` — version-agnostic (8.3/8.5).
  **Для MLC-160:** `/status` вызывать лениво + кэш (обнаружение перебирает все службы).
- `MLC-160` — **NEXT TASK** (см. выше). FE: блок диагностики (4 состояния) + лечение в
  «Подключение к 1С/RAS», диалоги с точной командой `sc …`, Zod parity + omit-null тест.
- `MLC-161` — FE: карточка здоровья RAS на дашборде сигналит «не зарегистрирована»/«остановлена»
  со ссылкой в настройки (переиспользовать `RasHealthProbingService`); бамп версии
  `0.5.4-beta` (`frontend/package.json` + `README.md`) + ROADMAP release-секция → «выпущен».
...
`MLC-001..156` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
релиза `0.5.0-beta` (`MLC-147..152` + `MLC-027`) и трек «Свежесть данных на страницах»
(`MLC-154..156`) **завершены** — индексы в `PROJECT_BACKLOG_ARCHIVE.md`.
`MLC-146` — уборка мёртвого кода после MLC-144. **Нет активного трека.** Полные отчёты — в
git-истории (коммиты `MLC-NNN:`/PR).

**Следующий свободный номер: `MLC-162`.** (`MLC-159..161` выданы треку «Авто-регистрация службы
RAS».) Куратор инкрементирует при выдаче; выдавать занятые номера нельзя (прецедент коллизии
`MLC-053`/`054`).
