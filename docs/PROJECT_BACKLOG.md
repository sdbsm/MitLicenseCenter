...
**Нет активной задачи.** Патч `0.5.6-beta` (`MLC-163` — фикс «Проверить публикацию»: джоба статусов
несёт concurrency-токен `RowVersion`) **выпущен**; индекс — `PROJECT_BACKLOG_ARCHIVE.md`. Фикс
подтверждён регрессионным тестом на SQLite (без фикса — падает). **Остаётся за владельцем:** после
апгрейда панели на `0.5.6` убедиться, что «Проверить публикацию» отдаёт результат (не «произошла
ошибка»). Следующую задачу/трек ставит куратор постановочным PR. Кандидаты — «Открытые опции» ниже
и `ROADMAP.md`.
