...
> **`MLC-106` — Bootstrap создаёт БД, если её нет.** Backend-фикс первопричины (follow-up к
> «GUI-установщик»): на пустом инстансе `MigrateAsync` под `EnableRetryOnFailure` не создаёт БД
> (3 ретрая на 4060 → краш, БД не создаётся; воспроизведено на стенде). Фикс: ранний bootstrap-шаг
> `CREATE DATABASE` к master (до Hangfire-регистрации и Migrate). Затрагивает **backend** (ADR-18).
> Канон: **ADR-18** + `OPERATIONS`. План — `.claude/plans/mlc-106-bootstrap-create-db.md`. Status: **Approved (NEXT)**.
...
`MLC-001..105` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
...
**Следующий свободный номер: `MLC-107`.** Куратор инкрементирует при выдаче; выдавать занятые
