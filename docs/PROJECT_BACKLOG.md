...
> _Не назначена — активных задач нет._ Трек «Обратный дрейф панель↔кластер»
> (`MLC-095..097`) **закрыт 2026-06-11** — секция перенесена в архив, строка в
> «Завершённых треках» выше. Пункт `ROADMAP.md` «Дрейф панель↔кластер — решить
> обязательно» **закрыт полностью** (оба направления: треки `MLC-092..094` и
> `MLC-095..097`).
