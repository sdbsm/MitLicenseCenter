...
**Нет активной задачи.** Трек «Авто-регистрация службы RAS» (`MLC-159..161`, релиз
`0.5.4-beta`) завершён (3/3) — индекс в `PROJECT_BACKLOG_ARCHIVE.md`. **Живая проверка на
стенде** (регистрация / перерегистрация под новую платформу / запуск службы RAS, обе роли) —
за владельцем. Следующую задачу/трек ставит куратор постановочным PR. Кандидаты — «Открытые
опции» ниже (по триггеру) и `ROADMAP.md`.
