...
> **`MLC-135` — R12b: freeze-дисциплина для Perf-enum (BE-13).**
> Backend + тесты · P3 · Small. Второй кластер R12. `PerfRecordingStatus` (`Active/Stopped/Interrupted`)
> и `PerfRecordingStopReason` (`Manual/TimeLimit/SampleLimit`) в
> `backend/src/MitLicenseCenter.Application/Performance/PerfRecordingModels.cs` сериализуются в БД через
> `HasConversion<int>` (PerfRecording-сущность), но **значения неявные** (ordinal) и **freeze-тестов
> нет** — вставка члена в середину молча перепишет смысл исторических строк БД.
> - **Правка enum (аддитивно, без смены семантики):** проставить **явные** int-значения, РАВНЫЕ текущим
>   ordinal: `Active=0, Stopped=1, Interrupted=2`; `Manual=0, TimeLimit=1, SampleLimit=2`. Это
>   фиксация, а не перенумерация — исторические строки не затрагиваются. Комментарий «не
>   переисполь­зовать» у обоих enum уже есть — оставить/усилить.
> - **Freeze-тесты** по образцу `backend/tests/MitLicenseCenter.Tests.Unit/Backups/BackupModelsTests.cs`
>   (или `Audit/AuditLogEnumMappingTests.cs`): `[InlineData]` на каждый член `(int)Member == N` +
>   reflection-инвариант полноты (новый член без `[InlineData]` роняет тест — как BE-14 для
>   `AuditActionType`).
> **Гочи:** значения МЕНЯТЬ нельзя (заморожены де-факто — в БД лежат int'ы); миграция НЕ нужна (значения
> совпадают с ordinal, схема БД не меняется). Никаких parity/FE-правок (на проводе enum строкой —
> `JsonStringEnumConverter`, контракт не меняется). Канон — при необходимости отметить freeze-дисциплину
> Perf-enum в 04 (рядом с замороженными enum'ами аудита).
> Критерии готовности: оба enum с явными значениями (= ordinal); freeze-тесты + reflection-полнота;
> `build.ps1` зелёный. Исполнитель — субагент (worktree); модель — Sonnet (Small, backend+тесты, точечно).
> _Состав R12 далее: R12c BE-02 (optimistic concurrency, аддитивный rowversion + миграция),
> R12d/e/f UX-кластеры D/термины · E/доступность · F/онбординг. Источник — `ROADMAP.md` R12;
> первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..134` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-135` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-136`.** Куратор инкрементирует при выдаче; выдавать занятые
