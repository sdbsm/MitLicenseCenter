...
## Запланировано — следующая версия (кандидат `0.4.1-beta`)

Не «опции по триггеру», а **принятые в работу** задачи по итогам приёмки `0.4.0-beta` на
dev-стенде (2026-06-14). Трек/`NEXT TASK` откроет релиз-лид постановочно. Обзор — `ROADMAP.md`
«Планируется — следующая версия».

### MLC-147 · xlsx (SheetJS) — устранить 2 high-уязвимости · Security · P2

**Задача.** `pnpm audit` на `0.4.0-beta` (2026-06-14) показал 2 **high** в `xlsx` 0.18.5:
Prototype Pollution (`GHSA-4r6h-8v6p-xvw6`, фикс ≥0.19.3) и ReDoS (`GHSA-5pgg-2g8v-p4x9`,
фикс ≥0.20.2). **Нюанс:** фикс-версии **отсутствуют в публичном npm-реестре** — SheetJS
раздаёт их только со своего CDN, простым бампом не лечится.
**Контекст риска.** `xlsx` используется только для **экспорта** (генерация `.xlsx` из данных
панели), не для парсинга загруженных пользователем книг — а обе уязвимости срабатывают при
разборе вредоносного ввода → практическая достижимость низкая. **Пред-существующее** (трек
экспорта `MLC-051/052`), не из `0.4.0-beta`.
**Файлы.** `frontend/package.json` (`"xlsx": "^0.18.5"`); места использования — фичи экспорта
(чанки `toXlsx`/`recordingXlsx`/`export-xlsx`).
**Варианты (выбрать в задаче).** (a) перейти на CDN-версию SheetJS (pin
`https://cdn.sheetjs.com/xlsx-0.20.x/...tgz` в `package.json`) — закрывает обе CVE, тот же API;
(b) заменить на поддерживаемую библиотеку (напр. `exceljs`) — больше работы, активный maintenance;
(c) зафиксировать как **принятый риск** в `SECURITY.md` (export-only, нет недоверенного парсинга)
+ исключение в `pnpm audit`. Рекомендация релиз-лида: (a), иначе (c).
**Готово.** Уязвимости устранены **или** осознанно приняты записью в `SECURITY.md`; `build.ps1`
зелёный. Объём S.

### MLC-148 · Холодный старт: /sessions пуст + «обновлено ~2000 лет назад» · Backend/Frontend · P2 · Bug

**Симптом (стенд, 2026-06-14).** Сразу после установки/рестарта службы `/sessions` показывает
пустой список и «Обновлено `<огромное число>` секунд назад», хотя сеанс 1С есть (виден на
`/performance`). Самоисцеляется после первого прогона джобы (до ~60 с).
**Первопричина (одна на оба симптома).** `ActiveSessionSnapshotStore` инициализируется
`([], DateTime.MinValue, 0, "None")`
(`backend/src/MitLicenseCenter.Infrastructure/Jobs/ActiveSessionSnapshotStore.cs:8`); наполняющая
Hangfire-джоба `cold-snapshot` (CRON `* * * * *`,
`backend/src/MitLicenseCenter.Web/Program.cs:360-363`) на свежем старте впервые срабатывает с
задержкой. `/sessions` читает кэш-снапшот (`.../Endpoints/Sessions/SessionsEndpoints.cs:27-52`),
`/performance` запрашивает кластер **напрямую** (`.../Endpoints/.../PerformanceEndpoints.cs:61`) —
отсюда расхождение. `MinValue` рендерится `RelativeTime`
(`frontend/src/components/ui/RelativeTime.tsx:36`,
`frontend/src/features/sessions/SessionsPage.tsx:48-52`) как «~2000 лет назад». **Не регрессия
`MLC-144a`** — пред-существующее (с введения cold-snapshot, коммит `04896d6`).
**Фикс (оба малых).** (1) **Прогрев:** разово enqueue/прогнать `cold-snapshot` при старте, чтобы
снапшот наполнился сразу; (2) **защита отображения:** при `CapturedAtUtc == DateTime.MinValue`
(ещё не захватывали) показывать «ещё не обновлялось»/«нет данных» вместо относительного времени
(на FE или маппить в null на BE). Проверить **holistically** — другие страницы тоже могут
рендерить `MinValue` через единый `RelativeTime`.
**Готово.** На свежем старте `/sessions` либо сразу показывает сеансы, либо честную метку «ещё не
обновлялось» (без «2000 лет назад» и без вводящей в заблуждение пустоты); parity соблюдён;
`build.ps1` зелёный. Объём S.
...
`MLC-001..146` закрыты (кроме отложенных `MLC-025/026/027/145` — см. «Открытые опции»; `MLC-062`/`073`
...
`MLC-036` — ROADMAP «RAS Strategy B»). Трек релиза `0.4.0-beta` (`MLC-140..144`) **завершён** —
индекс в `PROJECT_BACKLOG_ARCHIVE.md`. `MLC-146` — уборка мёртвого кода после MLC-144.
`MLC-147`/`148` — приняты в работу на следующую версию (см. «Запланировано — следующая версия»).
Полные отчёты — в git-истории (коммиты `MLC-NNN:`/PR).

**Следующий свободный номер: `MLC-149`.** Куратор инкрементирует при выдаче; выдавать занятые
