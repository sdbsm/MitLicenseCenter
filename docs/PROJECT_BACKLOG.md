...
**`MLC-161` · Frontend · P2 · Medium · Module: dashboard + release** — сигнал недоступности
RAS на дашборде + финализация релиза `0.5.4-beta`.

- **Сигнал на дашборде:** когда RAS недоступна — показать на дашборде actionable-подсказку
  со ссылкой в Настройки («Настроить»/«Проверить службу RAS»). Источник сигнала — **уже
  существующий дешёвый health-снимок RAS** в сводке дашборда (`RasHealthProbingService`),
  **НЕ** дорогой `GET /ras-service/status` (его дёргать с дашборда нельзя — перебор служб).
  Дашборд = «заметить проблему», Настройки = «поставить диагноз и вылечить» (ADR-47). Ключ
  `dashboard.ras.settingsLink` уже заведён в `DashboardPage` — переиспользовать. Статусы —
  только `StatusBadge`.
- **Финализация релиза:** бамп версии `0.5.4-beta` (`frontend/package.json` + `README.md` —
  единая версия BE/FE); ROADMAP-секция «Релиз 0.5.4-beta» → «выпущен (2026-06-15)» с итоговой
  сводкой 3 задач (как у прошлых релизов; сохранить заметку «живая проверка на стенде —
  за владельцем»).
- **Границы:** НЕ трогать BE и экран Настроек (MLC-159/160 готовы). Архивацию трека и
  схлопывание реестра делает **куратор** (закрывающий PR), не исполнитель.
- **Тесты + i18n:** тест сигнала на дашборде (виден при недоступном RAS, ведёт в Настройки);
  i18n. Канон — **ADR-47**; спека — `.claude/plans/ras-service-autoregister.md` (MLC-161).

> Трек «Авто-регистрация службы RAS» (`MLC-159..161`, релиз `0.5.4-beta`). Порядок:
> `MLC-159` ✅ → `MLC-160` ✅ → **`MLC-161`** (FE: сигнал дашборда + бамп версии).
> Архитектура — ADR-47 + `ROADMAP.md` (релиз `0.5.4-beta`).
...
- `MLC-160` — **Done** (PR #243, `0e2092b`). FE: блок диагностики (4 состояния) + лечение в
  «Подключение к 1С/RAS» (`RasServiceCard`/`RasServiceActionDialog`/`useRasService`), диалоги
  с точной командой `sc …`, Zod `omittable`-схемы + omit-null тест; `/status` ленивый
  (`enabled` по «Проверить») + `staleTime` 60с. namespace `settings.rasService.*`.
- `MLC-161` — **NEXT TASK** (см. выше). FE: сигнал недоступности RAS на дашборде (дешёвый
  health, не `/status`) + бамп версии `0.5.4-beta` + ROADMAP → «выпущен».
