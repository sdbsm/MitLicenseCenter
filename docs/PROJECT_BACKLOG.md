...
- `MLC-084`: 05/06 описывают пункт меню и страницу «Профиль» (`/profile`) и сайдбар из 11 пунктов в группах
  ...
  (вместо «Главная»).
- `MLC-085`: 05/06 описывают дашборд как статичные «6 KPI + статус RAS + топ клиентов» без переходов и
  режим «По клиенту» на `/infobases` — фактически: KPI 5 шт и все кликабельные («Клиенты» → /tenants,
  «Инфобазы» → /infobases, «Активные сеансы» → /sessions, «Использовано/Свободно лицензий» → /reports),
  отдельный ряд «Статус RAS + Здоровье хоста» (CPU/RAM/диск из `GET /performance/host`, опрос 45 с,
  «измеряю» на первой пробе, клик → /performance), имена в топе клиентов — ссылки на `/tenants/:id`;
  grouped-режима на `/infobases` нет (flat + фильтр клиента в URL `?tenantId=`), колонка «Базы» в таблице
  клиентов — ссылка на отфильтрованный список баз.
