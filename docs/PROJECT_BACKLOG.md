...
**Трек «Релиз `0.6.0-beta`» — поставлен (постановочный PR куратора).** Три задачи; **активная — `MLC-166`**
(ядро трека, несёт ADR-48 — модель лицензий меняется, остальные две на неё опираются по смыслу, но
технически независимы). Полные постановки — в секции трека ниже. Версию бампнуть `0.6.0-beta` в
закрывающей задаче трека.

**`MLC-166` (NEXT) — `ConsumesLicense` = факт `rac --licenses` (единственный источник).** BE, ядро.
Заменить эвристику white-list `app-id` фактом из `rac session list --licenses` (см. **ADR-48**):
холодный тир делает второй вызов `--licenses` и сшивает с обычным списком по `session`-GUID
(`ConsumesLicense` = членство в выводе `--licenses`; на стенде подтверждено — нелицензионный
`BackgroundJob` в `--licenses` отсутствует). Удалить `LicenseConsumingAppIds` + настройку
`OneC.LicenseConsumingAppIds` (и её FE-рендер) из механизма подсчёта; `app-id` остаётся атрибутом
сеанса. Инвариант «grace ≥ `ColdIntervalSeconds`» (guard) → каждое завершение на факте; свежий сеанс
(< первого факта) → статус «определяется…», в лимит не входит, grace держит; сбой `--licenses` →
enforcement на паузе + «данные о лицензиях недоступны», без отката на `app-id`. **Тесты обязательны
на SQLite** (concurrency/enforcement-путь; InMemory маскирует — урок MLC-163). Канон 03/04 (+05/06 по
FE-следам удалённой настройки) — present-tense в том же PR. Parity BE↔FE по контракту сеансов.

---

## Активный трек — Релиз `0.6.0-beta`

Тема — чистота того, что видит и на чём действует оператор: достоверность лицензий + читаемость
аудита и сеансов. Спайк rac проведён на стенде (вывод `--licenses`, правило сшивки) — выводы в
**ADR-48** и `ROADMAP.md`. Задачи технически независимы; порядок — `166 → 164 → 165` либо параллельно
в worktree. Бамп версии — в последней закрытой задаче.

- **`MLC-166`** (NEXT, BE, ядро) — `ConsumesLicense` = факт `rac --licenses`. Полная постановка — в
  «NEXT TASK» выше; контракт — **ADR-48**.
- **`MLC-164`** (BE, S) — **аудит: убрать ложную запись «публикация создана» при добавлении базы.**
  При `POST /infobases` создаётся служебная запись публикации (метаданные), но webinst не запускается
  и на IIS ничего не публикуется (`LastCheckStatus=Unknown`) — а аудит пишет `PublicationCreated=20`
  «Публикация … создана», что читается как состоявшаяся публикация. Реальное событие уже логируется
  отдельно `PublicationPublished` (webinst). Убрать второй `AuditAsync(PublicationCreated)` из
  `InfobasesEndpoints.CreateAsync` (остаётся только `InfobaseCreated=10`). Enum **не трогать**
  (значение 20 заморожено, исторические записи валидны) — только перестаём писать в этом флоу.
  Поправить затронутые тесты (`AuditFilterTests` сидирует 20 — оставить как исторический сид, не
  через create-флоу). Канон 04 (аудит-таблица) — present-tense.
- **`MLC-165`** (FE, S) — **«Сеансы»: фильтр по типам сеансов.** Мультивыбор `app-id` с человеческими
  именами (`1CV8C`→«Тонкий клиент», `BackgroundJob`→«Фоновое задание» и т.д.), по умолчанию выбраны
  интерактивные клиенты (фоновые скрыты, включаемы). Клиентский фильтр в том же паттерне, что
  поиск/инфобаза (`useSessionsPage`); `app-id` уже в контракте и колонке. Backend не трогаем. Канон
  05/06. Примечание: «лицензия/не лицензия» — отдельный признак (факт rac, `MLC-166`), фильтр здесь
  по типу отображения, не по лицензии.
...
- **Трек «Свежесть данных на страницах»** — `MLC-154..156` 3/3 (2026-06-15): устранение задержки
  ...
  `.claude/plans/parsed-mixing-puppy.md`; индекс — `PROJECT_BACKLOG_ARCHIVE.md`.
- **Трек «Авто-регистрация службы RAS»** — `MLC-159..161` 3/3 (2026-06-15, релиз `0.5.4-beta`,
  **ADR-47**): панель сама управляет локальной службой RAS через `sc.exe` — обнаружение по
  ...
  health, не `/status`). Version-agnostic (8.3/8.5). Спека — `.claude/plans/ras-service-autoregister.md`;
...
`MLC-001..163` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
(`MLC-163` — фикс «Проверить публикацию», concurrency-токен в джобе статусов) **завершены** —
индексы в `PROJECT_BACKLOG_ARCHIVE.md`.
`MLC-146` — уборка мёртвого кода после MLC-144. Полные отчёты — в git-истории (коммиты `MLC-NNN:`/PR).
**Активный трек — «Релиз `0.6.0-beta`» (`MLC-164..166`), см. секцию выше.**

**Следующий свободный номер: `MLC-167`.** (`MLC-164/165/166` выданы треку `0.6.0-beta`.)
