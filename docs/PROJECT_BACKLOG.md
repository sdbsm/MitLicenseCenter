...
`PROJECT_BACKLOG_ARCHIVE.md`. **Остаётся за владельцем:** живая проверка на стенде после апгрейда на
`0.6.1` — на «Сеансах» по умолчанию виден список потребляющих лицензию (факт 1С), завершение лишних —
по реально лицензионным. Следующую задачу/трек ставит куратор постановочным PR. Кандидаты — «Открытые
опции» ниже и `ROADMAP.md`.
...
`MLC-001..167` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
`0.6.0-beta` (`MLC-164..166`, ADR-48 — лицензии = факт `rac --licenses`) и follow-up `0.6.1-beta`
(`MLC-167` — «Сеансы» по умолчанию показывают лицензионные) **завершены** —
...
**Следующий свободный номер: `MLC-168`.** (`MLC-167` выдан follow-up'у `0.6.1-beta`.)
