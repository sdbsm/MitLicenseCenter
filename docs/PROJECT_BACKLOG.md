...
- **`MLC-138` — R12e: доступность WCAG AA и destructive-стиль (UX-кластер E) ✅** (2026-06-14).
  Frontend-only (стили/aria), audit-first с замерами контраста (oklch→sRGB→WCAG luminance). UX-28:
  `StatusBadge` варианты подняты до AA — warning amber-700→amber-800 (4.48→6.32:1 light), neutral
  muted-fg→zinc-700/zinc-400 (4.34→7.09:1 light); токены index.css не тронуты. UX-27: фантом
  (primary в тёмной теме 15.72:1, проходит AA). UX-26: `StopRecordingDialog`/`IisConfirmDialog` →
  `variant="destructive"` (прочие деструктив-диалоги уже несли `bg-destructive`). UX-40: русские
  aria-label/sr-only (pagination, sidebar, dialog Close→Закрыть, PaginationBar через i18n) + тест
  `a11y-aria-labels`. Канон 06 синхронен (таблица контраста ДО/ПОСЛЕ). `build.ps1` зелёный (857 BE +
  521 FE). **Гоча:** контраст замеряется инструментально (в коде нет авто-теста контраста); новые
  бейджи/токены сверять с таблицей 06.

## NEXT TASK

> **`MLC-139` — R12f: онбординг и точки трения (UX-кластер F). ПОСЛЕДНИЙ кластер R12.**
> Frontend-only · P3 · Small-Medium. **Audit-first** (урок: часть находок бывает фантом/уже-закрыта).
> Находки UX-25/36/39/41 (UX-46 уже закрыт в MLC-122):
> - **UX-25:** бэкап базы запускается только из строки таблицы баз — нет очевидной точки входа в разделе
>   бэкапов/на карточке. Проверь фактический флоу `BackupsDialog`/раздел бэкапов; если запуск только из
>   строки — добавь доступную точку (кнопка в диалоге списка бэкапов базы). Если уже есть — фантом.
> - **UX-36:** срок хранения журнала аудита не виден на `/audit` (retention есть в настройках/`useAuditRetention`,
>   но оператор не видит его на странице журнала) — показать срок хранения на `/audit` (подпись/тултип).
> - **UX-39:** в форме смены пароля (`ChangePasswordForm` в диалоге топбара) нет кнопки «Отмена» —
>   добавить «Отмена», закрывающую диалог (на блокирующем экране форс-смены — НЕ добавлять). Учесть, что
>   `showReset` уже есть; «Отмена» — отдельная кнопка только для диалогового режима.
> - **UX-41:** разрозненные мелкие полировки точек трения — взять только подтверждённые аудитом/очевидные;
>   не раздувать.
> Объём — UI/тексты/мелкая разводка; логику/контракты/parity/миграции НЕ трогать. Тесты затронутого
> поведения обновить/добавить. Канон 05/06 — затронутые экраны.
> Критерии готовности: перечень находок (реальные/фантом) в отчёте; подтверждённые точки трения закрыты;
> `build.ps1` зелёный; канон синхронен. Исполнитель — субагент (worktree); модель — Sonnet.
> **После MLC-139 R12 закрыт полностью → весь пострелизный трек R1–R12 завершён.**
> _Источник — `ROADMAP.md` R12; первоисточник — `TECH-DEBT.md` UX-кластер F._
...
`MLC-001..138` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-139` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-140`.** Куратор инкрементирует при выдаче; выдавать занятые
