...
> **`MLC-105` — Установщик: распознавать уже инициализированную БД.** Follow-up к закрытому треку
> «GUI-установщик» (ADR-31). Баг (воспроизведён на стенде 2026-06-11): «чистая установка» ≠ «пустая
> БД» — на непустой БД сидер игнорирует заданный в мастере пароль admin, мастер этого не сообщает.
> Фикс: мастер пробит наличие пользователей `auth.Users` → предупреждает + пропускает страницу
> пароля (как на апгрейде). Только установщик, backend не трогаем. Канон: правка **ADR-31** +
> `OPERATIONS`. План — `.claude/plans/mlc-105-installer-existing-db.md`. Status: **Approved (NEXT)**.
...
`MLC-001..104` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
...
**Следующий свободный номер: `MLC-106`.** Куратор инкрементирует при выдаче; выдавать занятые
