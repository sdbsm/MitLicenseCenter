...
> _Не назначена — активных задач нет._ Трек «Нераспределённые базы: discovery-first
> добавление» (`MLC-092..094`) **закрыт 2026-06-11** — секция перенесена в архив, строка
> в «Завершённых треках» выше. Дрейф панель↔кластер: направление (а) закрыто этим треком,
> направление (б) — обратный дрейф (база жива в панели, удалена из кластера) — остаётся
> **обязательным** кандидатом следующего трека (`ROADMAP.md` «Дрейф панель↔кластер»).
...
`MLC-001..094` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
