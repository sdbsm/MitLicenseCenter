...
> **`MLC-102` — Пароль admin задаёт оператор в мастере (вместо случайного).** Трек «GUI-установщик»,
> задача 5/6. Решение оператора: мастер спрашивает пароль admin (с подтверждением + политика), сидер
> берёт его при первом старте через одноразовый файл `%ProgramData%\MitLicenseCenter\initial-admin.secret`
> (использует и удаляет); random+EventId 1001 — fallback. Затрагивает **backend (сидер)** + установщик.
> Канон: правка **ADR-31** + `OPERATIONS`. План — `.claude/plans/mlc-102-admin-password.md`.
...
- **`MLC-102`** · Installer + Backend · P2 · Files: `IdentitySeeder.cs`, `IdentitySeederTests.cs`,
  `installer/MitLicenseCenter.iss`, канон ADR-31/`OPERATIONS`. Мастер спрашивает пароль admin
  (подтверждение + политика ≥12/4 класса; страница только на чистой установке), кладёт в
  `%ProgramData%\MitLicenseCenter\initial-admin.secret`; сидер при первом старте использует его и
  **удаляет** файл (cleanup при наличии-юзеров тоже). Random+EventId 1001 — fallback для dev/db-reset/
  ручного деплоя. Финал мастера: вход под заданным паролем + URL. План — `mlc-102-admin-password.md`. Status: **Approved (NEXT)**.
