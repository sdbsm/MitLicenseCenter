...
> **`MLC-134` — R12a: сброс диалогов при повторном открытии + инвалидация счётчиков (FE-07/08/02).**
> Frontend-only · P3 · Small. Первый кластер R12 (полировка UX, многокластерная). Три точечных FE-дефекта
> из аудита (A3), все с известными местами — разведка куратора 2026-06-14:
> - **FE-07:** `UserFormDialog` открывается повторно с «призраком» прошлого ввода — нет `key`-пропа
>   (паттерн `key={editing?.id ?? "create"}` уже применён у `TenantFormDialog`, см. `TenantsPage.tsx`).
>   Добавить `key` в месте использования — `frontend/src/features/users/UsersPage.tsx` (диалог
>   создания/редактирования пользователя).
> - **FE-08:** диалог смены пароля в топбаре (`ChangePasswordForm`) не сбрасывается при повторном
>   открытии — заполненные password-поля прошлой попытки видны снова. Сбрасывать при открытии
>   (`key={passwordDialogOpen}` на `Dialog` в `frontend/src/components/layout/Topbar.tsx`, либо
>   `useEffect(open → form.reset())` в форме). Выбрать единообразный с FE-07 приём (`key`).
> - **FE-02:** `useCreateInfobase()` и `useDeleteInfobase()` (`frontend/src/features/infobases/useInfobases.ts`)
>   инвалидируют только `infobasesQueryKey` — счётчик баз клиента в таблице `/tenants` остаётся устаревшим
>   до ручного refetch. Добавить `tenantsQueryKey` в `invalidate` обоих (как уже сделано у
>   `useReassignInfobase`: `[infobasesQueryKey, tenantsQueryKey]`).
> **Уже закрыто (не трогать, отметить):** FE-03 — `useUpdateTenant` инвалидирует `reportsQueryKey`
> (закрыт в MLC-122). Образец сброса диалога — `TenantFormDialog`/`TenantsPage.tsx`; образец множественной
> инвалидации — `useReassignInfobase`.
> Критерии готовности: оба диалога сбрасываются при повторном открытии (поведенческий тест:
> ввод → закрыть → открыть → пусто); create/delete инфобазы инвалидируют `tenantsQueryKey`; обновить/
> добавить затронутые тесты; `build.ps1` зелёный; канон 05 обновлён (паттерн сброса диалога + матрица
> инвалидаций). Исполнитель — субагент (worktree); модель — Sonnet (Small, frontend-only, точечно).
> _Состав R12 далее: R12b BE-13 (freeze-тесты Perf-enum), R12c BE-02 (optimistic concurrency,
> аддитивный rowversion + миграция), R12d/e/f UX-кластеры D/термины · E/доступность · F/онбординг.
> Источник — `ROADMAP.md` R12; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..133` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-134` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-135`.** Куратор инкрементирует при выдаче; выдавать занятые
