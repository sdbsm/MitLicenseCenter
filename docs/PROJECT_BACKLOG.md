...
> **`MLC-136` — R12c: optimistic concurrency на `Tenant` (BE-02).**
> Backend + миграция + FE/parity · P3 · Medium. Третий кластер R12. Сейчас два одновременных
> редактирования клиента дают lost update (особенно `MaxConcurrentLicenses` — enforcement начинает
> работать по «победившему» значению). Закрываем через rowversion-токен на `Tenant`.
> **МИГРАЦИЯ — СТРОГО АДДИТИВНА (критично, БД существует у владельца):** один новый столбец
> `RowVersion rowversion` на таблицу `Tenants`. SQL Server заполняет rowversion существующих строк
> автоматически при `ADD COLUMN` — **без backfill, без UPDATE/DELETE данных, без смены типов**. Миграция
> применяется на старте (`MigrateAsync`, штатно). **Проверить сгенерированную миграцию глазами:** только
> `AddColumn<byte[]>(..., rowVersion: true)` в Up, `DropColumn` в Down; никаких data-операций.
> **Состав:**
> - Сущность `Tenant` (`backend/src/MitLicenseCenter.Domain/Tenants/Tenant.cs`): `byte[]? RowVersion`;
>   конфиг в `AppDbContext.OnModelCreating` — `.Property(t => t.RowVersion).IsRowVersion()`.
> - DTO: read-ответ клиента несёт `rowVersion` (base64); запрос обновления (`UpdateTenantRequest`)
>   принимает ожидаемый `rowVersion`. Endpoint `TenantsEndpoints.UpdateAsync` — выставить
>   `entry.OriginalValues["RowVersion"]` из запроса, ловить `DbUpdateConcurrencyException` → **409**
>   с понятным кодом (напр. `TENANT_CONCURRENCY_CONFLICT`) и сообщением «данные изменены другим
>   пользователем, обновите страницу».
> - **Parity FE:** Zod-схема клиента (`features/tenants/types.ts`) добавляет `rowVersion`; форма
>   (`TenantFormDialog`) шлёт прочитанный `rowVersion` в update; обработка 409 — тост/перезагрузка
>   (паттерн `matchConflictCode`/`applyFieldErrors` из R5). Golden parity-тесты обновить.
> - Тесты: BE — happy update проносит новый rowVersion; конкурентный update (устаревший rowVersion) →
>   409. FE — схема принимает rowVersion (+ omit-null дисциплина, если применимо).
> **Гочи:** только `Tenant` (named-риск — лимиты); Infobase/Publication — отдельным follow-up по триггеру
> (не тащить в этот PR). `rowversion` несравним/не редактируется вручную; на проводе — base64-строка
> (System.Text.Json сериализует `byte[]` в base64). EF InMemory НЕ поддерживает rowversion-генерацию —
> concurrency-тесты писать так, чтобы InMemory их не маскировал (или помечать SQL-only; см. как устроены
> существ, тесты, что бьют по реальному провайдеру). Канон 03 (инвариант concurrency) + 04 (паттерн 409) +
> 05 (parity).
> Критерии готовности: миграция аддитивна (вычитана); конкурентный апдейт `Tenant` → 409; build.ps1
> зелёный; parity-тесты зелёные; канон обновлён. Исполнитель — субагент (worktree); модель — выбирает
> куратор (высокие ставки миграции → сильная модель + тщательное ревью).
> _Состав R12 далее: R12d/e/f UX-кластеры D/термины · E/доступность · F/онбординг. Источник —
> `ROADMAP.md` R12; первоисточник — `TECH-DEBT.md` BE-02._
...
`MLC-001..135` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-136` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-137`.** Куратор инкрементирует при выдаче; выдавать занятые
