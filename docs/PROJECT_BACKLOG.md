...
> **`MLC-129` — R11a: Аудит — поиск, фильтр по инициатору, переход на страницу N (UX-20/35/37/38).**
...
> (`AuditEndpoints.ListAsync`, `AuditFiltersBar`). Добавить недостающее:
> - **UX-20:** текстовый поиск (free-text) — новый query-параметр `search` в `AuditEndpoints.ListAsync`,
>   ищет по `Description` и/или `Initiator` (case-insensitive, `Contains`); поле ввода в `AuditFiltersBar`
>   (debounce, сброс page→1). Длину терма ограничить (валидация, как уже принято в R2).
> - **UX-35:** фильтр по **инициатору** — аудит-запись несёт поле `Initiator` (string,
>   `AuditContracts.cs`). Параметр `initiator` (точное/contains-совпадение по имени) + контрол в
>   `AuditFiltersBar` (свободный ввод или select из известных инициаторов — на усмотрение исполнителя;
>   проще — текст). Не путать с `tenantId` (это клиент, не инициатор).
> - **UX-37/38:** переход на страницу N — пагинация есть, но UI, вероятно, prev/next (`AuditPagination.tsx`).
>   Добавить ввод/переход на произвольную страницу N (с валидацией диапазона 1..totalPages).
> Parity BE↔FE: `AuditFilters`-тип/Zod + query-сериализация обновить синхронно; **API опускает null-поля**
> — nullable-поля фильтра на FE как `.nullish()` (урок MLC-067/071, память `api-omits-null-fields`).
> Замороженный enum `AuditActionType` НЕ трогать (только читается для фильтра). Канон 05 §3.5 (фильтры
> аудита) + 04 (если меняется контракт эндпоинта) — в том же PR.
> Критерии готовности: поиск/фильтр-инициатор/переход-на-N работают сервер-сайд; parity-тесты зелёные;
> канон обновлён; `build.ps1` зелёный. Исполнитель — субагент (worktree); модель выбирает куратор.
>
> **План кластеров R11** (порядок — по изоляции; номера выдаются по мере хода):
> - **R11a `MLC-129`** — аудит: поиск + фильтр-инициатор + переход-на-N (UX-20/35/37/38). ← _текущий_
