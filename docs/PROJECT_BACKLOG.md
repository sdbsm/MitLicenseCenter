...
> _Не назначена — активных задач нет._ Трек «UX-пересборка, этап 2: single-host бек-чистка»
> (`MLC-087..091`) **закрыт 2026-06-10** — секция перенесена в архив, строка в «Завершённых
> треках» выше. Single-host зафиксирован **ADR-28**; весь UX-цикл (этапы 1+2) завершён.
> Теги отката: `v1-pre-ux-redesign` (до этапа 1), `v1-pre-singlehost-stage2` (до миграций
> этапа 2; к ним нужен бэкап БД из `F:\MlcStage2Backups`).
...
`MLC-001..091` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
