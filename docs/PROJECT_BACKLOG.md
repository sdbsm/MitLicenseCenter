...
3. `MLC-077` · Backend · P2 — Оркестрация + API: `BackupOrchestrator` + `BackupPumpService` + recovery;
   `BackupRetentionJob` + recurring-регистрация; эндпоинты `/api/v1/backups` (list/detail/start=Viewer,
   delete=Admin) + `Problems.BackupActive`/`BackupFolderNotConfigured`; DI; правка `LayerBoundaryTests`
   (`Infrastructure.Backups` в forbidden-list); тесты оркестратора/джобы/эндпоинтов.
4. `MLC-078` · Frontend · P2 — Frontend + e2e: кнопка «Сделать бэкап» на карточке инфобазы (Viewer) + диалог
   списка/статуса с поллингом 5с + тост 409, Admin-удаление; секция настроек бэкапа на `/settings`; i18n;
   present-tense `04`/`05`/`OPERATIONS`. **Live-verify на стенде обязателен** (урок [[api-omits-null-fields]]).

---

## NEXT TASK

> **Нет активной задачи.** `MLC-076` закрыт (см. индекс «Закрыто»). Следующую `NEXT TASK` ставит внешний
> чат-куратор (исполнитель её сам не выставляет); очередная задача трека «Резервное копирование баз SQL» —
> `MLC-077` (оркестрация + API; спека — `.claude/plans/lazy-cuddling-finch.md`). По ходу MLC-076 найден
> кандидат `[Doc divergence]` (Recording-ключи MLC-070 отсутствуют в каталоге настроек 04 §4) —
> см. «Кандидаты от исполнителя» ниже.
>
...
## Кандидаты от исполнителя (ждут постановки куратором)

- `[Doc divergence]` (найден на MLC-076, 2026-06-09) — каталог настроек `docs/04_INFRASTRUCTURE.md` §4
  не содержит трёх ключей `Performance.RecordingSampleIntervalSeconds` / `…RecordingMaxDurationMinutes` /
  `…RecordingMaxSamples`, заведённых в `SettingKey`/`SettingDefinitions` в MLC-070 (сами настройки в
  каноне описаны — 04 §6.3/ADR-26, но таблица-каталог «единый список всех ключей» не пополнена; остальные
  17+ ключей в ней есть). Варианты: (а) дополнить таблицу тремя строками — механическая present-tense
  правка, рекомендую; (б) объявить таблицу каталогом только операторски значимых ключей и зафиксировать
  критерий в §4 (хуже: ProcessFamilyMap и прочие «тонкие» ключи в таблице уже есть). Не чинил молча —
  вне объёма MLC-076.
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 054, 055, 056, 057, 058, 059, 060, 061, 063, 064, 064a, 065, 066, 067, 068, 069, 070, 071, 072, 074, 075, 076) — индекс
