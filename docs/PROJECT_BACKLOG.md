...
**Нет активной задачи.** `MLC-170` (служба под virtual service account, ADR-49) и `MLC-171` (выбор
провижининг-кред: Integrated Security / SQL-sysadmin-логин) **реализованы** (ISCC зелёный). **Остаётся
за владельцем** — живая проверка на стенде: чистая установка под virtual account (служба под
`NT SERVICE\MitLicenseCenter`, в конфиге нет SQL-пароля, в SQL есть логин с `sysadmin`, health `ready`,
секреты 1С расшифровываются, нет 18456); вариант провижининга «SQL-логин `sa`» при админе-не-sysadmin;
негатив (неверный `sa` / админ не sysadmin → внятный отказ с откатом); апгрейд; ручная миграция B→A по
`OPERATIONS.md §8`. **`MLC-172`** — фиксы по стенду: (а) баг провижининга `sp_executesql` (0.6.5);
(б) поля SQL-логина не отображались — страница провижининга переведена с radio+кастомные контролы на
родную `InputQueryPage` (режим из поля логина: пусто=Integrated), + упрощение текста мастера (0.6.6).
Актуальный релиз — **`0.6.6-beta`** (`0.6.4`/`0.6.5` отозваны). Следующую задачу/трек ставит куратор.
...
**Следующий свободный номер: `MLC-173`.** (`MLC-172` выдан фиксу провижининга/текста мастера, релиз `0.6.5-beta`.)
