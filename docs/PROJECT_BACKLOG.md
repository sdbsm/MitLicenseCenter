...
> **`MLC-129` — R11a: Аудит — масштабируемые фильтры и навигация (UX-20/35/37/38).**
> Frontend+Backend · P3 · Medium. Первый кластер R11 (крупная задача разбита куратором на 4 кластера —
> см. план ниже). Аудит уже имеет серверную пагинацию (page/pageSize) + фильтры actionType/tenantId/from/to
> (`AuditEndpoints.ListAsync`, `AuditFiltersBar`, `AuditPagination` с окном `pageLinkRange(maxLinks=7)`).
> Добавить недостающее (точный маппинг находок A4):
> - **UX-20:** текстовый поиск (free-text) **+ фильтр по инициатору**. BE: новый query-параметр `search`
>   (case-insensitive `Contains` по `Description` И `Initiator`) и параметр `initiator` (точное совпадение
>   по `AuditLog.Initiator`, напр. `System`/имя admin); FE: поле поиска (debounce) в `AuditFiltersBar`,
>   контрол инициатора. Длину терма ограничить (валидация, как в R2). Не путать `initiator` (кто сделал) с
>   `tenantId` (над кем).
> - **UX-35:** при 2013 страницах нет перехода на произвольную/последнюю — `pageLinkRange` показывает окно
>   из 7. Добавить в `AuditPagination` переход на страницу N (ввод номера с валидацией 1..totalPages) и/или
>   кнопки «первая»/«последняя».
> - **UX-37:** плоский Select из 35 типов действий без поиска — сделать выпадающий список действий
>   **с поиском** (combobox-паттерн поверх shadcn, фильтр по локализованному названию).
> - **UX-38:** нет поиска в выпадающем списке клиентов фильтра аудита — тот же combobox-паттерн для tenant.
>   Сделать переиспользуемый searchable-Select (sessions-фильтр получит его в R11c — UX-38 там же).
> Parity BE↔FE: `AuditFilters`-тип/Zod + query-сериализация (`useAuditLog.buildQuery`) обновить синхронно;
> **API опускает null-поля** — nullable-поля на FE как `.nullish()` (урок MLC-067/071, память
> `api-omits-null-fields`). Замороженный enum `AuditActionType` НЕ трогать (только читается для фильтра).
> Канон 05 §3.5 (фильтры аудита) + 04 (контракт эндпоинта, новые query-параметры) — в том же PR.
> Критерии готовности: поиск/инициатор работают сервер-сайд; переход-на-N/последнюю работает; списки
> действий и клиентов с поиском; parity-тесты зелёные; канон обновлён; `build.ps1` зелёный. Исполнитель —
> субагент (worktree); модель выбирает куратор.
>
> **План кластеров R11** (порядок — по изоляции; номера выдаются по мере хода):
> - **R11a `MLC-129`** — аудит: поиск+инициатор (UX-20), переход-на-N (UX-35), searchable списки
>   действий/клиентов (UX-37/38). ← _текущий_
> - **R11b `MLC-130`** — серверная пагинация + поиск по клиенту для `/backups` и `/performance/recordings`
>   (UX-05/BE-17) — эти эндпоинты пока без пагинации.
...
`MLC-001..128` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-129` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-130`.** Куратор инкрементирует при выдаче; выдавать занятые
