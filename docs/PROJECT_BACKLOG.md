...
**`MLC-213` · Backend · P2 · Агрегатор статусов служб узла + управление сервером 1С.** Read-модель
статуса всех служб стека под `GET /api/v1/server/status` (Viewer): сервер(ы) 1С (`ragent` — discovery
по реестру, ImagePath с `ragent.exe`, при нескольких версиях платформы списком: имя/состояние/версия),
RAS (переиспользовать `IRasServiceManager.DiagnoseAsync`), SQL Server (**только наблюдение** — статус
службы `sqlservr` + имя инстанса через `ISqlInstanceDiscovery`; never-throws, деградация `Available:false`;
управление **не давать**, ADR-54 — стоп SQL = самоликвидация панели), сводка IIS (переиспользовать
`IIisLifecycleService.GetServerStateAsync`, деградация `Available:false`) + общий индикатор здоровья узла.
Деградация адаптеров как у `IisEndpoints` (флаги `Available`/`Error`, не 500). **Мутации (Admin) — только
сервер 1С** через `IWindowsServiceController`: `POST /api/v1/server/onec/{start,stop,restart}`, цель — имя
`ragent`-службы из discovery (**whitelist**: имя обязано быть в обнаруженном наборе — Admin не может дёрнуть
произвольную службу / SQL / саму панель); `stop`/`restart` — серверный Confirm-гейт (как `iisreset`);
маппинг `WindowsServiceOperationException → 409` (`SERVER_OPERATION_FAILED`, по образцу
`RAS_SERVICE_OPERATION_FAILED`); аудит 800-серия (`OneCServerStarted/Stopped/Restarted`, объявлена
в MLC-212) через `EndpointHelpers.AuditAsync` (`tenantId:null`). **Управление RAS/IIS не дублируем:** RAS
`register/update/start` остаются в `/ras-service/*` (Параметры, ADR-54), IIS — в `/api/v1/iis/*` (переезд
UI — MLC-215); RAS `stop/restart`, если понадобятся «Службам», — отдельный мелкий аддитив (новые
аудит-коды 603/604) в момент FE-нужды. **Без FE** (его даёт `MLC-214`). **Приёмка:** unit на агрегатор
(композиция адаптеров + деградация + правила общего здоровья) + endpoint-тесты (ролевой доступ Viewer/Admin,
whitelist, маппинг 409, Confirm-гейт, аудит `tenantId:null`/800); локальный `build.ps1` зелёный.
Якоря-образцы — в секции трека ниже.

См. трек «Сервер» ниже — полная цепочка `MLC-212..219` (`MLC-212` закрыт).
...
- **`MLC-213`** (NEXT) · BE · агрегатор статусов всех служб + управление сервером 1С (полная постановка —
  в «NEXT TASK» выше; scope уточнён куратором 2026-06-19: мутации — только сервер 1С, RAS/SQL/IIS в
  агрегаторе только наблюдаются, существующее управление RAS/IIS не дублируется).
...
(`MLC-212..219`, релиз 1.1.3) — постановка выше (ADR-54/55); `MLC-212` закрыт (2026-06-19), NEXT = `MLC-213`.
