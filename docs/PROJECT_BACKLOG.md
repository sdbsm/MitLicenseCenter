...
> **Нет активной задачи.** Трек «Пред-релизные фиксы по итогам аудита» (`MLC-108..115`)
> закрыт 2026-06-13 — все пред-релизные findings аудита А0–С1 устранены (индекс задач —
> `PROJECT_BACKLOG_ARCHIVE.md`; передаточная сводка — строка трека в «Завершённые треки —
> индекс» выше).
>
> **Следующий шаг ведёт куратор вне бэклога** (не `NEXT TASK`): (1) ручной прогон Setup.exe
> на чистой ВМ — установка/апгрейд/удаление вживую (слепая зона аудита) + 2-мин проверка
> UX-01-тоста; (2) этап D1 «Документация v2» (промт `.claude/plans/audit-prerelease-prompts.md`:
> выровнять security-слой и `02_ARCHITECTURE_REQUIREMENTS`, терминологию DPAPI→plaintext в
> ADR-21/CLAUDE.md/памяти, снять пред-релизную оговорку «канон не истина»); (3) перенос roadmap
> R1–R12 из `TECH-DEBT.md` в `ROADMAP.md`.
>
> Новую `NEXT TASK` ставит внешний чат-куратор постановочным PR.
