...
**`MLC-152` · Low-priv аккаунт службы (ADR-44)** — трек «Релиз 0.5.0-beta» (полная постановка в
одноимённом разделе ниже). **Двухфазная, нужна Procmon-трасса на стенде (участие владельца).** Одна
задача за сессию. **Закрыто:** `MLC-149`/`148`/`147`/`027`/`150`/`151` (2026-06-14, PR
#221/#222/#220/#224/#226/#228). Осталась последняя: 152. После неё — бамп версии → `0.5.0-beta`.
...
### MLC-151 · Optimistic concurrency на Infobase/Publication · Backend/Frontend · M · **Done** (2026-06-14, PR #228)

Зеркало паттерна `Tenant` (MLC-136): `RowVersion` + `.IsRowVersion()` + `OriginalValue` →
`DbUpdateConcurrencyException` → 409. **Развилка решена вариантом (b):** отдельный токен и на
Publication — у неё есть самостоятельный `PUT /publications/{id}`; `PUT /infobases/{id}` проверяет
оба токена (`INFOBASE_CONCURRENCY_CONFLICT` / `PUBLICATION_CONCURRENCY_CONFLICT`). Миграция
`20260614193205_MLC151...` (rowversion NULL на обе таблицы). **Гоча (фикс в ревью):** списочная
проекция должна нести оба токена — форма редактирования открывается из элемента списка, иначе FE
шлёт null и защита молча не работает (есть регресс-тест). Parity + omit-null.

### MLC-152 · Low-priv аккаунт службы (ADR-44) · Security/Backend/Installer · P2 · M–L · 2 фазы · ← NEXT TASK
