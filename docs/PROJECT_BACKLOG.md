...
>
> После закрытия: куратор пересобирает Setup.exe, владелец повторяет апгрейд-проверку на ВМ;
> далее вне бэклога — этап D1 «Документация v2» (промт `.claude/plans/audit-prerelease-prompts.md`)
> и перенос roadmap R1–R12 из `TECH-DEBT.md` в `ROADMAP.md`.
...
**Следующий свободный номер: `MLC-117`.** Куратор инкрементирует при выдаче; выдавать занятые
