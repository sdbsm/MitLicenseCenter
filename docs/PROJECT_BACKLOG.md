...
**`MLC-212` · Backend · P2 · Надёжный контроллер службы Windows.** Обобщить RAS-паттерн
(`ScRasServiceManager` / `ScProcessRunner` / `IScProcessRunner`) в универсальный сервис управления
Windows-службой с контрактом надёжности **ADR-55**: старт/стоп/рестарт + **верификация фактического
состояния через `ServiceController` до целевого с таймаутом** (паттерна «опросить до состояния» в коде
сейчас нет — писать), идемпотентность (коды `sc` 1056/1062 = успех), обобщённый замок сериализации (по
образцу `IisResetConcurrencyGate`). Новые значения аудита для управления сервером 1С (свободный диапазон,
см. `AuditActionType.cs`; enum **заморожен** — только append, существующие не переназначать). Чистая
Infrastructure + интерфейс Application, **без эндпоинтов/FE** (их даёт `MLC-213+`). **Приёмка:** unit-тесты
на верификацию/таймаут/идемпотентность; локальный `build.ps1` зелёный. Якоря-образцы — в секции трека ниже.

См. трек «Сервер» ниже — полная цепочка `MLC-212..219`.
...
**Задачи:**
- **`MLC-212`** (NEXT) · BE · надёжный контроллер службы Windows (полная постановка — в «NEXT TASK» выше).
- **`MLC-213`** · BE · агрегатор статусов всех служб (discovery нескольких версий 1С, RAS, статус SQL,
  сводка IIS) + общий индикатор здоровья + read/мутирующие эндпоинты (server-scope; Admin — управление,
  Viewer — чтение).
...
(`MLC-212..219`, релиз 1.1.3) — постановка выше (ADR-54/55).
