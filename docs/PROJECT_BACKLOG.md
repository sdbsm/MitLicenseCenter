...
> **Активных задач в бэклоге нет.** Трек пред-релизных фиксов и его follow-up'ы ВМ-проверки
> (`MLC-108..117`) закрыты. Дальнейшие шаги ведёт куратор **вне бэклога**:
> 1. Куратор пересобирает `Setup.exe` (с фиксом `MLC-117`); владелец повторяет на ВМ
>    апгрейд-проверку (служба останавливается, `MLC-116`) и публикацию/снятие публикации
>    сразу после установки без пересохранения настройки (`MLC-117`).
> 2. Этап **D1 «Документация v2»** (промт `.claude/plans/audit-prerelease-prompts.md`):
>    выровнять security-слой и `02_ARCHITECTURE_REQUIREMENTS` с кодом/отчётами аудита,
>    терминология DPAPI→plaintext в ADR-21/CLAUDE.md.
> 3. Перенос roadmap R1–R12 из `audit/2026-06/TECH-DEBT.md` в `docs/ROADMAP.md`.
>
> Новую `NEXT TASK` ставит внешний чат-куратор при появлении дефекта/спеки —
> исполнитель сам её не выставляет.
