...
2. `MLC-076` · Backend · P2 — Фундамент: settings-ключи (`Backup.FolderPath`/`TtlHours`/`MaxParallel`/
   `DiskSafetyMarginMb`) + definitions; сущность `DatabaseBackup` (`Infrastructure.Reporting`) + миграция;
   `ISqlBackupService` + `SqlBackupAdapter` (SQL-механика: проверка→оценка→место→mkdir→BACKUP+VERIFY→delete-older);
   слоты аудита `AuditActionType` 510–514; `FakeSqlBackupService`.
...
> **`MLC-076`** — Backend, фундамент трека «Резервное копирование баз SQL» (см. трек выше): settings-ключи
> (`Backup.FolderPath`/`TtlHours`/`MaxParallel`/`DiskSafetyMarginMb`) + definitions; сущность `DatabaseBackup`
> (`Infrastructure.Reporting`) + миграция; `ISqlBackupService` + `SqlBackupAdapter` (SQL-механика); слоты
> аудита `AuditActionType` 510–514; `FakeSqlBackupService`. `MLC-075` закрыт (ADR-15 изменён + ADR-27 заведён).
> Полная спека — `.claude/plans/lazy-cuddling-finch.md`. Дальше `MLC-077..078` (по одной за сессию).
>
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 054, 055, 056, 057, 058, 059, 060, 061, 063, 064, 064a, 065, 066, 067, 068, 069, 070, 071, 072, 074, 075) — индекс
