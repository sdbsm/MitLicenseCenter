...
**`MLC-229` · Снять оставшиеся ⚠ на стенде 8.5 (needs stand — за владельцем).**
Активен трек **версии 1.2** — `release/1.2` (ветка; `main` заморожен на теге `v1.1.4` = точка отката).
Это **стенд-задача**: требует действий владельца на живой платформе 8.5, субагент её выполнить не может.
Снять три открытых ⚠ из базы знаний и зафиксировать факты в каноне трека (`40_TECHLOG.md`/`60_SAFETY.md`):
1. **Единицы `duration` в JSON** — на заведомо медленной операции (мс vs мкс), нормализация в модели парсера.
2. **Имя/регистр поля длительности в logcfg-фильтре** `<ge property="…">` (`Dur` / `duration` / `Duration`).
3. **Грант ACL** сервисному аккаунту `NT SERVICE\MitLicenseCenter` на `conf`/`logcfg.xml` (иначе панель сама
   `logcfg.xml` не запишет — выбор пути из ADR-58: предпочтительно грант `Modify` установщиком).

Готово: все `⚠ verify` сняты или подтверждены, факты внесены в `40_TECHLOG.md`/`60_SAFETY.md`. Эти три
факта — вход для **MLC-230** (сервис logcfg): без них этап A строится на непроверенных допущениях
(зависимость `MLC-229 → A` из `80_TRACK_PLAN.md`). Куратор готовит скелет записи фактов; стендовый прогон —
за владельцем.

**Полный план трека и операционная модель — `research/perf-investigation/80_TRACK_PLAN.md`** (читать
первым; вся проектная основа — `research/perf-investigation/`, с `00_INDEX.md`). PR'ы трека целить в
**`release/1.2`**, не в `main`.
...
- **0 Канонизация/стенд:** `MLC-228` ✅ (ADR+канон), `MLC-229` (снять ⚠ на стенде 8.5, ← `NEXT TASK`).
...
- **A logcfg lifecycle:** `MLC-230` (сервис logcfg + сторож), `MLC-231` (безопасный сбор).
...
- **D Frontend:** `MLC-240..243` (Наблюдение / Мастер / Дело+список / Отчёт — по `70_UI_SPEC`).
...
**закрыт и выпущен в `v1.1.4`** (2026-06-19). **Активен трек «Расследование производительности» (1.2,
`release/1.2`)** — см. секцию трека выше; `MLC-228` закрыт (PR #427), `NEXT TASK = MLC-229`.

**Следующий свободный номер: `MLC-230`.** (`MLC-228` — канонизация трека «Расследование
производительности», **закрыт** в `release/1.2`, PR #427. `MLC-229` выдан как `NEXT TASK` — снятие ⚠ на
стенде 8.5. Номера `MLC-230..243` **зарезервированы активным треком** — выдавать по одной согласно
`research/perf-investigation/80_TRACK_PLAN.md`. `MLC-227` — отзыв SCM-зависимости службы от SQL, закрыт
в `v1.1.4`; `MLC-222..226` — хотфиксы трека «Сервер».)
