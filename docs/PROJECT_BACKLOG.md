...
**`MLC-216` · BE+FE · P2 · Вкладка «Обслуживание»: свежесть бэкапов SQL + алерт «устарел».**
Новая вкладка «Обслуживание» экрана «Сервер» (третья после «Службы»/«IIS») — **только чтение**:
свежесть резервных копий баз из `msdb.dbo.backupset` (**live-read, без новых таблиц/миграций/джобов**).
BE: новый read-эндпоинт (server-scope, Viewer) — по каждой базе последний бэкап (тип full/diff/log +
время), плюс флаг **«устарел»** (порог свежести; кураторское решение по дефолту — фиксированная
константа ~26 ч для full, документировать; отдельную настройку НЕ заводить без явной нужды). Источник —
переиспользовать `SqlBackupAdapter` (уже читает `msdb.dbo.backupset`) или соседнюю пробу; паттерн
**never-throws + `HAS_PERMS_BY_NAME`** (как `DatabaseSizeProbe`/`SqlPerformanceProbe`) — деградация
`Available:false`/`Error`, не 500. Учесть: «Резервные копии» (on-demand `COPY_ONLY`, ADR-27) **не
трогать** — это другое («сделать бэкап руками» vs «следить, что штатные идут»). FE: вкладка
«Обслуживание» с таблицей свежести + метка `StatusBadge` «устарел/свежо»; Zod-граница `/api/v1/server/*`
или новая группа (на усмотрение, omit-null). Алерт на «Обзоре» — НЕ в этой задаче (его даёт MLC-217 для
планов обслуживания). **Приёмка:** unit на расчёт свежести/«устарел» + деградацию; FE компонент/parity;
локальный `build.ps1` зелёный. Якоря-образцы — в секции трека ниже.

См. трек «Сервер» ниже — полная цепочка `MLC-212..219` (`MLC-212`..`MLC-215` закрыты).
...
- **`MLC-216`** (NEXT) · BE+FE · вкладка «Обслуживание»: свежесть бэкапов + алерт «устарел» (полная
  постановка — в «NEXT TASK» выше).
- **`MLC-216`** · BE+FE · вкладка «Обслуживание»: свежесть бэкапов (`msdb.dbo.backupset`, live-read,
  без новых таблиц) + алерт «устарел».
- **`MLC-217`** · BE+FE · планы обслуживания (`msdb.dbo.sysmaintplan_*` + задания SQL Agent) с
  детализацией по задачам, разделение «по расписанию / по запросу», алерт на «Обзоре».
