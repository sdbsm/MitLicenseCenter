...
> **`MLC-122` — R6: Видимость лимитов.** Frontend · P3 · Medium. Зависимость: R5 (`MLC-121`,
> инфраструктура обратной связи) и R3 (`MLC-119`, событие `LimitChanged`) — закрыты. Цель —
> ядро ценностного предложения: оператор с одного взгляда видит, кто превысил квоту лицензий.
> Состав (находки `audit/2026-06/TECH-DEBT.md`, UX-02 / FE-03 / UX-46):
> - **UX-02:** визуальный акцент превышения лимита **за пределами дашборда** — на `/reports`,
>   `/tenants` и карточке клиента (сейчас акцент только на дашборде, прогресс-бар ≥90%; на
>   остальных экранах — обычный текст без выделения). Единый визуальный язык «нарушитель квоты».
> - **FE-03:** обновление лимита клиента должно инвалидировать кэш `/reports` (сейчас проценты в
>   отчётах устаревают сразу после правки лимита — `reportsQueryKey` не сбрасывается в мутации
>   смены лимита).
> - **UX-46:** переход к деталям клиента из бокового меню/списка (навигационный хвост — облегчает
>   путь «увидел нарушителя → открыл карточку»).
> Критерии готовности: превышение квоты визуально акцентировано единообразно на `/reports`,
> `/tenants` и карточке клиента; смена лимита немедленно отражается в `/reports` (инвалидация
> кэша); переход к деталям клиента доступен из меню; **canon 05/06 обновлён в том же PR**, если
> меняется UI-язык акцентов; `build.ps1` зелёный. Исполнитель — субагент (worktree); модель
> выбирает куратор. Куратор ревьюит, вливает, ставит R7 в `NEXT TASK`. _Источник детализации —
> `ROADMAP.md` R6; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..121` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-122` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-123`.** Куратор инкрементирует при выдаче; выдавать занятые
