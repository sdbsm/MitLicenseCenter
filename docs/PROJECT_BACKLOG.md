...
> **`MLC-101` — Интерактивный мастер установщика: учётные данные SQL + сеть.** Трек «GUI-установщик»,
> задача 4/6. Решение оператора: мастер спрашивает creds (**оба** режима — Windows-аккаунт **или**
> SQL-логин; аккаунт создаётся заранее), проверяет подключение, пишет `appsettings.Production.json` +
> настраивает службу/порт. Канон: правка **ADR-31** + `OPERATIONS`. План — `.claude/plans/mlc-101-installer-wizard.md`.
...
- **`MLC-101`** · Installer · P2 · Module: Installer wizard · Files: `installer/MitLicenseCenter.iss`,
  канон ADR-31/`OPERATIONS`. Мастер: SQL-инстанс/БД, режим аутентификации (**Windows-аккаунт** —
  служба под ним + ACL key ring + `sc config obj=`; **или SQL-логин** — LocalSystem + creds в строке),
  порт (→ `Urls`+firewall), тест подключения (PS `System.Data.SqlClient`), генерация
  `appsettings.Production.json`. Аккаунт создаётся оператором заранее (права `sysadmin`). Без правки
  backend. План — `mlc-101-installer-wizard.md`. Status: **Approved (NEXT)**.
