...
> **`MLC-131` — R11c: Сортировка/пагинация таблицы сеансов и диалога «не найдены» (UX-14/15 + хвост UX-38).**
> Frontend-only · P3 · Medium. Третий кластер R11 (a/b закрыты `MLC-129`/`MLC-130`). **Важно:** сеансы и
> «не найденные базы» — **live-снапшоты** (`useSessionsSnapshot`, snapshot нераспределённых), НЕ серверные
> пагинируемые запросы → сортировка/пагинация здесь **клиентские** (над уже полученным снапшотом).
> Backend/DTO/parity/enum НЕ трогаем.
> Состав (находки A4):
> - **UX-14:** `SessionsTable` (`features/sessions/`) — добавить клиентскую **сортировку** по колонкам и
>   **пагинацию** (при сотнях сеансов список неуправляем). Переиспользовать `PaginationBar`/`pageLinkRange`;
>   сортировку — клиентскую (стабильную) поверх массива снапшота. Состояние — в `useSessionsPage`/локально.
> - **UX-15:** `MissingInfobasesDialog` (`features/infobases/unassigned/`) — ~50 позиций одним списком;
>   добавить клиентскую пагинацию (и сортировку, если уместно) над списком `MissingItems`.
> - **UX-38 (хвост):** `SessionsFiltersBar` — выпадающий список клиентов сделать **searchable**,
>   переиспользуя `components/ui/SearchableSelect` из R11a (тот же паттерн, что в `AuditFiltersBar`).
> Гочи: снапшот сеансов поллится (live) — сортировка/пагинация не должны «прыгать» при рефетче (стабильный
> ключ строки, clamp страницы при изменении длины); `useTenantConsumption`/parity снапшота не ломать. Канон
> 05 (разделы сеансов / нераспределённых) — в том же PR. i18n-ключи новых подписей — в соответствующих
> неймспейсах.
> Критерии готовности: таблица сеансов сортируется и листается клиентски; диалог «не найдены» пагинирован;
> список клиентов в фильтре сеансов с поиском; `build.ps1` зелёный. Исполнитель — субагент (worktree);
> модель выбирает куратор.
>
> **План кластеров R11** (a ✅ `MLC-129`, b ✅ `MLC-130`):
> - **R11c `MLC-131`** — сеансы + диалог «не найдены»: client-side сортировка/пагинация + searchable
>   фильтр клиентов (UX-14/15/38). ← _текущий_
...
`MLC-001..130` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-131` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-132`.** Куратор инкрементирует при выдаче; выдавать занятые
