...
**`MLC-171` (Security / Installer) — провижининг-креды на выбор: Integrated Security / SQL-sysadmin-логин.**
Уточнение к **ADR-49** (нота добавлена). Проблема: MLC-170 требует, чтобы запускающий установщик
Windows-админ был `sysadmin` на локальном SQL (создание логина идёт под его Integrated Security). На
современном SQL (2012+) локальные админы **не** sysadmin по умолчанию → на части серверов фермы
установка упрётся в предусловие. Решение: дать в мастере **выбор провижининг-личности** — (дефолт)
текущий админ через Integrated Security; (запасной) ввод SQL-логина с ролью `sysadmin` (`sa` или иной).
**Ключевое:** что СОЗДАЁТСЯ — всегда Windows-логин (`CREATE LOGIN … FROM WINDOWS`); SQL-логин — лишь
разовый «ключ» для провижининга, **в конфиг не пишется и нигде не сохраняется** (инвариант «секрета SQL
на диске нет» сохраняется). **Объём (PR исполнителя):** только `installer/MitLicenseCenter.iss` +
present-tense правки `SECURITY.md`/`INSTALL.md`/`OPERATIONS.md`. Новая страница мастера «Подключение
установщика к SQL» (radio Integrated / SQL-логин+пароль); выбранная провижининг-строка используется в
`TestSqlConnection`, `DatabaseHasPanelUsers` **и** `ProvisionSqlLogin` (тестируем то, чем будем
создавать); ортогонально выбору учётки службы (virtual/named). Оговорки в доках: `sa` часто отключён
(спрашивать «любой SQL-логин с sysadmin», не именно `sa`); SQL-логин работает только на mixed-mode
инстансе (на Windows-only — только Integrated Security). Гейт ISCC; бэкенд не трогаем. Спека — план-файл
`gleaming-sparking-cat.md` (раздел MLC-171). **Постановочный PR (нота ADR-49 + бэклог) вливается ДО PR
исполнителя.**

**Завершено в этой сессии:** `MLC-170` (служба под virtual service account, ADR-49) **реализован**
(ISCC зелёный) — живая проверка на стенде за владельцем (чистая установка / апгрейд / негатив / миграция
B→A по `OPERATIONS.md §8`).
...
`MLC-001..170` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
**Следующий свободный номер: `MLC-172`.** (`MLC-171` выдан задаче «провижининг-креды на выбор», ADR-49.)
