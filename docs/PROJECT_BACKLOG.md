...
> **`MLC-116` — Апгрейд: остановка службы перед заменой файлов, без ложной ошибки 1056
> (follow-up ВМ-проверки 2026-06-13).** Maintainability · P1 · High · `installer/MitLicenseCenter.iss`.
> Факт с живого прогона Setup.exe поверх работающей установки: (а) служба не останавливается
> перед заменой файлов → Inno показывает экран «приложения используют файлы» (restart manager);
> (б) в финале `ConfigureService` пытается стартовать уже работающую службу → код **1056**
> (ERROR_SERVICE_ALREADY_RUNNING) → оператору показывается тревожное «Служба создана, но не
> запустилась», хотя служба жива (скрины у владельца). Объём: на апгрейде (`ServiceExists`)
> останавливать службу в `ssInstall` ДО замены файлов (`sc stop` + ожидание остановки),
> запускать после установки; код 1056 при старте трактовать как успех, не предупреждение;
> экран «файлы заняты» при штатном апгрейде появляться не должен. Сверить тексты ошибок
> ConfigureService (сообщение «создана, но не запустилась» на апгрейде ложно — служба уже
> существовала). Критерии: апгрейд поверх работающей службы проходит без экрана «файлы
> заняты» и без предупреждений, служба после апгрейда работает; провал реального старта
> по-прежнему даёт явную ошибку (регрессия MLC-107/112 — поведение чистой установки не
> трогать). Только `.iss`. Исполнитель — субагент (opus, worktree); куратор ревьюит, вливает.
>
> **Следом в очереди (взять после MLC-116): `MLC-117` — Публикация не видит заданный
> `OneC.RAS.Endpoint` до пересохранения настройки (follow-up ВМ-проверки 2026-06-13).**
> Backend · P1 · High · `Infrastructure/Publishing/WebinstArgs.cs` + путь чтения настроек.
> Факт с живого прогона: на свежей установке значение `OneC.RAS.Endpoint` задано (1545),
> discovery/мониторинг кластера РАБОТАЮТ (баннер нераспределённых баз живой), но публикация
> и снятие публикации падают с «Не задан адрес 1С-кластера: укажите OneC.RAS.Endpoint»
> (`WebinstArgs.cs:33`); вылечилось пересохранением значения (1545→1544→сохранить→1545→
> сохранить). Расследовать расхождение путей чтения настройки (кэш настроек у пути
> публикации? формат/нормализация значения? snapshot на старте?), воспроизвести тестом,
> устранить. Критерии: публикация работает сразу после задания настройки И сразу после
> рестарта службы с уже заданной настройкой, без пересохранения; тест на повторение
> сценария; понятная ошибка остаётся только при реально пустом значении.
>
> После закрытия обеих: куратор пересобирает Setup.exe, владелец повторяет апгрейд-проверку
> и публикацию на ВМ; далее вне бэклога — этап D1 «Документация v2» (промт
> `.claude/plans/audit-prerelease-prompts.md`) и перенос roadmap R1–R12 из `TECH-DEBT.md`
> в `ROADMAP.md`.
...
`MLC-001..115` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
...
**Следующий свободный номер: `MLC-118`.** Куратор инкрементирует при выдаче; выдавать занятые
