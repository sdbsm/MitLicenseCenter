...
(ревизия канона) в основном; R2–R8 закрыты полностью (`MLC-118..124`); **R9 в работе** (кластер 1
security-барьер бэкенда закрыт `MLC-125`; остаются поставка/firewall и аккаунт службы); остаётся
R11/R12 (R10 — верификационный остаток). Отмечено в ROADMAP значком ✅.
...
> **`MLC-126` — R9b: Чистка поставки + firewall/bind hardening.** Security/Tooling · P2 · Medium.
> Второй кластер R9 (кластер 1 — security-барьер — закрыт `MLC-125`). Изолированный, низкорисковый:
> правки в `.iss` / publish-скрипте + канон; продуктовый код бэкенда не трогаем.
> Состав (находки `audit/2026-06/TECH-DEBT.md`, детализация — `ROADMAP.md` R9):
> - **REL-08:** убрать `*.pdb` / `appsettings.Development.json` / `web.config` из поставки
>   (`publish-release.ps1` publish-аргументы `/p:DebugType=none` или явные Excludes в `.iss`
>   `[Files]`) — information disclosure + непрофессиональный артефакт. Sanity-чек состава
>   Setup.exe/publish-каталога (желательно автопроверкой в скрипте/тесте).
> - **SEC-06 / SEC-09:** сузить firewall-правило установщика (`netsh advfirewall add rule` в
>   `ConfigureService`): добавить `profile=` (Domain/Private, не Public) и при возможности
>   ограничить scope; рассмотреть `localhost`-bind по умолчанию. **Не регресснуть** штатный
>   LAN-сценарий: оператор, задавший `Urls: http://+:<порт>` через мастер, должен сохранять
>   доступ из LAN (bind/`+` — решение оператора, дефолт можно сузить, но не ломать осознанный
>   выбор). Зафиксировать поведение в `OPERATIONS`.
> - **REL-17:** лицензия Inno Setup (баннер «Non-commercial use only» при сборке) — оценить,
>   зафиксировать **решением в `DECISIONS.md`** (ADR или binding-нота): юридически допустимо для
>   текущего масштаба / план покупки ключа. Не блокер поставки.
> Критерии готовности: состав поставки **не** содержит pdb/Development.json/web.config (проверяемо);
> firewall сужен по профилю и bind/LAN-сценарий не регрессит (описать в каноне); решение по REL-17
> зафиксировано в `DECISIONS.md`; **canon правится в том же PR** (`OPERATIONS`/`INSTALL`/`DECISIONS`);
> `build.ps1` зелёный. Замороженные enum аудита/parity здесь не затрагиваются. **Гоча (биллинг):**
> CI красный — гейт локальный (`build.ps1`); живая ISCC-компиляция `.iss` — за владельцем (ISCC на
> сборочной машине может отсутствовать). Исполнитель — субагент (worktree); модель выбирает куратор.
> **R9-остаток после MLC-126:** кластер 3 — аккаунт службы (SEC-05 low-priv + REL-06 требования
> аккаунта режима A + IIS-права) → отдельная задача `MLC-127`, завершит R9.
> _Источник детализации — `ROADMAP.md` R9; первоисточник находок — `TECH-DEBT.md`._
...
`MLC-001..125` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-126` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-127`.** Куратор инкрементирует при выдаче; выдавать занятые
