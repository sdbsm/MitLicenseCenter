...
«Сеансы» — фильтр по типам сеансов, по умолчанию пользовательские, фоновые скрыты (165). Версия
забампана `0.6.0-beta`. Индекс задач — `PROJECT_BACKLOG_ARCHIVE.md`. **Остаётся за владельцем:** живая
проверка на стенде после апгрейда на `0.6.0` — потребление лицензий по «Сеансам»/дашборду совпадает с
фактом 1С (`rac --licenses`), завершение лишних сеансов идёт по реально лицензионным. Следующую
задачу/трек ставит куратор постановочным PR. Кандидаты — «Открытые опции» ниже и `ROADMAP.md`.
...
`MLC-001..166` закрыты (кроме отложенных `MLC-025/026/145` — см. «Открытые опции»; `MLC-062`/`073`
...
(`MLC-163` — фикс «Проверить публикацию», concurrency-токен в джобе статусов) и трек релиза
`0.6.0-beta` (`MLC-164..166`, ADR-48 — лицензии = факт `rac --licenses`) **завершены** —
