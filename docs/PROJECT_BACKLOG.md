...
`OPERATIONS.md §8`. **`MLC-172`** — фикс показавшегося на стенде бага провижининга (`sp_executesql` +
упрощение текста мастера). Всё вошло в релиз **`0.6.5-beta`** (`0.6.4` отозван как нерабочий).
Следующую задачу/трек ставит куратор постановочным PR.
...
- **Вне трека (Installer, фикс по стенду):** `MLC-172` — на живом стенде выявлены: (1) `ProvisionSqlLogin`
  ...
  **Урок:** установщик-T-SQL гнать против живого SQL до merge — ISCC-компиляция семантику не ловит.
  Релиз `0.6.5-beta`. Только `.iss` + бамп. **Живая проверка — за владельцем.**
