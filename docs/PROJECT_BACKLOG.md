...
**`MLC-027` · Разбить `i18n/ru.json` на per-feature файлы** — трек «Релиз 0.5.0-beta» (полная
постановка в одноимённом разделе ниже). Одна задача за сессию; после закрытия куратор сдвигает
NEXT TASK на следующую по порядку трека. **Волна 1 закрыта:** `MLC-149`/`148`/`147` Done
(2026-06-14, PR #221/#222/#220). Осталось: 027 → 150 → 151 → 152.
...
### MLC-149 · Версия панели в подвале сайдбара · Frontend · S · **Done** (2026-06-14, PR #221)

`SidebarFooter` показывает версию установленной панели из анонимного `/api/v1/health` (хук
`features/health/useHealth.ts`, толерантная Zod-граница); скрыт при недоступном `/health` и
свёрнутом сайдбаре. **Гоча для MLC-027:** добавлен i18n-ключ `nav.version` — учесть при разбиении.

### MLC-148 · Холодный старт /sessions («~2000 лет назад») · Backend/Frontend · Bug · S · **Done** (2026-06-14, PR #222)

(1) Прогрев: разовый `BackgroundJob.Enqueue` cold-снапшота на старте (внутри `!Test`-блока).
(2) `RelativeTime` трактует даты до эпохи Unix (`DateTime.MinValue`) как «ещё не обновлялось» —
**holistically** (все страницы); `sessionColumns.startedAt` → «Нет данных». **Гоча для MLC-027:**
добавлен ключ `common.notUpdatedYet` — учесть при разбиении.

### MLC-147 · xlsx (SheetJS) — 2 high-уязвимости · Security · S · **Done** (2026-06-14, PR #220)

Вариант (a): pin `xlsx` на CDN-tarball SheetJS `0.20.3` (≥0.20.2 — закрывает обе CVE), API тот же,
код экспорта не менялся; `pnpm audit` 2 high → 0. **Гоча (см. `DEVELOPMENT.md`):** сборка теперь
зависит от `cdn.sheetjs.com` при `pnpm install` (integrity-хэш в lock защищает); Dependabot
URL-зависимость не отслеживает — patch SheetJS отслеживать вручную.

### MLC-027 · Разбить `i18n/ru.json` на per-feature файлы · Frontend/Maintainability · P3 · S · ← NEXT TASK

**Задача.** Все надписи UI — один плоский `frontend/src/i18n/ru.json` (1208 строк, 44 top-level
ключа; **+ новые ключи волны 1: `nav.version`, `common.notUpdatedYet`**). Триггер «> 1000 строк»
сработал. (Перенесена из «Открытых опций».)
