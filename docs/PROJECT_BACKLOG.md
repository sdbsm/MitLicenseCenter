...
менять его не нужно. **Активных задач нет.**
...
> **Нет открытых задач.** Активный реестр техдолга (MLC-020..024) закрыт; остались только
> отложенные опции (раздел ниже), каждая gated на свой триггер. Состояние аналогично точке после
> MLC-019 — новую задачу ставить не нужно, пока не сработает триггер одной из опций или не появится
> новый аудит/постановка.
