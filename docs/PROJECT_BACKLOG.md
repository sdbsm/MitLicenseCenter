...
> **`MLC-119` — R3: Аудит-целостность.** Backend · P3 · Medium ·
> `AuditLogger.cs` + call-site'ы (`*Endpoints.cs`, `KillEnforcer`). Закрыть остаток R3
> (BE-10 «аудит неудачных входов» уже сделан в `MLC-114`):
> - **BE-01:** аудит-запись неатомарна с действием — для `DELETE tenant` аудит коммитится
>   ДО удаления → при сбое остаётся ложная запись. Сделать аудит атомарным с действием
>   (единый `SaveChanges`/транзакция: запись аудита коммитится тем же `SaveChanges`, что и
>   само действие, либо откатывается вместе с ним). Проверить остальные мутационные call-site'ы
>   на тот же паттерн «аудит до коммита действия».
> - **BE-11:** событие `LimitChanged` (`AuditActionType` уже объявлен **= 201**, но мёртв —
>   смена квоты лицензий не пишется в журнал, неотличима от переименования). Подключить запись
>   при смене `MaxConcurrentLicenses` (старое→новое значение в описании). **Новый int НЕ вводить —
>   201 уже занят и заморожен; именно его и оживить.**
> - **BE-25:** в `KillEnforcer` до 20 последовательных `SaveChanges` под enforcement-замком —
>   сбатчить аудит-записи (один `SaveChanges`), чтобы замок держался короче (это латентность,
>   не дефект корректности — но в одном «окне» с правками `AuditLogger`).
> Детали находок — `audit/2026-06/TECH-DEBT.md` (строки BE-01/11/25) и `A1`. Критерии готовности:
> аудит атомарен с действием (нет ложных записей при сбое), `LimitChanged` пишется при смене квоты,
> `KillEnforcer` пишет аудит одним `SaveChanges`; новых эндпоинтов/миграций нет; **замороженные
> int-значения `AuditActionType` не трогать** (201 — оживить, не переназначать). Тесты на новое
> поведение обязательны (freeze/поведенческие). Исполнитель — субагент (opus, worktree); модель
> выбирает куратор по сложности. Куратор ревьюит, вливает, ставит следующую итерацию (R4) в `NEXT TASK`.
...
`MLC-001..118` закрыты (кроме отложенных `MLC-025/026/027`, `062`, `073` — см. «Открытые
опции», и активной `MLC-119` — см. «NEXT TASK»; `MLC-080` закрыта в составе `MLC-081`, `MLC-079` —
...
**Следующий свободный номер: `MLC-120`.** Куратор инкрементирует при выдаче; выдавать занятые
