...
> **`MLC-081` (UX-A)** — первая задача трека «UX-пересборка панели под single-host», полная
> постановка — в секции трека ниже. Перед стартом: `git fetch` + FF, ветка от `main`.
> Страховочный тег `v1-pre-ux-redesign` поставлен куратором на main 2026-06-10.
...
- _(пока пусто — пополняется Done-строками задач)_

**Задачи трека (исполнять по порядку):**

- `MLC-081` (UX-A) · Frontend · M · **NEXT TASK** — **«Базы» = Инфобазы + Публикации** (аудит §3.1, §6).
  Единая страница «Базы» (`/infobases`): таблица инфобаз обогащается публикационными колонками
  (Название · Клиент · Статус базы · Публикация: статус + сайт/путь в tooltip · Версия платформы ·
  Проверено · Действия); действия строки — Изменить / Проверить публикацию / Опубликовать / Сменить
  платформу / Бэкапы / Перенести / Удалить (всё существует, собирается в одно меню); чекбоксы +
  bulk-bar («Опубликовать выбранные», «Сменить платформу выбранным») переезжают как есть; колонка
  «Источник» (webinst/Конфигуратор — гейт перезаписи) сохраняется иконкой/tooltip; колонки
  «ID в кластере» и «Имя БД» уходят из списка в форму/детали. «Управление IIS» (пулы/сайты/iisreset) —
  второй вкладкой страницы: «Базы» | «IIS». Страница `/publications` и её пункт меню удаляются.
  **+ влито `MLC-080` (Testing · P3 · Low, отдельным коммитом)** — флейки тайминг-чувствительных
  тестов: (а) `BackupOrchestratorTests.PumpOnce_clamps_max_parallel_to_definition_range`
  (`backend/tests/MitLicenseCenter.Tests.Unit/Backups/BackupOrchestratorTests.cs`; CI PR #61, кейс
  `(99, 9, 8)` упал в `DrainAsync` «условие не наступило за 5с», перезапуск на том же SHA зелёный →
  таймаут тесен для windows-раннера; родственник гонки `MLC-077`/36ef47f) — поднять таймаут
  дренажа/`WaitUntilAsync` 5с → 30с; (б) `SettingsSnapshotTests.Concurrent_cold_readers_load_only_once`
  (второй подтверждённый флейк того же класса, CI docs-only PR #65, перезапуск зелёный) — выровнять
  запас аналогично. По обоим файлам пройтись по остальным тайминг-чувствительным ожиданиям и
  выровнять запасы (тесты ждут условие — зелёный путь не замедлится). Размещено в первой задаче,
  чтобы CI остальных PR трека гонялся стабильно.
- `MLC-082` (UX-B) · Frontend · S — **форма инфобазы без выбора сервера** (§4.1–4.2). Убрать поле
  «Сервер БД» из `PublicationFieldset` — значение из настройки скрыто подставляется в запрос, API
  не меняется; пикер «Имя БД» работает сразу (discovery с сервером из настройки, двухшаговый выбор
  исчезает); пикер «База в кластере 1С» остаётся; per-base переопределения «Сайт IIS» / «Версия
  платформы» / «Виртуальный путь» остаются со своими дефолтами. Из таблицы убрать колонку «Сервер БД».
...
опции»; `MLC-079`/`MLC-080` влиты в `MLC-084`/`MLC-081` UX-трека). Индекс закрытых задач и полные
отчёты — **`docs/PROJECT_BACKLOG_ARCHIVE.md`** (поиск по `MLC-NNN`).
