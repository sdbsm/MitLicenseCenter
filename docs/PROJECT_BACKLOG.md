...
3. `MLC-059` — **NEXT TASK** — Форс-смена пароля при первом входе + время последнего входа (с миграцией):
   две фичи дизайна §3.7, отложенные из MLC-058 (требуют по колонке на `AppUser`), подтверждены пользователем
   как нужные. Зависит от MLC-058. BE+FE, объём M.

**Трек 2/3 (MLC-057/058 закрыты; NEXT = MLC-059, завершит 3/3).**

---

## NEXT TASK

> **`MLC-059` — Форс-смена пароля при первом входе + время последнего входа.** Завершает трек «Полировка
> панели v1.1» (3/3). Backend+Frontend, объём M, **с миграцией** — две колонки на `AppUser`
> (`MustChangePassword`, `LastLoginAt`). Реализует две фичи дизайна §3.7, отложенные из MLC-058 (там
> миграция была запрещена) и подтверждённые пользователем как нужные: новый/сброшенный пользователь при
> первом входе обязан сменить временный пароль; в списке админов видно «последний вход». Зависит от
> MLC-058. Полная спека — в план-файле `C:\Users\andre\.claude\plans\eventual-hopping-pebble.md`.
>
