...
**`MLC-151` · Optimistic concurrency на Infobase/Publication** — трек «Релиз 0.5.0-beta» (полная
постановка в одноимённом разделе ниже). Одна задача за сессию; после закрытия куратор сдвигает
NEXT TASK на следующую по порядку трека. **Закрыто:** `MLC-149`/`148`/`147`/`027`/`150`
(2026-06-14, PR #221/#222/#220/#224/#226). Осталось: 151 → 152.
...
### MLC-150 · Серверный фильтр «Базы» по «не найдена в кластере» · Backend/Frontend · S–M · **Done** (2026-06-14, PR #226)

`GET /infobases?notInCluster=true` фильтрует по обратному дрейфу через общий TTL-кэш снапшота RAS
(`GetClusterSnapshotAsync`, без второго спавна rac.exe); фильтр до пагинации. Ответ несёт
`ClusterAvailable` (omit-null, только при фильтре): при недоступном RAS — пустой набор +
`false`, FE рисует честный баннер «не удалось проверить кластер» (не ложный «0»). Фильтр/чекбокс
только для админа. **Решение:** оставлен на `Roles.Viewer` (фильтр не отдаёт данные кластера, лишь
сужает уже видимые базы; довесок — TTL-опрос RAS, не утечка). Parity: `clusterAvailable ↔
omittable(z.boolean())`.

### MLC-151 · Optimistic concurrency на Infobase/Publication · Backend/Frontend · P2 · M · ← NEXT TASK
