...
> **`MLC-111` — Форма привязки базы: явный выбор клиента (UX-42).**
> Finding A4·UX-42 (High, подтверждён): форма создания/назначения инфобазы предвыбирает
> `tenants[0]` (fallback в defaultValues) — в главном онбординг-сценарии база легко
> привязывается к чужому клиенту, его лимиты считаются неверно; required-валидация есть,
> но бесполезна (поле непусто). Объём (frontend-only): убрать fallback `tenants[0]`;
> пустое значение + placeholder «Выберите клиента»; передача `defaultTenantId` (контекст
> карточки клиента) продолжает предвыбирать корректно. Критерии (тесты обязательны):
> диалог «Назначить» открывается без выбранного клиента; submit без выбора — ошибка поля;
> с `defaultTenantId` предвыбор работает как раньше. Полная постановка —
> `.claude/plans/audit-fix-prerelease.md`; finding — `audit/2026-06/MASTER-REPORT.md`
> (A4·UX-42). Исполнитель — субагент (opus, worktree); куратор ревьюит, вливает,
> ставит следующую задачу трека.
