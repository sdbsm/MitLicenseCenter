...
**Нет активной задачи.** Трек «Рефакторинг + редизайн установщика» **закрыт** (3/3): `MLC-173`
(разрезание `[Code]` на `#include`), `MLC-174` (чистка кода), `MLC-175` (редизайн GUI мастера:
явный радио-выбор провижининга, авто-тест без кнопки, страница-итог `UpdateReadyMemo`) — все влиты,
ISCC зелёный. Версия → **`0.7.0-beta`**. Следующую задачу/трек ставит куратор постановочным PR.
...
**Следующий свободный номер: `MLC-176`.** (`MLC-175` выдан редизайну GUI мастера — явный провижининг + сводка.)
