...
> **Нет активной задачи.** `MLC-077` закрыт (см. индекс «Закрыто»). Следующую `NEXT TASK` ставит внешний
> чат-куратор (исполнитель её сам не выставляет); последняя задача трека «Резервное копирование баз SQL» —
> `MLC-078` (frontend + e2e; спека — `.claude/plans/lazy-cuddling-finch.md`). Кандидат `[Doc divergence]`
> с MLC-076 (Recording-ключи MLC-070 отсутствуют в каталоге настроек 04 §4) по-прежнему ждёт постановки —
