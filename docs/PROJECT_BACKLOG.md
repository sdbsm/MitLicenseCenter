...
> **`MLC-092` — первая сессия трека «Нераспределённые базы»** (постановка — в секции трека
> ниже). Один PR кода backend. Перед стартом: `git fetch` + FF, ветка от `main`; перед
> накаткой миграции на стенд — бэкап БД (`F:\MlcStage2Backups`, обновить).
...
- `MLC-092` (UB-A) · Backend · M · **Approved** — **endpoint unassigned + игнор-лист**.
  Сущность `HiddenClusterInfobase` (Domain) + EF-конфигурация + reversible-миграция;
  `GET /api/v1/infobases/unassigned` с diff и TTL-кэшем (60 с, `refresh=true`,
  `CheckedAtUtc` = время фактического опроса; реализация кэша — по образцу
  `IClusterUuidCache` или `IMemoryCache`); `POST/DELETE …/hide` + гарды + аудит 14/15
  (через `HttpContext.AuditAsync`); чистка игнор-листа в `InfobasesEndpoints.CreateAsync`.
  Тесты (стаб `IClusterClient`, SQLite :memory:; образец `DiscoveryEndpointsTests.cs`):
  diff, кэш/refresh, гарды, `Available:false` без утечки сырой ошибки, чистка при
  создании, Viewer → 403. Ориентиры: `Endpoints/Infobases/` (новый слайс),
  `Application/Clusters/ClusterModels.cs` (контракт `ClusterInfobase`).
- `MLC-093` (UB-B) · Frontend · M · **Approved** — **баннер + диалог + discovery-first
  добавление**. Рукописные TS-типы (ADR-10.1; Zod опционален — тогда `.nullish()` на
  nullable, урок MLC-067/071); TanStack Query + инвалидация после create/hide/unhide;
  баннер (между фильтрами и таблицей вкладки «Базы») + диалог «Нераспределённые базы
  кластера» (строка = имя + UUID-моно; «Назначить» / «Скрыть»; свёрнутый блок
  «Скрытые: N» с «Вернуть»; пустое состояние §6/§9; подзаголовок про лимиты) + i18n;
  префилл `InfobaseFormDialog` (новые props по образцу `defaultTenantId`: предвыбор в
  пикере кластерных баз; `databaseName` — exact case-insensitive guess из `useDatabases`);
  «Добавить базу» → discovery-first c fallback. Тесты Vitest: баннер скрыт при
  `available:false` / count=0 / Viewer; диалог; префилл; fallback.
  Ориентиры: `InfobasesPage.tsx`, `InfobaseFormDialog.tsx`, `useInfobaseForm.ts`,
  `features/discovery/useDiscovery.ts`.
- `MLC-094` (UB-C) · Docs · S · **Approved** — **канон + ADR отдельным PR после вливания
  `MLC-093`**. Новый **ADR-29 «Discovery-first добавление инфобаз + игнор-лист»**
  (решение; rejected: nullable `TenantId`, фоновый снапшот-джоб, ключ настройки TTL;
  связь с ADR-4 read-only-наблюдением и ADR-16); канон `03` (таблица), `04` (endpoint,
  кэш, аудит-коды), `05` §3.3 (баннер/диалог/флоу; `/tenants/:id` — прежний),
  `06` §12 (новые фразы словаря) и §10 при новой иконке; `ROADMAP.md` — в пункте
  «Дрейф панель↔кластер» направление (а) пометить закрытым, (б) оставить обязательным.
  Present-tense, без changelog.
