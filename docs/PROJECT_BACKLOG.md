...
- `MLC-089` (ST-C) · Backend · S · **NEXT TASK (сессия с `MLC-090`/`MLC-091`)** — **удалить ключ
  `OneC.Cluster.Server`** (§7 п.3): из
  `SettingDefinitions` + миграция-чистка значения из `dbo.Settings` (ключ и так скрыт из UI);
  `ResolveClusterServer` (`WebinstArgs.cs`) → host из `RAS.Endpoint` (фактически localhost),
  fallback-логика «кластер не там, где RAS» удаляется.
- `MLC-090` (ST-D) · Backend+Frontend · S · **NEXT TASK (сессия с `MLC-089`/`MLC-091`)** —
  **фильтр «статус публикации»** (принятый кандидат
  с `MLC-081`): параметр в `GET /api/v1/infobases` (серверная пагинация — фильтр честный) +
  UI-фильтр на странице «Базы». Swagger-описание параметра; Zod/типы FE.
