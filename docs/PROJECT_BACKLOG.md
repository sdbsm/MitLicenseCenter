...
> **`MLC-085` + `MLC-086` — объединённая финальная сессия трека** (см. «Темп исполнения»).
> Обе постановки — ниже. **Два PR раздельно:** сначала `MLC-085` (код, коммиты `MLC-085: …`),
> после его вливания — `MLC-086` (канон 05/06, коммиты `MLC-086: …`). Перед стартом:
...
- `MLC-083`: 05 §3.8 описывает на /settings секцию «Значения по умолчанию для новых баз» с полем
  «Сервер СУБД (SQL Server) по умолчанию» — фактически секции нет: `Defaults.DatabaseServer` — отдельная
  секция «SQL Server» с подписью «SQL-инстанс, на котором живут базы клиентов» (единственное место правды,
  не «дефолт для форм»), `IIS.DefaultSiteName` живёт в секции «Публикации IIS»; ключи настроек не
  переименованы (бек-хвост §7 п.4 — этап 2).
- `MLC-084`: 05/06 описывают пункт меню и страницу «Профиль» (`/profile`) и сайдбар из 11 пунктов в группах
  «Операции/Конфигурация/Система» — фактически /profile удалён (смена пароля — диалог из user-меню топбара,
  там же логин + роль), сайдбар = 8 пунктов: Обзор (вне групп) · Мониторинг (Сеансы, Быстродействие, Отчёты) ·
  Управление (Клиенты, Базы) · Система (Аудит, Пользователи, Параметры); пункт и заголовок дашборда — «Обзор»
  (вместо «Главная»).
...
- `MLC-085` (UX-E) · Frontend · S · **NEXT TASK (сессия с `MLC-086`)** — **дашборд-обзор**
  (§3.2, §3.4). KPI-карточки кликабельные:
...
- `MLC-086` (UX-F) · Docs · S · **NEXT TASK (сессия с `MLC-085`, отдельным PR после него)** —
  **финальный док-PR**: канон `05_UI_REQUIREMENTS.md` /
...
`MLC-001..084` закрыты (кроме отложенных `MLC-025..028`, `036`, `062`, `073` — см. «Открытые
