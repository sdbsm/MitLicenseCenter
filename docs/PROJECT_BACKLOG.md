...
> **`MLC-099` — Self-contained single-file publish + publish-скрипт.** Трек «GUI-установщик»,
> задача 2/6. Канон в составе задачи: **ADR-14 update-нота** + `OPERATIONS`/`CLAUDE.md`. Детальный
> план — `.claude/plans/mlc-099-self-contained-publish.md` (куратор, 2026-06-11).
...
- **`MLC-099`** · Build · P2 · Module: Publish/packaging · Files: `scripts/publish-release.ps1`
  (new), канон `OPERATIONS`/`DECISIONS`(ADR-14)/`CLAUDE.md`. `scripts\publish-release.ps1` собирает
  self-contained single-file `win-x64` артефакт (рантайм вшит → хосту .NET не нужен) + `wwwroot` от
  MLC-098; флаги в скрипте, не в csproj; trimming off (EF/Hangfire рефлексивны); `-FrameworkDependent`
  — fallback для хоста с .NET. План — `mlc-099-self-contained-publish.md`. Status: **Approved (NEXT)**.
