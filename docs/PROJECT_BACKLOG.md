...
7. `MLC-068` / `MLC-069` — SQL DMV realtime (backend адаптер по ADR-20 + frontend). **Контур.**
8. `MLC-070` / `MLC-071` — Recording: запись по требованию (backend сущности+сервис+API, миграция;
...
> **`MLC-068` — Backend: SQL DMV realtime-проба** (трек «Анализ быстродействия 1С», Фаза 3;
> следующий по зафиксированному в плане порядку `…→068→069` — **финальное выставление за куратором**).
> Фаза 2 (`066`/`067`) завершена. Объём/права **подтверждены разведкой `MLC-063`**: `ISqlPerformanceProbe` +
> Infrastructure-адаптер DMV (активные запросы `sys.dm_exec_requests` + `sql_text`, цепочки блокировок,
> IO-stall, wait snapshot/дельта через singleton-стейт, **атрибуция database→Infobase→tenant** по
> `program_name='1CV83 Server'`→`DB_NAME`); настройка инстанса (опора на `ISqlInstanceDiscovery`/MLC-056) +
> требование `GRANT VIEW SERVER STATE TO [login]` (доку как «IIS permissions»). Endpoint `GET /performance/sql`.
> **SQL→сеанс→юзер невозможна** (подтверждено 063 — не закладывать). Адаптер по ADR-20 (паттерн host-пробы),
> `[SupportedOSPlatform]`/стаб для тестов; perf-события — не AuditLog.
>
> Полная спека — `C:\Users\andre\.claude\plans\spicy-discovering-torvalds.md` (Фаза 3, `MLC-068`).
> Зависит от 063 (Done). Оценка M. Дальше — `MLC-069` (frontend вкладка SQL), затем Фаза 4 (Recording).
>
...
> Фаза 5 этого трека) остаются gated на триггеры — по умолчанию не берутся.
...
## Закрыто (MLC-001..024, 029, 030, 031, 032, 033, 034, 035, 037, 038, 039, 040, 041, 042, 043, 044, 045, 046, 047, 048, 049, 050, 051, 052, 053, 054, 055, 056, 057, 058, 059, 060, 061, 063, 064, 064a, 065, 066, 067) — индекс
