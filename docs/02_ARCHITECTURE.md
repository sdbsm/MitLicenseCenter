...
В версии 1.2 раздел «Производительность» становится двухрежимным (ADR-57): к live-режиму «Наблюдение»
(probe, метрики `rac`, DMV, «Запись по требованию») добавляется режим «Расследование» — слой **сбора и
...
Реализован слой **управления `logcfg.xml`**: панель сама ставит на узле целевой (не полный) конфиг под
...
«забытого» конфига после краха ОС). **Безопасный сбор** (окно времени, лимит места, авто-снятие по
ADR-58), **NDJSON-парсер** и раскладка ТЖ в агрегаты под объект «Дело» — последующие задачи трека.
Детали backend-слоя — [`04_BACKEND.md`](04_BACKEND.md).
