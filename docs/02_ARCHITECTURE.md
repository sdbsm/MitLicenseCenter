...
### Слой технологического журнала 1С (планируется, трек 1.2)

> Целевое направление версии 1.2; **код ещё не написан**. Описание текущего раздела
> «Производительность/Быстродействие» выше — present-tense, это существующая система.

В версии 1.2 раздел «Производительность» становится двухрежимным (ADR-57): к существующему
live-режиму «Наблюдение» (probe, метрики `rac`, DMV, «Запись по требованию») добавляется режим
«Расследование» — слой **сбора и парсинга технологического журнала 1С** (целевая платформа 8.5,
формат NDJSON). Слой управляет `logcfg.xml` на узле (генерация целевого конфига, окно/лимит/авто-снятие
по политике безопасности ADR-58), пишет ТЖ в локальный каталог сбора и раскладывает его C#-парсером в
агрегаты под объект «Дело». ТЖ и метрики локальны узлу (single-host, ADR-28) — внешних коллекторов нет.
