...
### Слой технологического журнала 1С (трек 1.2)

В версии 1.2 раздел «Производительность» двухрежимный (ADR-57): к live-режиму «Наблюдение»
(probe, метрики `rac`, DMV, «Запись по требованию») добавлен режим «Расследование» — слой **сбора и
парсинга технологического журнала 1С** (целевая платформа 8.5, формат NDJSON). ТЖ и метрики локальны
узлу (single-host, ADR-28) — внешних коллекторов нет.

Слой **управления `logcfg.xml`**: панель сама ставит на узле целевой (не полный) конфиг под
сценарий расследования, пишет ТЖ в локальный каталог сбора под своим контролем и гарантированно снимает
конфиг. Управление файлом `<корень 1С>\conf\logcfg.xml` идёт под сервисным аккаунтом панели; права
записи в `conf` требуют явного гранта ACL (ADR-8/58) — при их отсутствии панель не падает молча, а
отдаёт оператору точную команду `icacls`. На старте процесса **сторож** сверяет фактический `logcfg.xml`
с ожидаемым состоянием и принудительно восстанавливает «снятый» вид, если активного дела нет (защита от
«забытого» конфига после краха ОС).

**Безопасный сбор** (окно времени, лимит места, авто-снятие по ADR-58, один активный сбор за раз),
**NDJSON-парсер** ТЖ 8.5 и набор **анализаторов** реализованы. Анализаторы раскладывают сырьё ТЖ в
результаты-находки (`Finding`) под объект «Дело» (`Investigation`): управляемые блокировки 1С
(TLOCK/TTIMEOUT/TDEADLOCK), долгие запросы к СУБД (DBMSSQL), исключения платформы (EXCP),
СУБД-блокировки (DBMSSQL c полями `lkX`) и серверные вызовы 1С (CALL). Доменная модель «Дела» —
[`03_DOMAIN_MODEL.md`](03_DOMAIN_MODEL.md), детали backend-слоя и эндпоинты — [`04_BACKEND.md`](04_BACKEND.md).

Проектная основа, методические источники и целевой дизайн — база знаний
[`../research/perf-investigation/`](../research/perf-investigation/00_INDEX.md); архитектурное
направление зафиксировано в ADR-57 (двухрежимный раздел) и ADR-58 (безопасный сбор ТЖ).
...
При завершении по лимиту `KillEnforcer` передаёт в `session terminate` флаг
`--error-message` с текстом настройки `Enforcement.TerminateMessage` (свободный текст:
причина + контакты провайдера) — тонкий клиент 1С показывает его пользователю модальным
окном. Пустая настройка → флаг не передаётся, сеанс гасится молча (текущее поведение).
Текст уходит только на enforcement-пути; ручное завершение сеанса оператором из панели
(`SessionsEndpoints`) сообщения не несёт.
