...
- **Whitelist лицензий.** Признак «сеанс потребляет лицензию» вычисляется по вхождению
  `app-id` сеанса в настройку `OneC.LicenseConsumingAppIds`.
...
в том же SQL) и **`IHostedService`-сервисы** вне Hangfire (для sub-minute циклов, которых
CRON-минимум Hangfire не покрывает).

### Hangfire recurring-джобы

| Id | Порт / метод | Cron | Что делает |
|---|---|---|---|
| `cold-snapshot` | `IReconciliationJob.RunColdAsync` | `* * * * *` (раз в минуту; внутренний throttle до `Polling.Cold.IntervalSeconds`) | Полный снимок сеансов кластера, расчёт потребления, продвижение клиентов в hot-tier, семпл телеметрии лицензий и enforcement-kill |
...
`cold-snapshot` защищён `[DisableConcurrentExecution]`; завершённым джобам выставляется
ограниченный срок хранения в схеме `hangfire`. Дашборд Hangfire доступен на `/hangfire`
только роли Admin.
...
Завершение сеансов выполняют **два** пути — cold-джоб (Hangfire) и hot-сервис
...
`cold-snapshot` раз в цикл снимает все сеансы, считает потребление по клиентам и
...
```mermaid
sequenceDiagram
    participant H as Hangfire (cold-snapshot)
