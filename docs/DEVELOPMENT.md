...
### Релизный процесс — релизный поезд (ADR-51)

Мердж в `main` ≠ релиз. Задачи вливаются в `main` **без бампа версии и без тега**; `main` копит
следующий релиз (vNext). Релиз режется **по готовности** (накопился стоящий батч, стенд зелёный),
без календарного ритма:

1. Поднять `<Version>` в `backend/Directory.Build.props` (semver: батч фич → minor, чистые фиксы → patch).
2. Коммит + тег `vX.Y.Z`, запушить тег → `release.yml` собирает `Setup.exe` и публикует GitHub Release.
3. Описание релиза = выжимка закрытых задач за период (`git log <предыдущий-тег>..main` /
   `PROJECT_BACKLOG_ARCHIVE.md`) — отдельный changelog в `docs/` не ведётся.
4. **Обычный релиз** → клиенты видят баннер (ADR-50). Нужна обкатка без шума — сперва **pre-release**
   `vX.Y.Z-rc1` (клиенты его не видят, `/releases/latest`), проверить на стенде, затем обычный релиз.

Закрытие отдельной задачи версию **не** бампает (ADR-51).
...
### 7.1 rac.exe / OEM-кодировка (не хардкод CP866)
...
### 7.3 shadcn только через `scripts/shadcn-add.ps1`
...
### 7.4 EF-миграции: UTF-8 без BOM + LF
...
### 7.5 Бэкенд требует прав администратора (IIS)
...
### 7.6 PlatformVersion — ровно 4 числовых сегмента
...
### 7.7 API опускает null-поля (camelCase, omit-null)
...
### 7.8 Encrypt=False в dev-конфиге SQL
...
### 7.9 PowerShell 5.1: stderr нативных команд как ошибка
