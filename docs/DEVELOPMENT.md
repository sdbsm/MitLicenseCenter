...
Требует Inno Setup 6: `winget install JRSoftware.InnoSetup`. Ищет `ISCC.exe` автоматически
(PATH → per-user → Program Files). Результат: `artifacts\<version>\MitLicenseCenter-Setup-<version>.exe`.
...
| Job | Runner | Шаги |
|---|---|---|
| `backend` | `windows-latest` | restore → build → test → format-check |
| `frontend` | `ubuntu-latest` | install → lint → type-check → build → test |

**Статус: красный (биллинг)**. Джобы не стартуют из-за биллинговых ограничений
аккаунта GitHub Actions. Это известное состояние. **Штатный гейт — локальный
