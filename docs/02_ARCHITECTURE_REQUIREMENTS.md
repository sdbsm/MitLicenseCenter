...
1. **Separation of Concerns:** 
   ...
   - All infrastructure interactions (RAS via `rac.exe`, IIS Administration) must be abstracted behind Infrastructure Adapters.
...
- **Infrastructure Adapters (Anti-Corruption Layer):**
  - *1C Cluster Adapter:* Communicates with 1C Platform (8.3-8.5) via RAS (`ras.exe`) driven by the `rac.exe` CLI — the sole cluster adapter (ADR-16).
