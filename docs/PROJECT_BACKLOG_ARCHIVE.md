...
> задач** `MLC-001..227`: одна строка на задачу (суть + дата + якорь в историю git).
...
## Индекс закрытых задач (MLC-001..227)
...
- `MLC-152` — Low-priv аккаунт службы: Procmon-трасса на стенде (вывод — в ADR-44 + `mlc-152-stand-trace.md`); кодовая попытка снять `sysadmin` с бэкапа (`xp_*`→.NET) — Done (2026-06-14, PR #230), но **откачена** `MLC-153` · `96ad04a`
...
### Housekeeping + Security/Installer (вне треков) — MLC-169..176 (2026-06-15..16)

- `MLC-169` — Ретеншен записей «Быстродействия»: ночная джоба `perf-recording-retention` (03:45 UTC) чистит `PerfRecordings`+сэмплы старше 90 дней (двухфазное батч-удаление, под execution strategy) — Done (2026-06-15) · `20692f0`
- `MLC-170` (ADR-49) — Служба под virtual service account / именная учётка / gMSA; SQL-пароль убран из конфига (`Trusted_Connection`), установщик сам создаёт SQL-логин учётки — Done (2026-06-15) · `fc7b437`
- `MLC-171` — Провижининг-креды на выбор: Integrated Security админа / SQL-логин `sysadmin` (mixed-mode); снимает предусловие MLC-170, SQL-логин транзиентен — Done (2026-06-15) · `e8d7ba9`
- `MLC-172` — Фиксы установщика по стенду: `sp_executesql @sql` вместо конкатенации, родная `InputQueryPage` для SQL-логина, сжатые тексты мастера; релизы `0.6.5`→`0.6.6-beta` — Done (2026-06-16) · `d520d66`
- `MLC-173` — Рефактор `.iss`: монолитная `[Code]` (~1600 строк) разнесена по 8 include-модулям `installer/include/*.iss`, поведение байт-в-байт — Done (2026-06-16, PR #280) · `120b40c`
- `MLC-174` — Чистка кода установщика перед Треком 2: единый `RunPowerShellFile`, общий `ScObjectTail`, сняты висячие параметры и мёртвая константа; поведение-сохраняющий — Done (2026-06-16) · `d0ef599`
- `MLC-175` — Редизайн GUI мастера (Трек 2, релиз `0.7.0-beta`): явный выбор личности провижининга (radio Integrated/SQL-логин + условная страница кредов), авто-тест при «Далее», сводка на «Готово» — Done (2026-06-16) · `fd07da8`
- `MLC-176` (ADR-50) — Проверка обновлений через GitHub Releases: ленивая сверка `latest`-релиза с версией сборки (semver), баннер «Доступна версия», эндпоинты `/updates/{status,check-now}` — Done (2026-06-16) · `c6159f5`

### Трек «Релиз 1.1.0 (расширенный)» — MLC-177..189 (2026-06-17)

- `MLC-177` — Отчёты: date-only трактуется как локальные сутки браузера → UTC-границы на проводе (DST-safe), ось и период сведены; оговорка свежести 15-мин (ADR-23) — Done (2026-06-17) · `4de4353`
- `MLC-178` — Список бэкапов сверяется с фактом `.bak` на диске (`fileAvailable` через батч `xp_fileexist`); keep-latest обнуляет путь вытесненных строк (ADR-27) — Done (2026-06-17) · `037d045`
- `MLC-179` — Полнота аудита: новая 700-серия записей «Быстродействия» (`PerfRecordingStarted/Stopped/Deleted`, host-scope), parity i18n+enum+golden — Done (2026-06-17) · `13d37ad`
- `MLC-180` — Жёсткое удаление учётки `DELETE /users/{id}` (guard'ы отключения, аудит `UserDeleted=109` при успехе) — Done (2026-06-17) · `0a615ee`
- `MLC-181` — Унификация управления списками (a/b/c): серверный поиск по «Базам», единый `SearchableSelect` клиента, bulk «Выбрать все N по фильтру» (`GET /infobases/ids`) — Done (2026-06-17, PR #298–300) · `4ab1fb9`
  - `MLC-181a` — серверный поиск по «Базам» (`Name||DatabaseName`, plain Contains) — PR #298 · `4ab1fb9`
  - `MLC-181b` — единый `SearchableSelect` клиента в тулбаре «Баз» + формах — PR #299 · `4c22d1c`
  - `MLC-181c` — bulk «Выбрать все N по фильтру» (read-only `GET /infobases/ids`, кэп 5000) — PR #300 · `008a3f0`
- `MLC-182` — Список бэкапов различает «вытеснен» (keep-latest, норма) vs «файл отсутствует» (пропал); FE-only — Done (2026-06-17) · `6af263f`
- `MLC-183` — Предпоказ disk-guard в окне бэкапов (`GET /backups/estimate`, Viewer); disk-guard вынесен в `EstimateAsync` — Done (2026-06-17) · `4f77f0f`
- `MLC-184` — «Базы» по фидбэку стенда (a/b): фикс 500 в `/infobases/ids` (ORDER BY до проекции) + редизайн `PublicationsBulkBar` (зона выбора/действий + 3 bulk-диалога) — Done (2026-06-17) · `bceecaf`
  - `MLC-184a` — фикс 500 `GET /infobases/ids`: `OrderBy` до проекции; тест на SQLite — · `bceecaf`
  - `MLC-184b` — редизайн `PublicationsBulkBar` + полный набор bulk-действий (ADR-45) — · `e05afce`
- `MLC-185` — Мониторинг размера баз MSSQL (a–g): `DatabaseSizeSnapshot`+миграция, порт `IDatabaseSizeProbe` (`sys.master_files`), ночные джобы, колонка «Размер», отчёт `/reports/database-size`+вкладка+экспорт — Done (2026-06-17, PR #309–315) · `3465fd8`
- `MLC-186` — Редизайн «Обзора» в живой дашборд (a–d): агрегат `GET /dashboard/alerts`, виджет «Требует внимания», тренды лицензий/размера, лента активности — Done (2026-06-17, PR #318–321) · `ab39710`
- `MLC-187` — ESLint-правило `mlc/no-raw-status-badge` (запрет сырых статус-чипов мимо `StatusBadge`) + миграция 4 живых нарушений на `variant=` — Done (2026-06-17) · `1158c24`
- `MLC-188` — Ярлык квоты ведётся от факта `consumed vs limit` (`quotaLabel`): превышение / лимит достигнут / близко — не от цвета-severity; FE-only — Done (2026-06-17) · `21d12d3`
- `MLC-189` — BE тест-хост уводит DataProtection key ring в `%TEMP%` (env Test) — снимает 8 ACL-падений middleware-тестов; нота к ADR-8 — Done (2026-06-17) · `dc47836`

### Трек «Релиз 1.1.1» — MLC-190..194 (2026-06-17..18)

- `MLC-190` — Текст-причина при enforcement-завершении сеанса по лимиту (`Enforcement.TerminateMessage` → `rac session terminate --error-message`); только enforcement-путь — Done (2026-06-17, PR #337) · `e596766`
- `MLC-191` (ADR-52) — Ночные Hangfire-джобы планируются по местному поясу хоста (`TimeZoneInfo.Local`); хранение времени остаётся UTC (ADR-23) — Done (2026-06-17, PR #340) · `9a0b244`
- `MLC-192` — Фильтр-бар «Аудит» упрощён до одного поля «Поиск» (по описанию+инициатору); поле/параметр `initiator` убраны симметрично FE+BE — Done (2026-06-18, PR #343) · `f572d5a`
- `MLC-193` — Виджет «Требует внимания» разведён на 3 фактических бакета квоты (превышение/достигнут/близко); контракт `QuotaExceeded/AtLimit/NearLimit`+Zod-parity — Done (2026-06-18, PR #346) · `a489723`
- `MLC-194` — Агент-порт 1С → настройка `OneC.RAS.AgentPort` (дефолт 1540); замена константы в `ScRasServiceManager`+heal на апгрейде; установщик не тронут — Done (2026-06-18, PR #347) · `2bfe4a7`

### Трек «Редизайн UX/UI» (Фазы 0–3, ADR-53) — MLC-195..210 (2026-06-18)

- `MLC-195` — Фаза 0 «Дизайн-система»: тёмная тема по умолчанию, монохром sidebar-токены, эталонный экран `/design` (kitchen-sink), namespace `design.*` — Done (2026-06-18, PR #353) · `64ea4b3`
- `MLC-196a` — Фаза 1: «Сеансы» = дом темы лицензий (проекции «По клиентам»/«Живые сеансы», переключатель `?view=`, клиентская склейка без нового BE) — Done (2026-06-18, PR #356) · `f63ac5c`
- `MLC-196b` — Фаза 1: «Отчёты» растворены (license→вид «Использование за период» в «Сеансах», size→вкладка «Размер баз» в «Базах»); FE-only — Done (2026-06-18, PR #358) · `cbe3266`
- `MLC-196c` (ADR-53) — Фаза 1: «Отчёты» убраны из навигации (меню 8→7), KPI лицензий → `/sessions?view=usage`; building-blocks `features/reports/*` сохранены — Done (2026-06-18, PR #362) · `013049c`
- `MLC-197` — CI-инфра: env-guard CP866-теста `ResolveOemEncoding` (self-skip на нерусском OEM-хосте); CI снова реальный гейт — Done (2026-06-18, PR #360) · `955e443`
- `MLC-198` — Фаза 2: редизайн «Обзора» (сигналы наверху, один тренд лицензий, строка здоровья на всю ширину, лента активности; убраны топ-клиенты и тренд размера баз) — Done (2026-06-18, PR #365) · `6c3c321`
- `MLC-199` — Фаза 2: доуборка present-tense канона под растворение «Отчётов» + редизайн «Обзора» (`USER_GUIDE`/`01_OVERVIEW`/`06_UI_GUIDE`); docs-only — Done (2026-06-18, PR #366) · `2739862`
- `MLC-200` — Фаза 2: редизайн «Клиентов» (слитая колонка «Лицензии» + полоса, карточка потребления + «Сеансы клиента →», тех-колонки скрыты) — Done (2026-06-18, PR #368) · `80b7a4d`
- `MLC-201` — Фаза 2: редизайн «Пользователей» (аватар-инициалы у логина, иконка роли щит/глаз) — Done (2026-06-18, PR #371) · `0382ff5`
- `MLC-202` — Фаза 2: редизайн «Параметров» (левая sticky-навигация по 8 секциям-якорям + scroll-spy, монохром) — Done (2026-06-18, PR #374) · `f5098f0`
- `MLC-203` — Фаза 2: редизайн экранов входа (`LoginPage`→shadcn-примитивы, общий `AuthCardShell` с версией панели из `useHealth`) — Done (2026-06-18, PR #376) · `ed00f2f`
- `MLC-204` — Фаза 2: редизайн «IIS» (зелёные кнопки «старт» → вариант `success` компонента `Button`, не токен `--status-success`) — Done (2026-06-18, PR #377) · `436a6e7`
- `MLC-205` — Фаза 2: сквозная ревизия ~32 диалогов (P1=0; 8 кнопок на `variant="destructive"`; KillSession сохраняет type-to-confirm, ADR-45) — Done (2026-06-18, PR #379) · `c201f25`
- `MLC-206` — Фаза 2: ревизия «Баз» (уже соответствовала §1.4; колонки Платформа/Проверено скрыты по умолчанию) — Done (2026-06-18, PR #380) · `5f4e5c7`
- `MLC-207` — Фаза 3 «Быстродействие» Срез A: drill-down переключатель Хост/1С/SQL (`PerformanceDrillDown` Tabs) + авто-фокус по вердикту (`autoFocusLayer`); FE-only — Done (2026-06-18, PR #382) · `307c852`
- `MLC-208` — Фаза 3 Срез B: кликабельный светофор (`MetricGauge` опц. `<button>`) → фокус слоя (`layerForResource`); FE-only — Done (2026-06-18, PR #383) · `bb70e3e`
- `MLC-209` — Фаза 3 Срез C: расшифровка SQL топ-ожиданий (`waitCategory`) + колонка «Клиент» в IO-stall (`attributionFor`); FE-only — Done (2026-06-18, PR #385) · `ef720f6`
- `MLC-210` — Фаза 3 Срез D: единый блок «Блокировки» (`PerformanceBlockingBlock` — SQL-цепочки `blockingSessionId` + 1С `blockedByDbms/Ls`); FE-only — Done (2026-06-18, PR #386) · `3719adf`

### Трек «Сервер» (ADR-54/55/56, выпущен в `v1.1.3`) — MLC-212..226 (2026-06-19)

- `MLC-212` — Надёжный контроллер службы Windows: `IWindowsServiceController` (команда `sc` + верификация фактического состояния `ServiceController` до целевого с таймаутом, идемпотентность 1056/1062, per-service-замок), ADR-55 — Done (2026-06-19)
- `MLC-213` — Агрегатор статусов служб узла `GET /api/v1/server/status` (1С/RAS/SQL/IIS + overall, never-throws) + управление сервером 1С `POST /server/onec/{start,stop,restart}` (Admin, confirm, whitelist, аудит 800–802), ADR-54/55 — Done (2026-06-19)
- `MLC-214` — FE-экран «Сервер» (меню 7→8, группа «Мониторинг») + вкладка «Службы» + индикатор здоровья + плашка `ServerHealthCard` на «Обзоре» — Done (2026-06-19)
- `MLC-215` — Переезд управления IIS в вкладку «IIS» экрана «Сервер» (`features/server/iis/`); вкладка IIS убрана из «Баз» — Done (2026-06-19) · PR #401
- `MLC-216` — Вкладка «Обслуживание»: свежесть бэкапов (`msdb.dbo.backupset`, live-read, `is_copy_only=0`, never-throws + `HAS_PERMS_BY_NAME`), `BackupFreshnessPolicy` 26ч — Done (2026-06-19) · PR #404
- `MLC-217` — Планы обслуживания (`sysmaintplan_*` + SQL Agent) с детализацией по задачам, scheduled/on-demand, `AgentUnavailable` для Express; алерт обслуживания на «Обзоре» — Done (2026-06-19) · PR #406
- `MLC-218` — Расписание авто-рестартов сервера 1С (Hangfire, местный пояс, `OneCAutoRestartJob`, аудит 803/804) + FE-маппинг всей 800-серии аудита (parity) — Done (2026-06-19) · PR #408
...
- `MLC-223` — Хотфикс (приёмка на стенде): `WindowsServiceController` квотирует имя службы в `sc start/stop` — службы 1С зовутся `1C:Enterprise 8.5 Server Agent (x86-64)` (пробелы/двоеточие/скобки), без кавычек `sc` возвращал 1639 ERROR_INVALID_COMMAND_LINE. + регресс-тест на имя с пробелами (фейк `FakeScRunner` стал писать полную строку аргументов, а не резать по пробелу). Версия `1.1.3` — Done (2026-06-19) · PR #415
- `MLC-224` — Хотфикс (приёмка на стенде): бюджет верификации состояния `WindowsServiceController` 30с→300с (ADR-55). Холодный старт/стоп кластера 1С идёт минутами → рестарт ложно падал «не успела запуститься за 30 с», хотя служба поднималась (повторный рестарт срабатывал). Полл возвращается сразу при достижении состояния, потолок бьёт лишь по детекту реального отказа. Версия `1.1.3` — Done (2026-06-19) · PR #415
- `MLC-225` — Хотфикс (приёмка на стенде): рестарт службы 1С ждёт ПОЛНОЙ остановки перед стартом. `ServiceState.IsRunning` = (Status==Running) считал службу остановленной уже на `StopPending`, и `sc start` уходил в ещё останавливающуюся службу (не действовал → старт-фаза висела до таймаута; второй рестарт срабатывал). Добавлен `ServiceState.IsStopped` (Status==Stopped; IsRunning/IsStopped — НЕ инверсии, Start/StopPending дают оба false); `WaitForState`: старт ждёт IsRunning, стоп — IsStopped. Регресс-тест «пендинг ≠ остановлена». Версия `1.1.3` — Done (2026-06-19, подтверждён на стенде) · PR #416
- `MLC-226` — Хотфикс (приёмка на стенде): память rphost показывала ~1 МБ вместо ~990 МБ (≈1000×). `rac process list` отдаёт `memory-size` в КИЛОБАЙТАХ, а FE `formatBytes` (и «Сервер», и «Быстродействие») трактует как байты. `ParseProcessLoads` нормализует ×1024L → поле в байтах. Сессионная `memory-current` остаётся в байтах (1С непоследователен). Версия `1.1.3` — Done (2026-06-19, подтверждён на стенде) · PR #416

### Вне трека — вход из локальной сети (2026-06-20)

- `MLC-240` — Хотфикс: вход из LAN не проходил (а с самого сервера работал), включая `admin`. Auth-кука `mlc.auth` в проде помечалась `Secure` безусловно, но штатный деплой — plaintext http (`"Urls":"http://+:port"`); браузер принимает `Secure`-куку на `http://localhost` (защищённый контекст), но выбрасывает на `http://<имя-хоста>` из сети → логин проходил, клиент оставался неавторизованным («пароль не подходит»). Секьюрность куки привязана к транспорту (`TransportSecurity.AuthCookieSecurePolicy`, ADR-59): дефолт http-LAN → `SameAsRequest` (чинится сам, без правки конфига); `Always` — при `EnforceHttps=true` или новом `Security:RequireSecureCookie=true` (TLS-реверс-прокси). Канон: ADR-59 + OPERATIONS + SECURITY + шаблон конфига. Версия `1.1.5` — Done (2026-06-20)

### Вне трека — устойчивость службы (2026-06-19)

- `MLC-227` — Отзыв жёсткой SCM-зависимости службы `MitLicenseCenter` от SQL (пересмотр ADR-40): жёсткий `depend= MSSQLSERVER` каскадно гасил панель при ручной остановке SQL, а Windows не поднимал её обратно. Установщик удаляет `ConfigureSqlDependency`/`DeriveLocalSqlServiceName` и активно снимает зависимость на install+upgrade (`sc config … depend= /`); устойчивость к медленному старту SQL — на recovery-политике; панель сама переподключается (EF `EnableRetryOnFailure`). Канон: ADR-40 + OPERATIONS. Версия `1.1.4` — Done (2026-06-19) · PR #418
