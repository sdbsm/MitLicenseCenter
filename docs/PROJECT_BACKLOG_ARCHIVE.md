...
### MLC-109 — Немедленный отзыв доступа при disable/reset/смене роли (SEC-01) — Done (2026-06-12, PR #121)

**Постановка.** Finding A2·SEC-01 (High, подтверждён): отключение пользователя, сброс пароля
и смена роли не отзывали активные cookie-сессии — SecurityStampValidator не был подключён,
sliding-кука 8h при активности жертвы делала доступ фактически бессрочным.

**Сделано (PR #121, ветка `mlc-109-security-stamp-revocation`, только backend):**
- `Infrastructure/DependencyInjection.cs` — явная регистрация
  `ISecurityStampValidator`/`ITwoFactorSecurityStampValidator` +
  `SecurityStampValidatorOptions.ValidationInterval = 2 мин` (середина требуемого окна
  1–5 мин; компромисс «свежесть отзыва ↔ round-trip в БД на горячем пути» зафиксирован
  комментарием). **Гоча:** `AddIdentityCore` (в отличие от полного `AddIdentity`) не
  регистрирует ни валидатор, ни его опции — без явной регистрации `OnValidatePrincipal`
  падает на резолве.
- `Web/Program.cs` — `options.Events.OnValidatePrincipal =
  SecurityStampValidator.ValidatePrincipalAsync` в ручном cookie-пайплайне + регистрация
  служебной no-op куки `IdentityConstants.TwoFactorRememberMeScheme`. **Найденный по ходу
  скрытый баг:** при отклонении куки валидатор делает «sign-out everywhere» — и по
  Application-, и по TwoFactorRememberMe-схеме; ручной пайплайн регистрировал только
  Application → первый же реальный отзыв доступа упал бы с
  `InvalidOperationException: No sign-out authentication handler is registered for the
  scheme 'Identity.TwoFactorRememberMe'` (2FA в панели не используется, кука чисто служебная).
- `Web/Endpoints/Users/UsersEndpoints.cs` — `UpdateSecurityStampAsync` в `DisableAsync`
  (lockout сам закрывает только будущий вход, выданную куку не отзывает) и `ChangeRoleAsync`
  (`Add/RemoveFromRoleAsync` stamp не трогают). `ResetPasswordAsync` без изменений — Identity
  ротирует stamp внутри `UserManager.ResetPasswordAsync` (подтверждено тестом).
- `Web/Endpoints/Auth/AuthEndpoints.cs` без изменений — регрессионный механизм self-смены
  пароля (`RefreshSignInAsync` переиздаёт куку свежим stamp'ом) уже существовал.

**Тесты (новый `SecurityStampRevocationTests.cs`, 4 шт.):** реальные
`UserManager`+`SignInManager` над EF InMemory, зеркало прод-DI (явная регистрация валидатора,
обе cookie-схемы), `ValidationInterval = Zero`; кука прогоняется через
`SecurityStampValidator.ValidatePrincipalAsync` ровно как в `OnValidatePrincipal`:
- отключённый пользователь — старая кука отвергается;
- Admin→Viewer — старая кука с Admin-claims отвергается без пере-логина;
- reset-password — кука жертвы отвергается (доказывает внутреннюю ротацию stamp);
- self-смена пароля — собственная сессия живёт. Тест усилен по ревью куратора: перехват
  `IAuthenticationService` захватывает principal, **реально переизданный** эндпоинтом через
  `RefreshSignInAsync` (валидатор его принимает), а старая кука отвергается; самопроверка —
  с закомментированным `RefreshSignInAsync` тест красный. Без перехвата тест проходил бы и
  при сломанной регрессии.

**Проверки:** `dotnet test` — 640/640 зелёные (исполнитель + независимый повтор куратора);
`dotnet format` чисто, pre-commit пройден.

**Гочи следующим задачам трека:** существующие сессии переживают деплой этого изменения —
