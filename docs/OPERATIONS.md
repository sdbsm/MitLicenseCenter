...
| `Security.RequireSecureCookie` | `true` за TLS-реверс-прокси, чтобы сохранить флаг `Secure` на auth-куке (ADR-59); по умолчанию off |
...
#### Флаг `Secure` на auth-куке (`Security:RequireSecureCookie`, ADR-59)

Кука сессии `mlc.auth` помечается флагом `Secure` (браузер хранит и шлёт её только
по HTTPS) **по тем же условиям, что и `EnforceHttps`** — секьюрность куки следует за
транспортом (`TransportSecurity.AuthCookieSecurePolicy`):

- **штатный http-LAN** (дефолт мастера: `"Urls":"http://+:<порт>"`, `EnforceHttps=false`) —
  кука **без** `Secure`. Иначе вход из локальной сети по `http://<имя-хоста>` невозможен:
  браузер делает исключение для `http://localhost` (защищённый контекст), но молча
  выбрасывает `Secure`-куку на не-localhost http → логин на сервере проходит, но клиент
  остаётся неавторизованным. На http-деплое без TLS флаг `Secure` защиты и не даёт;
- **Kestrel сам терминирует TLS** (`EnforceHttps=true`) — кука `Secure` автоматически;
- **за TLS-реверс-прокси** (`EnforceHttps=false`, redirect/HSTS делает прокси): Kestrel
  видит http от прокси, поэтому для сохранения `Secure` задайте `Security:RequireSecureCookie=true`.

> Регресс «вход из LAN не проходит, а с самого сервера работает» (баг MLC-240, до 1.1.5)
> вызывался жёстким `Secure` в проде поверх http-деплоя. С 1.1.5 дефолт чинит это сам.
...
LAN-выбор, поддерживается и не считается регрессией. Вход из сети по http при этом работает,
т.к. auth-кука в таком деплое не помечается `Secure` (см. «Флаг `Secure` на auth-куке», ADR-59).
