...
- **What it installs.** Files into `{autopf}\MitLicense Center`; the service `MitLicenseCenter` running `MitLicenseCenter.Web.exe` under **LocalSystem** (`start= auto`); a default `appsettings.Production.json` (co-located SQL `Server=.`, `Trusted_Connection=True`, `Urls=http://+:8080`, `EnforceHttps=false`) written with `onlyifdoesntexist`; an inbound firewall rule **"MitLicense Center"** allowing TCP **8080**; and the key-ring directory `%ProgramData%\MitLicenseCenter`. It requires admin (UAC elevation). **Interactive SQL / hostname / port entry is MLC-101** — the skeleton uses the working defaults above; edit `appsettings.Production.json` after install if the SQL instance or port differs.
- **Upgrade (re-run the new `Setup.exe` over the existing install).** A fixed `AppId` detects the install. The wizard **stops the service and waits** for it to stop (`PrepareToInstall`) so the locked `.exe` can be replaced, copies the new files, and starts the service again. **Preserved across upgrade:** `appsettings.Production.json` (excluded from the bulk copy + `onlyifdoesntexist`), the Data Protection key ring under `%ProgramData%\MitLicenseCenter` (never deleted), and the SQL database. On start, EF migrations apply **fail-fast** (ADR-18): a schema upgrade is applied synchronously or the service exits without serving — **take a database + key-ring backup before upgrading a version that carries migrations** (forward-only; rollback = restore the backup).
- **Uninstall** stops and deletes the service and removes the firewall rule. It **leaves the database and the key ring** in place (keep-data prompt + polish is MLC-103) — to fully remove, drop the database and delete `%ProgramData%\MitLicenseCenter` by hand after confirming no other use of the key ring.
- **Diagnosing service start.** The backend runs service-aware (`UseWindowsService`, ADR-31), so under the service its logs go to the **Windows Event Log** (Application source `MitLicenseCenter`), not a console. If the service fails to start or stops immediately, look there first: fail-fast bootstrap (ADR-18) writes a `Critical` entry explaining why it refused to serve (e.g. SQL unreachable, pending migration error), and on a first run against a fresh database the **initial `admin` password** is emitted to the same log. (If `sc start` itself times out with error 1053 the binary is not service-aware — but the shipped build always calls `UseWindowsService`, so that points to running a wrong/old `.exe`.)
