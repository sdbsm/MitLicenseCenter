...
Роль `sysadmin` для бэкапа **не нужна** (MLC-152): достаточно права `BACKUP DATABASE`
(входит в `db_owner` на базе панели). Файловые операции (проверка места, создание
подкаталога, удаление прежних `.bak`) выполняет сама служба панели вызовами .NET
(`BackupFileStore`; single-host, ADR-28) — расширенные процедуры
`xp_fixeddrives`/`xp_create_subdir`/`xp_delete_file` больше не используются. Нет права
`BACKUP DATABASE` → ошибка `PermissionDenied` с подсказкой, не 500.

**NTFS-права на каталог бэкапов** (`Backup.FolderPath`): `.bak` пишет служба **SQL Server**,
каталог создаёт и старые `.bak` удаляет служба **панели** — каталог должен быть доступен на
запись/удаление **обеим** учётным записям.
