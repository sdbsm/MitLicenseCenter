...
- **Export** («Скачать» menu in each section header — the summary and the drill-down export **separately**, hidden while the series is empty). Formats: **CSV** (UTF-8 **with BOM**, `;` separator, decimal comma — RU-Excel-friendly; plain bucket table), **XLSX** (two sheets — «Сводка» metadata + «Данные» with numeric cells so Excel pivots/charts work), **HTML** (a self-contained offline file reproducing the panel's interactive chart — the Chart.js engine is inlined into the document, no CDN; opens in any browser with working tooltips, plus the summary stats and bucket table) and **PDF** (title + summary + a rendered chart image + bucket table; a Cyrillic-capable font — Roboto Regular, Apache-2.0 — is embedded, since jsPDF's built-in fonts lack Cyrillic). Export is **fully client-side** — the already-loaded `LicenseUsageSeriesResponse` is serialized in the browser, no export endpoint; the heavy libraries (`xlsx`, `chart.js`, `jspdf`/`jspdf-autotable`) are `dynamic import`-ed on click and split into per-format lazy chunks. Lives in `features/reports/export/`.
