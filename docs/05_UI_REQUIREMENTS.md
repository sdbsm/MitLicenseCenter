...
- **Export** («Скачать» menu in each section header — the summary and the drill-down export **separately**, hidden while the series is empty). Formats: **CSV** (UTF-8 **with BOM**, `;` separator, decimal comma — RU-Excel-friendly; plain bucket table) and **XLSX** (two sheets — «Сводка» metadata + «Данные» with numeric cells so Excel pivots/charts work). Export is **fully client-side** — the already-loaded `LicenseUsageSeriesResponse` is serialized in the browser, no export endpoint; the heavy `xlsx` library is `dynamic import`-ed on click. Lives in `features/reports/export/`.
