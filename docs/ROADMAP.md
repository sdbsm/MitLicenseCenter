...
### R9 — Инсталлятор hardening ⏳ В РАБОТЕ (кластеры 1–2/3 закрыты)
...
- **Кластер 3 — аккаунт службы** (`MLC-127`): REL-06 (явные требования к аккаунту службы в режиме A +
  IIS-права — экран/чек-лист мастера, иначе публикации в «Ошибка проверки»); SEC-05 (служба сверх
  потребности LocalSystem/local-admin — фиксация принятого риска в `DECISIONS.md`/`SECURITY.md`; узкий ACE на
  `inetsrv\config` требует runtime-трассировки на стенде → отложенный low-priv follow-up).
