...
> **R10** (этап D1 + верификация `MLC-128`) и **R11** (`MLC-129..133`). Остаётся **R12** (полировка UX).
...
### R12 — Полировка UX ⏳ В РАБОТЕ (разбита на кластеры)
...
- **Кластер e `MLC-138`** — UX-кластер E (доступность, контраст бейджей WCAG AA, destructive-стиль кнопок,
  primary-кнопка в тёмной теме, русские aria-label — UX-26/27/28/29/40). Frontend-only. ← _текущий_
- **Кластер f** — UX-кластер F (онбординг и точки трения: бэкапы не только из строки базы, видимость
  срока хранения журнала, «Отмена» в смене пароля — UX-25/36/39/41; UX-46 уже закрыт в MLC-122).
