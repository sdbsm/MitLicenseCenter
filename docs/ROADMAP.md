...
> **R5** (`MLC-121`), **R6** (`MLC-122`), **R7** (`MLC-123`), **R8** (`MLC-124`) и (этапом D1) **R10**.
...
### R9 — Инсталлятор hardening ⏳ В РАБОТЕ (кластер 1/3 закрыт)
...
**Остаток (кластеры 2–3):**
- **Кластер 2 — чистка поставки + firewall/bind** (`MLC-126`): REL-08 (убрать pdb / Development.json /
  web.config из поставки); SEC-06 / SEC-09 (scope firewall-правила по профилю, localhost bind по
  умолчанию — не регресснуть LAN); REL-17 (лицензия Inno Setup — решение в `DECISIONS.md`).
- **Кластер 3 — аккаунт службы** (`MLC-127`): REL-06 (явные требования к аккаунту службы в режиме A +
  IIS-права); SEC-05 (low-priv аккаунт службы вместо LocalSystem — оценить совместимость с key ring ACL
  и SQL-доступом).
