...
- **Кластер a `MLC-129`** — аудит: текстовый поиск + фильтр по инициатору (UX-20), переход на
  страницу N / последнюю (UX-35), searchable-списки действий (UX-37) и клиентов (UX-38). Пагинация и
  фильтры actionType/tenantId/from/to уже есть.
- **Кластер b `MLC-130`** — серверная пагинация + поиск по клиенту для `/backups` и
  `/performance/recordings` (UX-05/BE-17) — пока без пагинации.
