...
- **UI: заспечено в `05`/`06`, но не построено в v1.** Дизайн-канон описывает эти фичи, код их пока не реализует (каждое место помечено «не в v1» по месту в `05_UI_REQUIREMENTS.md` / `06_UI_DESIGN.md`):
  - **Экран «Администраторы»** (`05` §3.6) — список/создание/отключение/сброс пароля админов. Сейчас один админ сидится при старте, пароли меняются через Профиль, роли — в БД.
  ...
  - **Графики дашборда на `recharts`** (`06` §2) — библиотека в зависимостях, не подключена; дашборд на карточках/прогресс-барах.
