...
| `dashboard` | `/` — живой «Обзор»: виджет «Требует внимания», KPI-карточки (кликабельны; спарклайн лицензий + live-точка сеансов), тренд использования лицензий 7д, строка состояния системы (здоровье RAS + хоста + сервера 1С, `ServerHealthCard` — светофор `overall` + **сигнал обслуживания** «бэкап устарел»/«план обслуживания упал», MLC-217; ссылка на `/server`), лента свежей активности |
...
| `performance` | `/performance` — двухрежимный раздел (MLC-241/243, ADR-57): переключатель **«Наблюдение» \| «Расследование»** в шапке; дефолт — «Наблюдение». **Режим «Наблюдение»** (`ObservationMode`): метрики хоста, 1С, SQL — воронка: вердикт «почему тормозит» → светофор ресурсов (`SaturationGauges`) → **drill-down переключатель слоёв** Хост/1С/SQL (`PerformanceDrillDown`, контролируемые shadcn `Tabs`) → единый блок «Блокировки» (`PerformanceBlockingBlock`) → запись по требованию; CTA «Начать расследование» переключает в режим «Расследование». **Режим «Расследование»** (`InvestigationMode`, MLC-242/243): хранит под-состояние list/wizard/progress/detail(id) — маршрут остаётся `/performance`. **Приоритет прогресса**: если есть активное дело (`Collecting`/`Analyzing`) — показывается **Прогресс активного сбора** (`InvestigationProgress`, экран 6): статус, сценарий, цель, прошедшее время, объём собранного, кнопка «Остановить сейчас» (Admin → `useStopInvestigation`), плашка-гарантия авто-снятия `logcfg`. **Дефолт (нет активного дела)** — **Список дел** (`InvestigationList`, экран 5, MLC-243): заголовок «Расследования» + кнопка «+ Новое расследование»; строка-баннер активного сбора сверху с кнопкой «Перейти к прогрессу»; таблица дел (колонки: №/период/сценарий/цель/статус `StatusBadge`/запустил/находок); клиентские фильтры по сценарию и статусу; клик по строке → **Дело** (`InvestigationDetail`, экран 3); удаление строки (Admin, `useDeleteInvestigation`, диалог `DeleteInvestigationDialog`; 409 INVESTIGATION_ACTIVE → тост). **Дело** (`InvestigationDetail`, экран 3, MLC-243): шапка (id, бейджи статус/сценарий, период, цель, кнопка «Назад» к списку, кнопка «Отчёт» открывает экран «Отчёт» (`InvestigationReport`, документ-вид + экспорт PDF, MLC-244)); **Вердикт и рекомендации** из `useInvestigationReport` (ранжированные `items[]` — severity `StatusBadge` + headline + recommendation); **Сводка-метрики** (числа из типизированных result найденных `Finding`); **блок «Цепочка блокировок 1С»** (finding `ManagedLocks` → `LockAnalysisResult`: waitEdges, timeouts, deadlocks — только 1С-уровень); **блок «Топ долгих запросов»** (finding `SlowQueries` → `SlowQueryAnalysisResult`: topQueries с длительностью, контекстом, SQL, planText в `<details>`); **блок «Исключения»** (finding `Exceptions` → `ExceptionAnalysisResult`: topExceptions с типом, описанием, count, флаг isDatabaseException с оговоркой о парных EXCP); **блок «СУБД-блокировки»** (finding `DbmsLocks` → `DbmsLockAnalysisResult`: waitEdges жертва→источник — ⚠ ОТДЕЛЬНЫЙ блок, не смешивать с 1С); **блок «Серверные вызовы 1С (по контексту)»** (finding `Call` → `CallAnalysisResult`, MLC-249: topCalls с длительностью/контекстом/методом в `<details>` + агрегат групп по контексту — размещён рядом с «Долгими запросами», это CALL-сторона `GeneralSlow`; ⚠ у `CALL` нет привязки к арендатору). **Полировка отображения агрегатов (MLC-249):** под-секундные длительности в группах (`SlowQueries` и `Call`) показываются в **миллисекундах** («макс 6 мс») через хелпер `fmtDurationShort` (`< 1 c → «N мс»`, иначе `fmtSeconds`), а не «0.0 с»; в блоке «Долгие запросы» — строка-итог **«SQL всего: ~X c»** (сумма `totalDurationSeconds` по группам — у SQL это допустимо, каждое `DBMSSQL` — лист). **В блоке Call (MLC-252 A-2) аналогичной строки «Вызовы всего» НЕТ** — время `CALL` вложенное (gross), суммировать нельзя; вместо неё подпись о вложенности + бейджи групп «контекст не указан» (`isUnspecified`) и «обёртка» (`isWrapper`). **Группировка CALL — по ОСМЫСЛЕННОМУ контексту (MLC-252 A-1):** числовые коды/служебные токены сведены в одну группу «контекст не указан», а не плодят мусорные группы. **Счётчик покрытия контекста (MLC-252 B)** над группами долгих запросов: «Контекст 1С привязан к N из M групп». **Мастер запуска** (`InvestigationWizard`, экран 2): открывается из списка кнопкой «+ Новое расследование» — выбор сценария (4 варианта: Locks/SlowQueries/Exceptions/GeneralSlow; DbmsLocks не показывается) + scope + плашка безопасности + кнопка «Запустить» (Admin → `useStartInvestigation`; 409 INVESTIGATION_ACTIVE / INVESTIGATION_START_FAILED — inline). **Контракт `finding.result`** типизирован пер-Kind (MLC-243): Zod-схемы `LockAnalysisResult` / `SlowQueryAnalysisResult` / `ExceptionAnalysisResult` / `DbmsLockAnalysisResult` / `CallAnalysisResult` (MLC-249) — parity с BE-DTO анализаторов (`*AnalysisResult.cs`), camelCase, WhenWritingNull; graceful `.catch(null)` — несовместимый future-result не роняет деталь. Слой «Хост» = атрибуция семей процессов, «1С»/«SQL» = собственные live-источники (`OneCLoadSection`/`SqlLoadSection`). Авто-фокус (`useDrillDownFocus`) наводит на релевантный слой по вердикту (`culpritFamily`: OneC→1С, Mssql→SQL, иначе Хост), ручной выбор слоя «прибивает» его — авто-фокус больше не перебивает. Светофор кликабелен: клик по гейджу ресурса наводит drill-down на слой (`layerForResource`) — CPU/RAM ведут на слой доминирующей по ресурсу семьи (как авто-фокус), а Диск всегда на слой SQL (у диска нет атрибуции по семьям, доказательство дисковой нагрузки — IO-stall по базам — живёт в SQL-слое); клик пинит выбор, как ручной выбор вкладки. Все три слоя `forceMount` (неактивные скрыты `hidden`) — их polling 1С/SQL не прерывается в фоне, переключение мгновенно (MLC-207/208). SQL-слой, секция «Конкуренция за ресурсы» (`SqlContentionTables`): топ-ожидания показывают сырой `waitType` (моно) с приглушённой подписью-расшифровкой смысла под ним, когда тип распознан по префиксу (`waitCategory` в `sqlLoad.ts` → ключ `performance.sql.waits.meanings.*`; нераспознанный → без подписи); IO-stall по базам несёт колонку «Клиент» — связка база→инфобаза→клиент через ту же атрибуцию payload (`attributionFor`), что и активные запросы/нагрузка по базам, без BE-правок (MLC-209). Единый блок «Блокировки» (`PerformanceBlockingBlock`) между drill-down и записью сводит в одно место два сигнала контеншена, иначе разнесённые по слоям 1С и SQL: цепочки блокировок SQL (`blockingSessionId` → бейджи «ждёт сеанс N»/«блокирует», `lockChainRows`) и заблокированные сеансы 1С (`blockedByDbms`/`blockedByLs`, `blockedSessions`); читает оба live-источника повторно (общий кэш React Query по ключу — двойного polling нет), рендерится всегда (сквозной сигнал, не зависит от host-снимка); пусто (никто никого не ждёт) → нейтральное «блокировок нет» (хороший случай, без красных акцентов); акценты только через `StatusBadge`, без BE-правок (MLC-210) |
...
| `server` | `/server` — раздел «Сервер» (MLC-214/215, ADR-54/55). Viewer наблюдает, Admin управляет сервером 1С и IIS. Экран с табами (`Tabs`, дефолт «Службы»). **Вкладка «Службы»**: общий индикатор здоровья узла (`ServerHealthBadge` — светофор `overall`: Healthy→success/Degraded→warning/Down→danger/Unknown→neutral), список серверов 1С (имя/версия/`StatusBadge`; Admin: «Запустить» прямой кнопкой, «Остановить»/«Перезапустить» через `OneCServerActionDialog` с `confirm:true`, ADR-55 «да/нет» + предупреждение о простое баз), сводки RAS/SQL/IIS только наблюдением (`available:false` → плашка ошибки, не падение); загрузка статуса (`useServerStatus`) живёт в этой вкладке. Под списком 1С — карточка **«Расписание авто-рестартов»** (`AutoRestartCard`, MLC-218, ADR-55): тумблер «включено» + поле времени (`HH:mm` по часам хоста, `<input type="time">`) + строка «прошлый прогон» + перечень целевых служб (запущенные `ragent`); сохранение (`useSetAutoRestartSchedule`) — **только Admin** (роль-гейт как у `OneCServerActionDialog`), Viewer видит состояние без правки; Zod-граница `useAutoRestartSchedule.ts` (`omittable()` для `lastRunUtc`). Ниже — блок **«Рабочие процессы 1С»** (`OneCProcessesCard`, MLC-219/220, ADR-54/56): таблица процессов кластера (`rphost`) из `GET /server/onec/processes` — UUID процесса (короткий + полный в тултипе), `PID`, доступная производительность (тинт ↓ через `availablePerformanceBand` из `features/performance/onecLoad`), ср. длительность вызова и память; форматтеры/тинт **переиспользуются** из «Быстродействия» (тот же источник `rac process list`). Отсутствующие perf-поля → «—», пустой список → подсказка «процессов нет». **Admin** (роль-гейт как у `OneCServerActionDialog`) видит колонку «Действия» с кнопкой **«Перезапустить»** в строке — мягкий рестарт `rphost` по `Pid` через `OneCProcessRestartDialog` (`confirm:true` на `POST /server/onec/processes/restart`, MLC-220): диалог «да/нет» с предупреждением, что операция **разорвёт активные сеансы на этом процессе** (у `rac` нет «restart process» → рестарт = завершение ОС-процесса `rphost`, кластер 1С поднимает новый); кнопка недоступна, если `Pid` не пришёл. Мутация `useRestartOneCProcess` (`useInvalidatingMutation`) инвалидирует `oneCProcessesQueryKey` — таблица перезапрашивает снимок. Viewer — только чтение (колонки действий нет). Zod-граница `useOneCProcesses.ts` (`omittable()` для perf-полей; ответ рестарта `{ pid, outcome }`). **Вкладка «IIS»**: детальное управление IIS (`IisManagementCard`, дом IIS = «Сервер», ADR-54) — пулы/сайты/`iisreset`, переехало сюда из «Баз» (MLC-215); карточка живёт в `features/server/iis/`, монтируется при активации вкладки (запросы `/iis/*` стреляют лениво). **Вкладка «Обслуживание»** (`MaintenanceTab`, монтируется лениво): свежесть бэкапов SQL (таблица D/I/L + `StatusBadge` «устарел/свежо», MLC-216) и под ней **блок планов обслуживания** (`useMaintenancePlans`, MLC-217) — под-планы каждого плана с итогом последнего прогона (`OutcomeBadge`: Failed/Overdue→danger, Succeeded→success, NeverRun→neutral), пометкой «по расписанию / по запросу», длительностью и **развёрткой по задачам** (`<details>` — что именно упало, `StatusBadge` на каждом шаге); деградация (`AgentUnavailable`/`PermissionDenied`/`Unavailable`) — честной плашкой, не падением. Zod-граница `/api/v1/server/*` (`useServerStatus.ts`, `useMaintenanceBackups.ts`, `useMaintenancePlans.ts`) — nullable-поля через `omittable()`; `overall`/`state`/`status`/`outcome` — `z.string()` (forward-compat). Тот же `serverStatusQueryKey` делит плашку «Обзора» (`ServerHealthCard`) — один запрос на оба места; плашка «Обзора» дополнительно запрашивает обе пробы обслуживания для **сигнала обслуживания** (контракт `/server/status` не трогаем — parity 213, алерт композируется на FE) |
...
| `server` (статус + обслуживание) | `serverStatusSchema`, `serverOperationSchema`, `backupFreshnessSchema` (свежесть бэкапов SQL, MLC-216), `maintenancePlansSchema` (планы обслуживания + детализация по задачам, вкладка «Обслуживание», MLC-217), `autoRestartScheduleSchema` (расписание авто-рестартов, MLC-218) | `useServerStatus`, `useOneCServerOperation`, `useMaintenanceBackups`, `useMaintenancePlans`, `useAutoRestartSchedule`, `useSetAutoRestartSchedule` |
...
/                 — ProtectedRoute → AppShell
  ...
  /server         — ServerPage; три вкладки «Службы»/«IIS»/«Обслуживание» (вкладка «Обслуживание»: свежесть бэкапов SQL + планы обслуживания с детализацией по задачам, read-only, MLC-216/217), «IIS»/«Обслуживание» монтируются лениво. Viewer наблюдает; управление сервером 1С — Admin, гейт на действиях, не на маршруте
  /audit          — AuditPage
...
**Двухрежимный раздел `/performance` (MLC-241, ADR-57).** Раздел переключается сегмент-контролом
(`Tabs`, `default`-вариант) в шапке: **«Наблюдение»** | **«Расследование»**, дефолт — «Наблюдение».
`PerformancePage` — тонкая оболочка: хранит `mode: "observation" | "investigation"`, рендерит
...
`onOpenReport(id)` → под-состояние `report`). Блок «Долгие запросы» показывает топ единичных тяжёлых
(`topQueries`) **и** агрегат **«Похожие запросы (по суммарному времени)»** (`similarGroups`: нормализованный
SQL + «N раз · суммарно X · макс Y»; MLC-248) — независимый от порога; если топ пуст, а группы есть, агрегат
всё равно показан (кейс «много мелких»). **В группе (MLC-251)** показывается **типовой контекст 1С** (моно,
подпись «Контекст 1С (типовой)» — мостик «тяжёлый SQL → код 1С», корреляция с серверным вызовом `CALL` на
бэкенде) и **база** (бейдж); если у группы нет `sampleContext` (сценарий без сбора `CALL`) — строка контекста
не выводится, база показывается при наличии. **Счётчик покрытия (MLC-252 B):** над группами — строка
«Контекст 1С привязан к N из M групп» (`groupsWithContext`/`groupsTotal`), чтобы пустая корреляция была видна,
а не молчала; у исторических дел поля опущены (`?? 0`, строка скрыта). **Блок «Серверные вызовы 1С»**
(`CallBlock`, MLC-249/252): строка-итог **«Вызовы всего» убрана** (время `CALL` вложенное/gross — суммировать
нельзя), вместо неё подпись «Время вызовов вложенное…»; группа-«мусорка» помечена бейджем
**«контекст не указан»** (`isUnspecified` — числовые коды/служебные токены), фоновый вызов-обёртка — бейджем
**«обёртка»** (`isWrapper`, вынесена в конец). **Мастер** (`InvestigationWizard`, экран 2: сценарий + scope из
реестра ИБ + **поле «Порог длительности запроса, c»** — только для сценариев `SlowQueries`/`GeneralSlow`,
дефолт 1, шлётся как `slowQueryThresholdSeconds`; для прочих сценариев скрыто и не шлётся — MLC-248 + плашка
безопасности + «Запустить» Admin). **Документ-«Отчёт»** (`InvestigationReport`, экран 4,
...
server.*          — раздел «Сервер»: tabs.*, health.*, onec.*, summary.*, dialog.*, toasts.*, autoRestart.* (карточка расписания авто-рестартов, MLC-218), dashboard.* (+ dashboard.maintenance.* — сигнал обслуживания на «Обзоре», MLC-217), maintenance.* (вкладка «Обслуживание»: свежесть бэкапов MLC-216 + maintenance.plans.* планы обслуживания MLC-217)
audit.*           — аудит
