...
- **Сеансы** — `useSessionsPage` строит экземпляр `DataTable` (`useReactTable`, ADR-46) с
  ...
  density. **Дефолт `appIds`** (параметр отсутствует) — только интерактивные типы
  (`INTERACTIVE_APP_IDS` ∩ присутствующие): фоновые задания скрыты, пока оператор их не добавит;
  явный пустой выбор = «показать пусто» (`appTypes.ts`/`resolveAppIds`).
