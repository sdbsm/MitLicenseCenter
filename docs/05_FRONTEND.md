...
**`InvestigationMode` (MLC-242)** = вход в воронку: если есть активное дело (статус `Collecting`/`Analyzing`,
по `useInvestigations`) — рендерит **Прогресс активного сбора** (`InvestigationProgress`, экран 6: статус/
сценарий/цель/прошедшее время/объём + «Остановить сейчас» для Admin); иначе — **Мастер запуска**
(`InvestigationWizard`, экран 2: сценарий + scope из реестра ИБ + плашка безопасности + «Запустить» для
Admin). Полноценные экраны Дело (3), Отчёт (4), Список дел (5) — **планируются** в MLC-243..244.
Целевой UX (концепция) — [`30_TARGET_DESIGN.md`](../research/perf-investigation/30_TARGET_DESIGN.md),
