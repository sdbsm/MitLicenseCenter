...
(`?tenantId=`) и статуса публикации (`?publishStatus=`) сохранены как есть (через
`useSearchParams`, вне tanstack `columnFilters`) и размещены в слоте тулбара рядом с меню
видимости колонок и density. Множественный выбор для bulk-операций (публикация/смена
