...
**Режим «Расследование» (планируется, трек 1.2).** Целевое направление версии 1.2; **экраны ещё
не реализованы**. Текущая `PerformancePage` («Быстродействие») описана выше present-tense — это
работающий live-экран. В 1.2 раздел `/performance` становится двухрежимным (ADR-57): «Наблюдение»
(существующий live-режим) | «Расследование» (новое). Под «Расследование» планируются новые экраны —
Мастер запуска, Дело, Отчёт, Список дел, Прогресс сбора. Целевой UX (концепция) — база знаний
[`30_TARGET_DESIGN.md`](../research/perf-investigation/30_TARGET_DESIGN.md), детальная спека 6 экранов —
[`70_UI_SPEC.md`](../research/perf-investigation/70_UI_SPEC.md); направление зафиксировано в ADR-57.
