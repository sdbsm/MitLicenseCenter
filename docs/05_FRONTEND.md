...
| `server` (статус + обслуживание) | `serverStatusSchema`, `serverOperationSchema`, `backupFreshnessSchema` (свежесть бэкапов SQL, MLC-216), `maintenancePlansSchema` (планы обслуживания + детализация по задачам, вкладка «Обслуживание», MLC-217) | `useServerStatus`, `useOneCServerOperation`, `useMaintenanceBackups`, `useMaintenancePlans` |
...
server.*          — раздел «Сервер»: tabs.*, health.*, onec.*, summary.*, dialog.*, toasts.*, dashboard.* (+ dashboard.maintenance.* — сигнал обслуживания на «Обзоре», MLC-217), maintenance.* (вкладка «Обслуживание»: свежесть бэкапов MLC-216 + maintenance.plans.* планы обслуживания MLC-217)
audit.*           — аудит
