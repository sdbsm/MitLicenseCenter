...
- **Тренды** (`LicenseTrendCard`/`DatabaseSizeTrendCard`) — компактные recharts на хуках отчётов
  ...
  мемоизирован один раз — стабильный query-key; локально-суточные границы общие с building-blocks
  отчётов, MLC-177). Один license/size-запрос на страницу делят трендовая карточка и KPI-спарклайн.
