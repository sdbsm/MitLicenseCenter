...
При старте процесс разово ставит `BackgroundJob.Enqueue<IReconciliationJob>(RunColdAsync)`
(прогрев): рекуррентный тик срабатывает не сразу, и без прогрева окно до первого опроса
оставляет `ActiveSessionSnapshotStore` в исходном состоянии (`CapturedAtUtc =
DateTime.MinValue`), а `/sessions` показывает пустой список с меткой «ещё не обновлялось».
Прогрев наполняет снапшот сразу после запуска воркера. В тест-среде (`Test`) прогрев,
как и регистрация рекуррентных джоб, пропускается.
