...
места/авто-стоп, single-active по БД, orphan-recovery) и **NDJSON-парсер** собранного ТЖ; **агрегаты**
(анализаторы блокировок/долгих запросов/исключений) под объект «Дело» — последующие задачи трека.
...
- **`ITechLogParser`** (Application) / `TechLogParser` (Infrastructure) — построчный парсер собранного
  ...
  → аксессор возвращает `null`. На нём встанут анализаторы этапа B (блокировки/долгие запросы/исключения).
