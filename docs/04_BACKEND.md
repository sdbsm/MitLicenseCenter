...
сбор технологического журнала режима «Расследование» (**806–808**: `TechLogCollectionStarted = 806`
— старт сбора, `TechLogCollectionStopped = 807` — снятие сбора с причиной в описании (его же пишет
авто-стоп по окну/диску), `TechLogConfigForceRestored = 808` — сторож на старте снял «забытый»
`logcfg` без активного дела; host-scope `TenantId=null`, ADR-57/58);
удаление завершённого «Дела» расследования (**809**: `InvestigationDeleted = 809` — Admin удалил дело
через `DELETE /api/v1/investigations/{id}` каскадом с находками, host-scope `TenantId=null`,
зеркаль `PerfRecordingDeleted = 702` — MLC-239);
...
| `InvestigationStatus` | `Domain/TechLog/InvestigationStatus.cs` | `Collecting = 0`, `Analyzing = 1`, `Completed = 2`, `Interrupted = 3`, `Failed = 4` |
| `InvestigationStopReason` | `Domain/TechLog/InvestigationStopReason.cs` | `Manual = 0`, `TimeLimit = 1`, `DiskLimit = 2`, `Error = 3` |
| `FindingKind` | `Domain/TechLog/FindingKind.cs` | `ManagedLocks = 0`, `SlowQueries = 1`, `Exceptions = 2`, `DbmsLocks = 3`, `Call = 4` |
| `InvestigationScenario` | `Domain/TechLog/InvestigationScenario.cs` | `Locks = 0`, `SlowQueries = 1`, `Exceptions = 2`, `GeneralSlow = 3`, `DbmsLocks = 4` |
...
- **`ISlowQueryAnalyzer`** (Application) / `SlowQueryAnalyzer` (Infrastructure) — анализатор
  **долгих запросов к СУБД** (MLC-234/248). Строит из `IEnumerable<TechLogEvent>` топ медленных вызовов
  (`TopQueries`, отсортирован по длительности убывающим, топ-N, **гейт по порогу** — единичные тяжёлые)
  и группы похожих запросов (`SimilarGroups`, агрегат по нормализованному SQL: строковые/числовые
  литералы и параметры схлопываются к `?`; отсортированы по суммарной длительности убыв., ограничены
  топ-N). **Граница: ТОЛЬКО `DBMSSQL`**. Ключевой факт стенда (`40_TECHLOG §6`):
  фильтр по длительности (`<ge property="Dur"/>`) в `logcfg` для JSON-ТЖ 8.5 не работает — **порог
  длительности (`thresholdMicroseconds`) применяет этот анализатор**, а не logcfg. **Агрегат
  `SimilarGroups` НЕЗАВИСИМ от порога `TopQueries` (MLC-248): считается по ВСЕМ `DBMSSQL` с непустым
  `Sql`, включая под-пороговые, — типовая медленность («закрытие месяца»: тысячи быстрых запросов одной
  формы) всплывает суммарным временем, даже когда ни один запрос не длиннее порога.** **Корреляция
  SQL↔CALL (MLC-251):** у событий `DBMSSQL` фоновых заданий часто НЕТ поля `Context` («какой код 1С выдал
  этот SQL»), а у `CALL` — есть. Анализатор связывает каждый долгий запрос с породившим серверным вызовом
  `CALL` по `t:connectID` + временно́му окну. Событие ТЖ логируется в МОМЕНТ ОКОНЧАНИЯ, поэтому окно вызова
  `CALL` = `[ts − duration, ts]`; вложенные `DBMSSQL` того же `connectID` лежат внутри окна. Алгоритм — **два
  прохода по событиям** (конвейер передаёт `List`, повторная итерация дёшева; `CALL` кратно меньше `DBMSSQL`):
  проход 1 — индекс окон `CALL` по `connectID` (только с непустым `Context`; `ts` парсится толерантно,
  `InvariantCulture`); проход 2 — для каждого `DBMSSQL` бинарным поиском по `Start` находится **охватывающее**
  окно (при вложенности — самый внутренний, последний по `Start`), его `Context` копится в гистограмму группы.
  Группа несёт `SampleContext` (самый частый контекст; `null`, если CALL/контекста нет — штатно для сценария
  `SlowQueries` без сбора `CALL`), `Database` и `InfobaseName` (самые частые среди вхождений — **прямо из
  `DBMSSQL`**, корреляция для базы/ИБ не нужна). **Корреляция матчит по `t:connectID`** — это сквозное
  транспортное поле соединения, присутствует и у `DBMSSQL`, и у `CALL` (имя поля одно с обеих сторон;
  фикстуры стенда подтверждают `t:connectID` у `CALL`). **Fallback (MLC-252 B):** если строго охватывающего
  окна нет (фоновый/отчётный `DBMSSQL` вне окна `CALL`, но на том же соединении), берётся **ближайший
  предшествующий** `CALL` с непустым `Context` (его окно завершилось не позже `ts` запроса) — fallback
  ограничен тем же соединением, контекст пустым не подставляется. **Счётчик покрытия (MLC-252 B):** результат
  несёт `GroupsTotal` / `GroupsWithContext` — скольким из возвращённых групп удалось привязать контекст, чтобы
  пустая корреляция (на стенд-приёмке 1.2 с параллельными нагрузками контекст не привязался ни к одной группе)
  была видна оператору, а не молчала. Расшифровка имён таблиц (`_Reference229`→объект) и синонимы —
  НЕ здесь (требуют метаданные ИБ, отдельная фича). Порог настраивается
  пер-расследование (см. `TechLogCollectionService`/эндпоинт старта ниже), дефолт **1 c** (`1 000 000` µs;
  раньше был жёстко 5 c — давал 0 находок на типовой нагрузке, стенд-приёмка 1.2). Объём входа
...
- **`ICallAnalyzer`** (Application) / `CallAnalyzer` (Infrastructure) — анализатор **серверных вызовов
  ...
  агрегат **групп по контексту** (`SimilarGroups`: ключ — **осмысленный** `Context`; счётчик/суммарное/макс;
  отсортированы по собственному (gross) времени убыв., топ-N). **Граница:
  ...
  `CpuTime`/`Memory` (как сырые строки). **Осмысленный ключ группы (MLC-252 A-1):** предпочитается
  `Context` (стек 1С); иначе **именованный** метод, но только если он не чисто числовой (`^\d+$`) и не в
  денлисте служебных токенов (`methodsCount`/`Release`/`methods`/…); иначе — единая группа
  **«контекст не указан»** (`IsUnspecified`). Числовые коды (`0`/`5`/`7`/`83`) и служебные токены НЕ
  плодят отдельных групп (стенд-приёмка 1.2 дала десятки мусорных групп — причина: группировка падала на
  сырой `Method`/`MName`/`IName`). **Вложенность времени (MLC-252 A-2):** время `CALL` ВЛОЖЕННОЕ (gross) —
  родительский вызов включает вложенные SQL/под-вызовы, поэтому суммы по группам **не складываются** в общий
  итог (FE-строка «Вызовы всего» убрана как вводящая в заблуждение). Верхний фоновый вызов-**обёртка**
  (`IsUnspecified` + длительность ≥ 30 c) помечается `IsWrapper` и выносится в конец сортировки, чтобы не
  доминировал в топе. **Агрегат `SimilarGroups` НЕЗАВИСИМ от порога** (как у
...
- **`ITechLogCollectionService`** (Application) / `TechLogCollectionService` (Infrastructure, singleton)
  ...
  **single-active по БД** (есть `Status==Active` дело → `AlreadyActive`, ещё до записи конфига —
  ...
  повторная установка не переписывает конфиг, снятие хранит снимок установленного. **Порог долгих запросов
  (MLC-248):** `InstallAsync` принимает опц. `slowQueryThresholdMicros` (микросекунды), кладёт его в снимок
  `CollectionConfig.DurationThresholdMicros` для сценариев `SlowQueries`/`GeneralSlow` (иначе `null`),
  дефолт **1 c**; конвейер анализа при снятии берёт порог **из снимка дела** (а не из хардкод-константы).
...
удаление дела — `InvestigationDeleted` (809).

**Эндпоинты `/api/v1/investigations` (`InvestigationEndpoints`, MLC-239; зеркаль `PerformanceEndpoints`
— чтение Viewer, мутации Admin):**

| Метод и путь | Роль | Назначение |
|---|---|---|
| `GET /investigations` | Viewer | Список дел, свежие сверху, серверная пагинация (конверт `{items,total,page,pageSize}`). |
| `GET /investigations/{id}` | Viewer | Деталь дела: шапка + снимок сбора + находки с разобранным `result`-объектом. 404, если нет. |
| `GET /investigations/{id}/report` | Viewer | Отчёт: ранжированные находки + текстовые рекомендации (шаблоны по `Kind`). |
| `GET /investigations/{id}/progress` | Viewer | Лёгкий поллинг: статус + старт + прошедшее время + (для активного) размер собранного. |
| `POST /investigations` | Admin | Старт расследования: резолвит инфобазу из реестра, привязывает дело, ставит `logcfg`. `409` если уже идёт; `400` при отрицательном пороге. |
| `POST /investigations/{id}/stop` | Admin | Ручной стоп активного дела (`StopReason=Manual`). `409`, если `id` не текущее активное. |
| `DELETE /investigations/{id}` | Admin | Удаление дела каскадом с находками. `409 INVESTIGATION_ACTIVE`, если дело в `Collecting`/`Analyzing`. Аудит `809`. |

Старт/стоп идут через `ITechLogCollectionService` (он же аудирует 806/807); список/деталь/отчёт/прогресс
читают `AppDbContext` напрямую (вертикальный срез, ADR-20). Enum'ы на проводе — строкой
