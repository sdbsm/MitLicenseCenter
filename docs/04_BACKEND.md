...
(MLC-234), **исключений платформы** (MLC-235) и **СУБД-блокировок** (MLC-236). Этап B (парсер +
анализаторы) завершён; анализатор СУБД-блокировок реализован (MLC-236).
...
- **`ILogcfgBuilder`** (Application) / `LogcfgBuilder` (Infrastructure) — чистый генератор целевого
  ...
  `EXCP/EXCPCNTX`+`<dump/>`; общая медленная работа `CALL/DBMSSQL`; **СУБД-блокировки `DbmsLocks` —
  `DBMSSQL` + config-level `<dbmslocks/>` + целевые `lkX`-свойства (`lka/lkp/lkpid/lkaid/lksrc/lkpto/
  lkato`) внутри `<log>` без `<property name="all"/>`**) — никогда `<property name="all"/>`.
  **Теги-обогатители `<plansql/>`, `<dump/>` и `<dbmslocks/>` — глобальные директивы УРОВНЯ `<config>`
  (рядом с `<log>`, не внутри него; шаблоны infostart 2020498/1431026): `<plansql/>` включает захват
  планов, `<dump/>` (с атрибутами `location`/`create`/`type`, дампы в подкаталог каталога сбора →
  их объём учитывает сторож размера) — дампы аварий, `<dbmslocks/>` — сбор СУБД-блокировок. ⚠ Тип/объём
  дампа — за стенд-приёмкой (full-dump тяжёл на проде); точная семантика `<dbmslocks/>` и структура
  полей lkX в JSON-ТЖ 8.5 — за стенд-приёмкой (infostart 1431026, 40_TECHLOG §5).**
...
- **`ILockTreeAnalyzer`** (Application) / `LockTreeAnalyzer` (Infrastructure) — анализатор
  ...
  **Граница: ТОЛЬКО 1С-уровень** (менеджер блокировок платформы). Блокировки уровня СУБД НЕ видны
  в этих событиях — их источник отдельный тег `<dbmslocks/>` с полями `lkX`; реализовано
  `IDbmsLockAnalyzer` (MLC-236). Не обещать единое дерево всех блокировок, иначе ложная полнота
...
- **`IDbmsLockAnalyzer`** (Application) / `DbmsLockAnalyzer` (Infrastructure) — анализатор
  **СУБД-блокировок** (MLC-236). Строит из `IEnumerable<TechLogEvent>` дерево «жертва → источник»
  по полям `lkX` событий `DBMSSQL`. **ГРАНИЦА: ТОЛЬКО СУБД-уровень** (тег `<dbmslocks/>` + поля
  `lkX`; отдельный механизм от управляемых блокировок 1С — `ILockTreeAnalyzer`, MLC-233; не
  обещать единое дерево, иначе ложная полнота, `40_TECHLOG §5`). Алгоритм: `жертва.lksrc` →
  `источник.connectID` → ребро `DbmsLockWaitEdge`; если источник в окне не найден — ребро всё равно
  добавляется с `SourceMatched=false` (частичное), счётчик `UnmatchedVictimCount` растёт. Привязка
  к ИБ через `TechLogProcessName.Normalize` (`p:processName`; GUID-суффикс фоновых сессий
  отсекается, сырое значение сохраняется в `RawProcessName`, `40_TECHLOG §8`). Устойчив к
  «полям-призракам» `lkX` (появляются только при блокировке), вариантам имён `connectID`
  (`t:connectID`/`connectID`), дублям ключей. **Никогда не бросает**. Stateless singleton.
  ⚠ **Структура полей lkX собрана по документации (infostart 1431026, 40_TECHLOG §5); точная
  форма в JSON-ТЖ 8.5 подлежит подтверждению на стенде (приёмка владельца).**
