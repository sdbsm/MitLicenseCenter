...
- **`GET /server/onec/processes` (Viewer)** — рабочие процессы 1С (`rphost`) для блока
  ...
  **Never-throws + деградация пустым списком:** `rac` не настроен/недоступен → адаптер отдаёт
  пустой список, эндпоинт **не 500-ит**. **Рестарт процесса НЕ реализован** (исследовательская
  часть MLC-219): у `rac` нет команды «restart process», а единственный «мягкий» путь —
  OS-завершение процесса по `Pid` с авто-перезапуском кластером — требует проверки на живом
  кластере (контракт надёжности ADR-55 не имеет для эфемерного `rphost` аналога `ServiceController`
  для верификации состояния). До подтверждения безопасного механизма блок — **только список**.

Управление **RAS и IIS здесь не дублируется** (остаётся в `/ras-service/*` и `/api/v1/iis/*`);
**SQL — без управления** (ADR-54, только наблюдение). Рабочие процессы 1С (`rphost`) — пока
**только список** (рестарт исследуется, см. выше).
