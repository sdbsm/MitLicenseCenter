...
места/авто-стоп, single-active по БД, orphan-recovery), **NDJSON-парсер** собранного ТЖ,
**анализатор управляемых блокировок 1С** (MLC-233) и **анализатор долгих запросов к СУБД**
(MLC-234); агрегатор исключений — последующая задача трека (MLC-235).
