...
- `.\scripts\build-installer.ps1 [-Configuration Release] [-OutputDir <путь>] [-SkipPublish]` — собрать GUI-установщик (Inno Setup): гонит `publish-release.ps1` → `installer\MitLicenseCenter.iss` через ISCC → `MitLicenseCenter-Setup-<version>.exe` (дефолт `artifacts\<version>`). Предусловие — Inno Setup 6 (`winget install JRSoftware.InnoSetup`). Ставит панель одной Windows-службой под LocalSystem, порт 8080 в firewall, обновление поверх (ADR-31).
