...
- `.\scripts\publish-release.ps1 [-FrameworkDependent] [-OutputDir <путь>] [-SkipSpaBuild] [-PrebuiltSpaDist <dist>]` — собрать установочный артефакт: по умолчанию self-contained single-file `win-x64` (рантайм .NET вшит → на хосте .NET не нужен) + `wwwroot` (SPA из MLC-098) + `appsettings`; `-FrameworkDependent` — fallback для хоста с .NET 10. Trimming не используется (EF/Hangfire/Identity рефлексивны).
- `.\scripts\build-installer.ps1 [-Configuration Release] [-OutputDir <путь>] [-SkipPublish]` — собрать GUI-установщик (Inno Setup): гонит `publish-release.ps1` → `installer\MitLicenseCenter.iss` через ISCC → `MitLicenseCenter-Setup-<version>.exe` (дефолт `artifacts\<version>`). Предусловие — Inno Setup 6 (`winget install JRSoftware.InnoSetup`). Интерактивный мастер: SQL-инстанс/БД, режим аутентификации (Windows-аккаунт под службой / SQL-логин при LocalSystem), учётка (аккаунт создаёт оператор заранее, права sysadmin), тест подключения, порт → Urls+firewall; пишет appsettings.Production.json из ввода; обновление поверх (ADR-31).
- `.\scripts\shadcn-add.ps1 <name>` — добавить shadcn-компонент (обход pnpm-isolation на Windows; см. гочи).
- `.\scripts\perf-seed.ps1` / `.\scripts\perf-counters.ps1` — нагрузочный seed + dotnet-counters (dev/test). Флаг `-Realistic` — реалистичная демо-БД «как будто пользовались» (лимиты 5–150, история `/reports`, живые сессии = потреблению); см. `docs/OPERATIONS.md` «Реалистичная демо-БД».
